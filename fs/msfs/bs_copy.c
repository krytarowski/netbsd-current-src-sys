...
)
{
	...
	for (i = 0; i < bfPageRangeCnt; i++) {
		if (bfPageRange[i].pageType == STG) {
			...
			/* It seems like a foul thing to do, to pass in the
			 * address of a private member of the lock structure
			 * to allow a foreign routine to access. It also would
			 * appear that this may be a little late in the
			 * calling tree to be checking this data. It may be
			 * more appropriate to check this much sooner. But
			 * this would require a more thorough investigation of
			 * this code and a clearer understanding of the
			 * intent.
			 * 
			 * LATEST : The reason we're doing this "foul" thing here
			 * is that just before we start the migrate, we check
			 * to see if a truncate has come in. If so, we abort
			 * the migrate (though caller can retry) as an
			 * optimization and let the truncate in. This way we
			 * may end up not having to migrate anything when
			 * retried later. */
...
)
{
	...
	/* FIX - only write pages that were in the original.  Is this taken
	 * care of by the caller??  What I mean is, the caller never specifies
	 * a range that it doesn't want written. */
	/*
	...
	for (i = bfPageOffset; i < (bfPageOffset + bfPageCnt); i++) {
		...
		if (bs_find_page(bfAccess, i, 0) != 0) {
			...
			/* Clear the FLUSH flag to allow other routines to
			 * flush page. */
			sts = bs_clear_bufstate(pgPin, FLUSH);
			if (sts != EOK) {
				RAISE_EXCEPTION(sts);
...
)
{
	...
	/* FIX */
	ADVFS_SAD0("compare_copies --- not yet supported (XXX: dead-code removed)");
	return EOK;
}				/* end compare_copies */
