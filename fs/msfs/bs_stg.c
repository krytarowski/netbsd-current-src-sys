...
#include <fs/msfs/ftx_privates.h>	/* for FTX_MX_PINP */
#include <fs/msfs/bs_ods.h>	/* needed for BS_IS_TAG_OF_TYPE() */
...
alloc_from_one_disk(
    ...
    ftxHT parentFtx,		/* in */
    uint32_t * reservedPagesCount,	/* in/out */
    uint32_t * reservedPagesLocation,	/* in/out */
    uint32_t * allocPageCnt	/* out */
);
...
)
{
	 ...
	 * active. This is a problem since a directory could have run a prior
	 * undo routine (i.e. fs_insert_undo) which causes the in-mem extent
	 * maps to be read in. If this is the case then it is imperative that
	 * we fix them since shortly after this the on-disk MCELLS will be
	 * changed and we will have a mismatch. */
	...
	if (v4bmt_or_rbmt || this_is_v3bmt) {
		/* If this is undoing of storage of either the BMT or the RBMT
		 * then we want to skip this routine. We have already opened
		 * the RBMT/BMT and read in the extent maps. These may not be
		 * totally correct since we are in the middle of recovery but
		 * we should only be accessing those parts of the RBMT/BMT
		 * that have valid extents (since this undo is because of an
		 * extension). At this point we have not actually undone the
		 * storage or fixed up the on-disk extent maps. We can not
		 * just pitch the extent maps as we would for a regular file,
		 * since the subsequent undo routines will need to access the
		 * RBMT/BMT. So we must keep going with the extens we have
		 * until the on-disk xtnts are corrected then we can force
		 * them to be re-read in. */

		goto HANDLE_EXCEPTION;
	}
	if (domain->state != BFD_ACTIVATED) {
		/* We are in recovery, must open the bitfile set.  Remember
		 * that we must close this on the way out. */
		...
		if (bfSetp == NULL) {
			...
			/* fs_create_file_set => create_root_file =>
			 * rbf_add_stg */
			/* then there will be no bfSetp. This undo routine is */
			/* to fix the in-memory bfap nextPage, allocPageCnt
			 * and to */
			/* invalidate cachec UBC pages. Since the set creation
			 * failed, */
			/* the set does not exist and the files don't exist.
			 * There is */
	...
	if (sts != EOK) {
		 ...
		 * all been cleaned up the purpose of this undo routine is to
		 * make sure the in memory extent maps are correct. Since the
...
)
{
	...
	if (domain->state != BFD_ACTIVATED) {
		/* I'm not sure why we need to do this in recovery. I know
...
)
{
	...
	/* If this is a v3 domain allocate a page for the hole. It will be
	 * zero-filled */
...
)
{
	...
	if (bfSetp->cloneSetp != NULL) {
		...
		/* The set has been cloned at least once.  Check to see if we
		 * need to do any 'copy on write' processing. bs_cow will do
		 * something only if this is a sparse allocation. */
...
)
{
	bsInMemXtntT *xtnts;
	bsInMemXtntMapT *xtntMap;
	uint32_t mapIndex, i, pageCnt, temp = 0;
	bsInMemSubXtntMapT *subXtntMap;
	int sts;
	 ...
	 * modified only thru rbmt extension which is done under an exclusive
	 * transaction. The caller must already hold the mcellList_lk */
	 ...
	 * vd->allocClust here.  But I hate adding code that has no other
	 * purpose than cosmetic consistency.  We can't race here with any
	 * other threads since we are in an exclusive domain transaction, so
	 * everyone else is blocked out of the storage allocation paths. */
	...
	/* Since we are adding a subXtntmap to the end of the array we don't
	 * need to call merge_xtnt_maps. Also We are not replacing any mcells
	 * so set origStart to updateStart. */
...
)
{
	...
	case BSXMT_STRIPE:
		...
		while ((i < segmentCnt) &&
			...
			/* There could be a race with migrate and rmvol. If
			 * there is a vdp, bump its vdRefCnt by 1. */
			vdp = vd_htop_if_valid(xtntMap->allocVdIndex,
			    bfap->dmnP,
			    TRUE,
			    FALSE);
			/* If vdp is now NULL, what use to be the preferred
			 * allocation disk no longer exists (it may have have
			 * been rmvol-ed).  If this is the case, reset
			 * xtntMap->allocVdIndex to -1.  The next allocation
			 * will reset it to a proper value.  If not, flag that
			 * we have raised the vdRefCnt by 1.  We will
			 * decrement it after we call alloc_stg(). */
			if (vdp != NULL) {
				vdpCnt_bumped = TRUE;
			} else {
			...
			if (sts != EOK) {
				/* FIX - if run out of blocks, go to new disk
				 * if possible */
				RAISE_EXCEPTION(sts);
			}
...
)
{
	...
	KASSERT(lock_holder(&bfap->xtntMap_lk.lock));
...
{				/* in */
	...
	bsXtntT bsXA[BMT_XTRA_XTNTS];	/* tmp array of disk extent
					 * descriptors */
	int sts;
	int i, j;
	...
	/* save off the extents to tmp that are currently in the new
	 * subxtntmap */
	...
	for (i = 0; i < prevOldSubXtntMap->cnt; i++) {
		if ((i == (prevOldSubXtntMap->cnt - 1)) &&	/* on last extent */
			...
			/* skip the terminator cause next extent in next map
			 * has pages */
			break;
		}
...
)
{
	...
	/* step 2: copy submap where stg to be added TO extra map
	 * (updateStart) */
	sts = imm_copy_sub_xtnt_map(bfap,
	    xtntMap,
	    &xtntMap->subXtntMap[mapIndex],
	...
	/* step 4: 4a.unlink orphan mcell from orig, 4b.setup reuse of mcell
	 * Id, 4c.free from memory the copy 4d.try to merge sub extent maps
	...
	if (newSubXtntMap->updateStart == newSubXtntMap->cnt) {
		...
		if ((newSubXtntMap->cnt == 1) &&
			...
			/* We have an orphan subextentmap. This happens if we
			 * are adding storage into a hole that lives in the
			 * first descriptor of a subextent map and there was
			 * no more storage available. We will remove this from
			 * the update range, unlink it from the on-disk mcell
			 * chain and mark it to be reused after the new
			 * storage descriptors.
			 * 
			 * We will orphan a shadow extent since it is the first
			 * extent in the map. This is legal. It would be too
			 * much headache to try to do otherwise. */
			...
			/* Try to combine previous map contents with new stg
			 * map contents to eliminate another mcell creation. */
			if (mapIndex > 1) {	/* make sure PRIME extent
						 * mcell is not involved */
				...
				/* error just means that combining submaps did
				 * not succeed - we'll just use existing two
				 * maps to describe the stg added */
				if (sts == EOK) {
					/*
	...
	if (newSubXtntMap->bsXA[newSubXtntMap->cnt - 1].bsPage <
		...
		/* copy new tmp hole desc into a subXtntMap after any new stg
		 * maps */
	...
	/* Step 5b: Copy the remaining entries from the split sub extent map
	 * to after hole */
	sts = imm_copy_xtnt_descs(oldSubXtntMap->vdIndex,
	...
	if (!MCID_EQL(reuseMcellId, bsNilMCId) &&
		...
		/* Delete newly alloced mcell, we'll keep the old (mapIndex)
		 * mcell. */
		...
		newSubXtntMap->mcellId = reuseMcellId;	/* this is mapIndex
							 * mcell */
	...
	for (i = xtntMap->updateEnd + 1; i < xtntMap->cnt; i++) {
		...
		if ((!MCID_EQL(reuseMcellId, bsNilMCId)) &&
		    (i == xtntMap->cnt - 1)) {
			/* Reuse the mcell from the orphan. We know this must
			 * go on the last MCELL on the update list. */
...
)
{
	...
	while (totalPageCnt < bfPageCnt) {
		...
		if (stg_type == PERM_HOLE_START) {
			/* We really don't need any storage but we have to
			 * pick a disk so chose one that has at least a page */
...
)
{
	...
	uint32_t reservedPagesCount = 0, reservedPagesLocation = 0;
	...
	if (subXtntMap->vdIndex == bfVdIndex) {
		...
		sts = alloc_from_one_disk(bfap,
		    ...
		    &totalPageCnt);
		/* We must return the pages we successfully allocated even if
		 * we */
	...
	while (totalPageCnt < bfPageCnt) {
		...
		if (BF_IS_BMT(bfap) && reservedPagesCount == 0) {
			/* Accept less than the request for reserved files. If
			 * reservedPagesCount > 0 then alloc_from_one_disk
			 * found the mcell was full. We get a neww mcell in
			 * this loop and use the pages found from the first
			 * call to alloc_from_one_disk. */
			break;
		}
		if (xtntMap->cnt >= xtntMap->maxCnt) {
			sts = imm_extend_xtnt_map(xtntMap);
		...
		totalPageCnt = totalPageCnt + pageCnt;
		/* We need to return this now in case we fail on subsequent
		 * passes */
		/* thru the loop. */
		*allocPageCnt = totalPageCnt;
...
)
{
	...
	while (totalPageCnt < bfPageCnt) {
		...
		if (xtntMap->cnt == 0) {
			...
			/* In this case startPage will be smaller than
			 * bfPageOffset. */
			firstPg = startPage;
		} else {
			prevSubXtntMap = &xtntMap->subXtntMap[xtntMap->cnt - 1];
		...
		/* cp_stg_alloc_in_one_mcell may stop this transaction and it
		 * may */
		/* start another. We pass a pointer to the ftxH so we can know */
		/* if it returned with a transaction in progress or not. */

		if (stg_type == PERM_HOLE_START) {
			/* When migrating a clone special care must be taken
			 * to also migrate any permenant holes. Backing
			 * storage is not required but, the extent record must
			 * be there. */
		...
		if (pageCnt > 0) {
			if (ftxH.hndl) {
				/* We still have a transaction going. Update
				 * the on disk xtnt */
			...
			totalPageCnt = totalPageCnt + pageCnt;
			/* We need to return this now in case we fail on
			 * subsequent passes */
			/* thru the loop. */
			*allocPageCnt = totalPageCnt;
 ...
 */
	...
	if (bfPageOffset == subXtntMap->bsXA[i].bsPage) {
		...
		if (i > 0) {
			if (subXtntMap->bsXA[i - 1].vdBlk != XTNT_TERM &&
				...
				if (BF_IS_BMT(bfap) && *reservedPagesCount == 0) {
					/* We only need one extent for
					 * reserved files. If
					 * reservedPagesCount was passed in >
					 * 0, we will use it here. */
					goto done;
				}
			}
		}
	...
	while (totalPageCnt < bfPageCnt) {
		...
		if (BF_IS_BMT(bfap) && *reservedPagesCount == 0) {
			...
			/* If reservedPagesCount was passed in > 0, we will
			 * use it here. */
			break;
		}
	}			/* end while */
...
)
{
	...
	*allocPageCnt = 0;
	...
	while (totalPageCnt < bfPageCnt) {
		...
		if (sts != EOK) {
			/* The only failure here is a failure to start a new
			 * ftx. */
...
)
{
	...
	/* Currently we are only ever coming in here with a new mcell. but
	 * this is coded to handle a partially full subextent map. */
	...
	if (subXtntMap->bsXA[xI].bsPage < bfPageOffset) {
		/* We may come in here with a trailing hole, which we must
		 * preserve. So use the next extent entry */
...
add_extent(
    ...
    uint32_t blkOff,		/* first block of extent */
    uint32_t pgCnt,		/* extent to be this long */
    ftxHT * ftxHp,
    bsXtntT * termDescp,
    int *pinCntp)
{
...
stg_alloc_one_xtnt(
    ...
    uint32_t * allocBlkOffp,	/* in/out  At this location. */
...
retry:
	...
	if ((stgDesc == NULL) &&
		 ...
		 * the storage reserved for smartstore and then retry the
		 * find.
		 * 
		 ...
		 * aborting the whole migrate cycle and picks a new file. The
		 * file will go back on the frag list when it is next
		 * accessed. */
	...
	/* We can only change so many pages of the SBM at once. If a really
	 * big */
...
xfer_stg_to_xtnts(
    ...
    uint32_t startblk,		/* Storage is at this block */
    vdIndexT vdIndex,		/* on this disk */
    ftxHT parentFtx
)
{
	...
	if (sts == E_PAGE_NOT_MAPPED) {
		...
		/* bfPageOffset is before the first page of the first
		 * subextent or */
		/* after the last page of the last (valid) subextent. However, */
		/* the extent map always starts at 0, if only with a
		 * termination */
		/* extent (vdBlk == -1), so bfPageOffset is after the last
		 * extent. */
		/* Append page after the last page mapped by the last sub
		 * extent map. */
...
xfer_append_stg(
    ...
    u_long bfPageCnt,		/* add this many pages */
    uint32_t startblk,		/* The storage is at this blk */
    vdIndexT vdIndex,		/* on this disk */
    ftxHT parentFtx
)
{
	...
	int sts;
	bsInMemSubXtntMapT *subXtntMap;
	uint32_t mapIndex = xtntMap->validCnt - 1;
...
xfer_hole_stg(
    ...
    u_long bfPageCnt,		/* add this many pages */
    uint32_t startblk,		/* storage is at this blk */
    vdIndexT vdIndex,		/* on this disk */
    ftxHT parentFtx
)
{
	...
	/*
         ...
         * extent map.
         */
	/* truncate extent map, only those subs thru the modified sub are
	 * valid */
	newSubXtntMap->cnt = descIndex + 1;
	KASSERT(newSubXtntMap->cnt <= newSubXtntMap->maxCnt);
	...
	/* this will modify the last subextent and change updateStart &
	 * updateCnt */
	sts = xfer_stg_on_one_disk(bfap,
	    bfSetDesc->dirTag,
	    xtntMap,
	 ...
	 * because a hole cannot be the last thing in a sub map. Test for new
	 * maps added by comparing saved mapcnt above with new mapcnt. */
...
xfer_stg_in_one_subxtnt(
    ...
    uint32_t blkOff		/* from this place */
)
{
	uint32_t i;
	uint32_t nextBlkOff;
	...
	if (bfPageOffset == subXtntMap->bsXA[i].bsPage) {
		...
		if (i > 0) {	/* there exists some pages already */
			if (subXtntMap->bsXA[i - 1].vdBlk != XTNT_TERM &&
				...
				if (blkOff == nextBlkOff) {
					/*
		                         ...
		                         * the previous extent.
		                         */
					/* E.g., add page 2 at block 64 to the
					 * following map, i = 2 page   0   1
					 * 2   ->  0   1   3 block  16  48  x
					 * ->  16  48  -1 */
					/* bsPage & vdBlk at bsXA[i] will be
					 * modified below. */
				} else {
					/*
		                         * The extents are not contigous.
		                         */
					/* E.g., add page 2 at block 80 to the
					 * following map, i = 2 page   0   1
					 * 2   ->  0   1   2   3 block  16  48
					 * x   ->  16  48  80  -1 */
					if (i == (subXtntMap->maxCnt - 1)) {
						/* One entry left. */
						sts = imm_extend_sub_xtnt_map(subXtntMap);
					...
					/* bsPage at bsXA[i] will be modified
					 * below. */
					subXtntMap->bsXA[i].vdBlk = blkOff;
					i++;
				}
				...
				/* E.g., add page 2 at block 80 to the
				 * following map, i = 2 page   0   1   2   ->
				 * 0   1   2   3 block  16  -1  x   ->  16  -1
				 * 80  -1 */
				if (i == (subXtntMap->maxCnt - 1)) {
					sts = imm_extend_sub_xtnt_map(subXtntMap);
					if (sts != EOK) {
			...
			/* E.g., add page 2 at block 80 to the following map,
			 * i = 0 page   x   ->  2   3 block  -1  ->  80  -1 */
			if (i == (subXtntMap->maxCnt - 1)) {
				sts = imm_extend_sub_xtnt_map(subXtntMap);
				/* curently ms_malloc waits, so this can't
				 * fail */
				if (sts != EOK) {
					RAISE_EXCEPTION(sts);
				}
		 ...
		 * page   0   1   ->  0   1   2   3 block  16  -1  ->  16  -1
		 * 80  -1 */
		/*
	         * Is there enough room in the extent descriptor array for two
	         * entries, the hole and the termination extent descriptors?
    ...
    ftxHT ftxH)
{
	...
	if (origXtnts->type == BSXMT_APPEND) {
		...
		if (xtntDesc.volIndex) {
			/* If the page is mapped in the orig, it must be a
			 * hole. */
			...
			holeStart = page;
			/* The page is beyond the end of the orig file, set
			 * holeEnd to */
			/* the max. However, the page is within maxClonePgs to
			 * get here. */
		...
		/* The page is mapped, but it can be off the end of a stripe
		 * extent. */

		if (xtntDesc.volIndex == bsNilVdIndex) {
			/* There'll be a subextent for this stripe even if it
			 * has no stg. */
			KASSERT(origXtntMap->validCnt);
			KASSERT(origXtntMap->cnt);

			/* The hole starts after the last block in this
			 * stripe. */
			subXtntMap = &origXtntMap->subXtntMap[origXtntMap->cnt - 1];
			holeStart = subXtntMap->bsXA[subXtntMap->cnt - 1].bsPage;
			/* Since this page is off the end of the extent map
			 * for this  stripe, the hole ends at the end of the
			 * stripe. */
	...
	if (xtntDesc.volIndex == bsNilVdIndex) {
		...
		if (xtntDesc.blkOffset == PERM_HOLE_START) {
			/* The clone already has a perm hole here. Nothing to
			 * do. */
			RAISE_EXCEPTION(EOK);
		}
...
{
	bfSetT *bfSetp;
	uint32_t lSubI;		/* last subextent index */
	uint32_t uSubI;		/* update subextent index */
	int sts;
	bsInMemSubXtntMapT *subXtntMap;
	...
	if (updateFlg) {
		...
		/* Just checking. */
		/* This should be true except when actively changing the
		 * xtntMap. */
		KASSERT(xtntMap->cnt == xtntMap->validCnt);
		...
		if (uSubI >= xtntMap->maxCnt) {
			/* One more decriptor needed to hold the permanent
			 * hole start. */
			sts = imm_extend_xtnt_map(xtntMap);
			if (sts != EOK) {
				return sts;
		...
		if ((subXtntMap->cnt == 1) && (uSubI == 1)) {
			...
			if ((subXtntMap->mcellId.page == bsNilMCId.page) &&
			    (subXtntMap->mcellId.cell == bsNilMCId.cell)) {
				if (bfap->xtnts.type == BSXMT_STRIPE) {
					/* Get a new mcell on the correct
					 * volume for this stripe. */
					...
					/* Try to get an mcell on the volume
					 * of the prim mcell. */
    ...
    ftxHT ftxH)
{
	...
	/* If the copy above was made from an update sub, and since we have
	 * now duplicated that subMap, cleanup the stg map from add_stg.
	 * Otherwise the copy was made from an orig sub, and we don't want to
	 * delete the orig here. */
	if (updateFlg)
		ms_free(oldbsXA);
    ...
    ftxHT ftxH)
{
	...
	if (holeStart == subXtntMap->bsXA[xI].bsPage) {
		...
		/* The start of the hole will replace the original extent
		 * terminator. */
		...
		if (xI == subXtntMap->maxCnt - 1) {
			...
			if (sts == EXTND_FAILURE) {
				...
				/* The PERMHOLE we are about to create should
				 * not be filling a hole that lives at the end
				 * of an extent map (this is the RULE THAT
				 * HOLES MUST STICK TO THE STORAGE FOLLOWING
				 * IT). We could leave the extent map that is
				 * full alone and use a new one without
				 * initializing pageCnt.
				 * 
				 * However since this on-disk anomaly can exist
				 * in older domains we must deal with it here.
				 * Set the pageCnt in the passed in subxtntmap
				 * to reflect that we may be moving the hole
				 * and therefor this descriptor's terminator
				 * onto the new subxtnt map we are about to
				 * create.  This will have the effect of
				...
				/* The subXtntMap array has changed.
				 * Recalculate subXtntMap. */
				subXtntMap = &xtntMap->subXtntMap[xtntMap->cnt - 1];
				xI = 0;
			}
			if (sts != EOK) {
		...
		/* The hole starts beyond the last extent. We need 2 more
		 * descriptors */
		/* to describe the hole, a hole start and a terminator. */
		xI++;
		if (subXtntMap->maxCnt < 2 || xI > subXtntMap->maxCnt - 2) {
			/* There are not enough free descriptors. We must get
			 * more. */
			if (xI + 1 < subXtntMap->onDiskMaxCnt) {
				/* There are at least 2 descs left in this
				 * subextent (mcell). */
				/* Extend it knowing we will get at least 2
				 * more. */
				sts = imm_extend_sub_xtnt_map(subXtntMap);
				KASSERT(xI + 1 < subXtntMap->maxCnt);
			} else {
				/* There is not enough room in the subextent
				 * (mcell), get */
				/* a new one. */
				KASSERT(subXtntMap->bsXA[subXtntMap->cnt - 1].vdBlk ==
				    XTNT_TERM);
				...
				/* Start the new extent where the last
				 * subextent left off. */
				subXtntMap->bsXA[0].bsPage = lastPage;
				subXtntMap->bsXA[0].vdBlk = XTNT_TERM;
				/* Since the permanent hole is not adjacent to
				 * the last */
				/* extent, this subextent starts with a normal
				 * hole. */
				xI = 1;
			}
			if (sts != EOK) {
...
)
{
	 ...
	 * RBMT extents are only modified under an exclusive transaction.
	 * (Ver. 4 domains only) e. This is the BMT on a Ver. 3 or earlier
	 * domain and this thread already has the BMT's xtntMap lock
	 * write-locked. f. This is not the RBMT, BMT, SBM, LOG, or TAG, but
	 * its xtntMap_lk is already write-locked by this thread and its
	 * xtnt.validFlag is true. (If write-locked and its xtnt.validFlag is
	 * false, return an error). These optimizations also avoid lock
	...
	if (!(this_is_log_file ||
		...
		if (lock_holder(&bfap->xtntMap_lk.lock)) {
			...
			/* Be sure in-memory maps are valid; on successful
			 * return, the xtntMap_lk is READ locked. */
			sts = x_load_inmem_xtnt_map(bfap, X_LOAD_REFERENCE);
			if (sts != EOK) {
				return sts;
			}
...
)
{
	 ...
	 * it during file creation/deletion, and it is migrated under an
	 * exclusive transaction. e. This is the BMT on a Ver. 3 or earlier
	 * domain and this thread already has the BMT's xtntMap lock
	 * write-locked. These optimizations also avoid lock hierarchy
	 * violations. */
	if (!(this_is_log_file ||
		this_is_sbm_file ||
		this_is_rbmt ||
...
)
{
	...
	/* Make sure that we catch all the rogue add/remove storage places
	 * that don't take the migtrunc lock. */
	KASSERT(lock_rwholder(&(bfap->xtnts.migTruncLk)));
	...
	*delList = NULL;
	...
	/* we are deleting since bs_cow starts a subtransaction. A large
	 * number */
	/* of pages to cow would fill the log. Instead we set up the clone
	 * here */
	/* but defer cowing the pages until later. The pages will be put on
	 * the */
	 ...
	 * call to extend the subxtntmaps. Extending the subxtnt maps involves
	 * replacing the subXtntMap pointer in the xtntMap.
	 * 
	...
	if (relQuota && bfap->bfVp) {
		 ...
		 * the original page, the quota for the frag is applied here
		 * by not removing those blocks.  note we may also have
		 * additional preallocated pages being removed too. */
...
)
{
	...
	if (emptyFlag == 0) {
		...
		if (subXtntMap->vdIndex != xtntMap->subXtntMap[xtntMap->origStart].vdIndex) {
			/* We can not reuse the first MCELL. We know that the
			 * origStart is not the PRIMARY otherwise the indexes
			 * would have matched */
			uint32_t rType = xtntMap->subXtntMap[xtntMap->origStart].type;
...
)
{
	...
	if (endPageOffset < (subXtntMap->bsXA[0].bsPage)) {
		/* This is a case where the originalEnd range has no pages in
		 * the removal range. This is because the hole being created
		 * needs to stick to the storage in this subextent. This made
		 * it onto the delete list since the MCELL must go but not any
		 * of the pages. Mark the range as a one entry termination
		 * descriptor with no corresponding storage. NOTE: This is one
		 * of the few places that a subextent can contain only a
		...
		/* Set it up so that the next to last is used for the end
		 * range */
...
)
{
	...
	}			/* end else update subextents in middle of
				 * subextent array */
