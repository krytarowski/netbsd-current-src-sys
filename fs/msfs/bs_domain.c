...
start:
	...
	if (!AdvfsFixUpSBM && dmnMAttr.recoveryFailed) {
		if (isRoot) {
			/* If we are activating the root domain, allow
			 * mounting so the system can still boot */
			...
			/* If not the root, then don't allow the activation to
			 * occur */
...
start:
	...
	for (vdi = 0; vdi < dmntbl->vdCount; vdi++) {
		...
		if (sts != EOK) {
			...
			if (global_rootdev[vdi] == global_rootdev_opinprog) {
				...
				goto get_attrs_error;	/* failover handle the
							 * error */
		}
		if (vdAttrp->state == BSR_VD_VIRGIN) {
			dmnState = BFD_VIRGIN;
	...
	/*
         ...
         */
	if (!AdvfsFixUpSBM && dmnMAttr.recoveryFailed) {
		/* If we are activating the root domain, allow mounting so the
		 * system can still boot */
		ms_uprintf("ADVFS : Domain %s activated despite an inconsistency\n", domainName);
		ms_uprintf("found in the AdvFS SBM metadata file.\n");
		ms_uprintf("Recommend you follow the procedures described\n");
		ms_uprintf("in the 'Trouble Shooting: Can't Clear Bit Twice Error\n");
...
)
{
	...
	ss_put_rec(dmnP);	/* ignore error, must not hold up ss
				 * deactivate */
	...
	vdCnt = 0;
	for (vdi = 1; vdCnt < dmnP->vdCnt && vdi <= BS_MAX_VDI; vdi++) {
		/* The DmnTblLock prevents racing addvol/rmvol; no need to
		 * bump the vdRefCnt here. */
		if ((vdp = vd_htop_if_valid(vdi, dmnP, FALSE, FALSE))) {
			vdCnt++;
...
)
{
	...
	/* alignment problem of dmnP->bfDmnMntId, not necessarily 64-bit
	 * aligned */
	TIME_READ(ltime);
...
)
{
	...
	if (RBMT_THERE(dmnP)) {
		 ...
		 * record and use this to set vdp->lastRbmtPg.  This value is
		 * the number of (mcells-1) since the first mcell is on page
		 * 0. */
...
read_n_chk_last_rbmt_pg(vdT * vdp)
{
	...
	if (nextfreeMCId.cell == 0 && nextfreeMCId.page == 0 &&
		 ...
		 * after reboot, restart the rbmt extension. Note that on any
		 * other page, cell 0 may be a valid free cell. */
...
)
{
	 ...
	 * get the OTYP_MNT flag if the open/close was done as the result of a
	 * mount/unmount. While OSF/1 can pass a flag parameter to device
	 * open/close routines, it is only supported in spec_open() and
	 * spec_close(). These are the functions invoked via VOP_OPEN and
	 * VOP_CLOSE for device special files. Therefore, we need to inform
...
)
{
	...
	/* Initialize the length limit of the device queue to a small, finite
	 * value.  This value will be adjusted later as we sample the I/O
	 * completion rate or get I/O statistics from the driver. vd_lbolt
	 * will only be used if the driver does not provide feedback. */
	...
	if (i != 0 || geomst.geom_info.prefer_trans == 0 ||
		...
		/* If a driver returns a preferred IO byte transfer size as
		 * large as the threshold or more, then assign a lower default
		 * size. This keeps readahead or write consolidation from
		 * consuming too much CPU or memory resouces to process a
		 * large transfer. */
...
)
{
	...
	mutex_init(&vdp->vdIoLock);
	mutex_init(&vdp->vdStateLock);
	...
	mutex_init(&vdp->consolQ.ioQLock);
	...
	mutex_init(&vdp->blockingQ.ioQLock);
	...
	mutex_init(&vdp->ubcReqQ.ioQLock);
	...
	mutex_init(&vdp->waitLazyQ.ioQLock);
	...
	mutex_init(&vdp->readyLazyQ.ioQLock);
	...
	mutex_init(&vdp->smSyncQ11.ioQLock);
	...
	mutex_init(&vdp->smSyncQ12.ioQLock);
	...
	mutex_init(&vdp->smSyncQ13.ioQLock);
	...
	mutex_init(&vdp->smSyncQ14.ioQLock);
	...
	mutex_init(&vdp->smSyncQ15.ioQLock);
...
)
{
	...
	/* reclaim any old reserved CFS vnodes that are hanging around on
	 * vnode free list for any of the reserved files on the vd */
	sts = bs_reclaim_cfs_rsvd_vn(vdp->sbmp);
	if (sts != EOK) {
		RAISE_EXCEPTION(sts);
	}
	...
	/* Checkpoint log now so there are no undo or redo log records for the
	 * bitfiles on this disk.  This prevents a recovery error if we crash
	 * between this point and vd_remove(). */
	lgr_checkpoint_log(dmnP);
...
)
{
	...
	while (sts == EOK) {
		...
		nullvp = NULL;
		if (clu_is_ready()) {
			/* If we are in a cluster we need to make this appear
			 * like an external open (ie we need to get a vnode).
			 * Clusters requires the vnode in order to synchronize
			 * with clients that may have this files xtnt map
			 * cached. The migrate call will revoke the CFS token
			 * but rquires a vnode to do this. Since metadata
			 * files can not have cached extents don't bother
			 * getting a vnode for these. */
...
)
{
	 ...
	 * from the service class before the system call to actually remove
	 * the volume.  However, this is still needed in the domain
	 * deactivation path. */
	(void) sc_remove_vd(dmnP, vdp->serviceClass, vdp->vdIndex);

	/* on normal umount or rmvol, change the on-disk state.  being unable
	 * to do this vd update does not affect the rest of the domain and it
	 * is safer to allow the caller to continue for cluster failover. if
	 * the domain has paniced then we can not update vd state on disk.
	 * 
	...
	if (!flags || dmnP->dmn_panic)	/* don't report deactivating a paniced
					 * dmn */
		sts = EOK;
	else {
		int mcell_index;
		...
		if (sts == EOK) {
			...
			if (vdAttr == NULL) {
				...
				/* for rmvol clear the disk's mount id so that
				 * it cannot be brought back into the domain. */
		...
		/* On permanent removal, try to checkpoint the log so there
		 * can be no undo or redo for bitfiles on this disk.  This is
		 * a no-op for a paniced domain but valid even if the vd is
		 * inaccessible. NOTE: with addition of lgr_checkpoint_log()
		 * call in bs_vd_remove_active(), this call may no longer be
		 * needed. */
		if (flags & VD_REMOVE_PERM)
		...
		if ((dmntbl->vdCount > dmnP->vdCnt) && (dmnTAttr->op == BSR_VD_RMVOL)) {
			...
			for (vdi = 1; vdCnt < dmnP->vdCnt && vdi <= BS_MAX_VDI; vdi++) {
				...
				if (vdp->devVp->v_rdev == dmnTAttr->dev) {
					...
					/* change vd on-disk state so it is
					 * permanently removed and
					 * update/remove corresponding
					 * in-memory data. */
					vd_remove(dmnP, vdp, VD_STATE_CHANGE | VD_REMOVE_PERM);
...
)
{
	...
	if (clu_is_ready()) {
		ulong domainId_cast;
		/* We are passing the actual domainId instead of a pointer to
		 * it. Rather than try to cast a non-scalar just copy it into
		 * a scalar container. See the timeout callers to see why. */
...
)
{
	...
	*dmnP = nilDomain;
	...
	mutex_init(&dmnP->mutex);
	mutex_init(&dmnP->lsnLock);
	mutex_init(&dmnP->vdpTblLock);
	...
	advfs_cv_init(&dmnP->ftxTbld.slotCv);
	advfs_cv_init(&dmnP->ftxTbld.trimCv);
	advfs_cv_init(&dmnP->ftxTbld.excCv);
	...
	if (DmnSentinelP != NULL) {
		/* here: we already have a domain(s) in the list, link it to
		 * the end of the chain, this leaves DmnSentinelP as the first
		 * mounted advfs domain (possibly root ) */
		...
		/* here: must be first advfs domain -  link to itself and set
		 * DmnSentinelP */
...
)
{
	...
	if (TEST_DMNP(dmnP_start) == EOK) {
		/* we have a bucket with domains - scan the chain for
		 * bfDomainId */
		dmnP = dmnP_start;
		do {
...
)
{
	...
	while (!eofflag) {
		...
		dirEntOffset = 0;	/* set offset to first byte in the dir
					 * block */
		...
		if (vp->v_mount->m_flag & M_NEWRDDIR) {
			...
			while (dirEntOffset < dirbuflen) {
				...
				if (vdi == BS_MAX_VDI) {
					/* too many device entries in the
					 * directory */
					error = EINVAL;
					goto _error;
				}
				...
				if (vddp == NULL) {
					if (error != EINVAL) {
						/* If error is ENODEV return
						 * error so problem can be
						 * corrected */
						goto _error;
					}
				} else {
			...
			while (dirEntOffset < dirbuflen) {
				...
				dirEntOffset += dp->d_reclen;	/* move to next dir
								 * entry offset */
	...
	if (dmnDescp->vdCount < 1) {
		/* Hmmm...  No device link names where found in the
		 * directory!? */
...
)
{
	...
	*vdd = nilVdDesc;
	...
	/* Follow the link to see if the device special file has been create.
	 * The device could have been moved. If the file does not exist report
	 * ENODEV. Exit through _error so a NULL vdd pointer is returned. */
	ndp->ni_nameiop = LOOKUP | FOLLOW;
	ndp->ni_segflg = UIO_SYSSPACE;
	ndp->ni_dirp = vdName;
	...
	vdd->device = ndp->ni_vp->v_rdev;	/* save for
						 * advfs_enum_domain_dev_ts() */
			...
			while (!done && dirEntOffset < dirbuflen) {
				...
				if (strcmp(kdp->kd_name, domain) == 0) {
					...
					*dmnMajor = ent | 0x00000800;
					done = eofflag = TRUE;	/* done -- terminate
								 * loops */
				}
			}
		} else {
	...
	if (VDI_IS_VALID(vdi, dmnP)) {
		...
		if ((vdp->vdState == BSR_VD_MOUNTED) ||
			...
			(vdp->vdState == BSR_VD_VIRGIN ||
			    vdp->vdState == BSR_VD_ZOMBIE ||
			    vdp->vdState == BSR_VD_DMNT_INPROG))) {
			/* If requested, bump the refcnt so that this vdT
			 * can't be deallocated until all threads stop access
			 * to it. */
			if (bump_refcnt)
				vdp->vdRefCnt++;
			mutex_unlock(&vdp->vdStateLock);
