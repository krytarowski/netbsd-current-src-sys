...
typedef struct actRangeHdr {
	struct actRange *arFwd;	/* active: pointer to next higher range */
	struct actRange *arBwd;	/* active: pointer to next lower  range */
	struct actRange *arWaitFwd;	/* waiters: pointer to next waiter
					 * (fifo) */
	struct actRange *arWaitBwd;	/* waiters: pointer to prev waiter
					 * (fifo) */
...
typedef struct bfAccess {
	...
	saved_statsT *saved_stats;	/* if non-null; points to struct with
					 * stats that need to be flushed to
					 * disk; this file no longer has an
					 * fsContext structure (it was
					 * deallocated when its vnode was
					 * reclaimed.) */

	/* Following fields modifed in xxx_TRANS state or when holding the
	 * bfaLock */
	struct vnode *bfVp;
	...
	lsnT logWriteTargetLsn;	/* Log bitfile's target LSN to flush & sync
				 * to. */
	...
	unsigned long raStartPage;	/* When read ahead, start read with
					 * this page */
	unsigned long seqWritePgCnt;	/* Track # of sequentially written
					 * pages */
	krwlock_t trunc_xfer_lk;	/* prevent clone reads during orig
					 * truncation */
	krwlock_t cow_lk;

	krwlock_t clu_clonextnt_lk;	/* clusters only lock protecting clone
					 * xtnts */
	...
	struct bfAccess *origAccp;	/* link to orig bitfile's access
					 * struct */
	unsigned long cowPgCount;	/* Protected by the migTruncLk; number
					 * of */
	...
	unsigned deleteWithClone;	/* true if bf should be deleted with
					 * clone */
	...
	/* Analysis of the code reveals that the following locks seem to be
	 * protecting the xtntmap in the following way:
	 * 
	 * trunc_xfere_lk - used for clone access migTruncLk - Held across add,
	 * removing, migrating. mcellList_lk - Protects the on-disk Mcells
	 * exclusively but also allows read access to the xtntMap.
	 * 
	 * The following lock gives exclusive access to the xtntMap
	 * 
	 * xtntMap_lk - Must be held when merging xtntmaps and across any call to
	 * imm_extent_xtnt_map.
	 * 
	 * NOTE: The use of these appears to be fairly inconsistent and needs to
	 * be investigated.
	 * 
	 */
	 ...
	 * to synchronize manipulation of a given file region without
