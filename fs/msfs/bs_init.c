...
)
{
	...
	if (BS_BFTAG_EQL(bfDmnParams->bfSetDirTag, bfSetDirTag)) {
		...
		/* this is not the tagdir's disk so don't allocate any pages
		 * to it */
		tagBlks = 0;
	}
	...
	*dmnAttrp = bsNilDmnAttr;
	...
	if (BS_BFTAG_EQL(bfDmnParams->ftxLogTag, ftxLogTag)) {
		...
		/* Try to put a BSR_DMN_FREEZE_ATTR on the disk. This record
...
)
{
	...
	*dmnAttrp = bsNilDmnAttr;
	...
	/* If so, set up BFM_RSVD_CELLS_V3 mcell for a BMT extent and
	 * pre-allocate the specified amount */
	...
	if (bmtPreallocPgs != 0) {
		...
		/* Assign and set up a new extent record for preallocation
		 * pages */
		/* These pages will be allocated after the SBM pages */
