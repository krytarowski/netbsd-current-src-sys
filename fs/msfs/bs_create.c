...
)
{
	 ...
	 * into the FTX_TRANS state, therefore no additional locks appear to
	 * be necessary.  The root-done action will change the state of the
	 * mcell, as well as take the bsaccess structure out of FTX_TRANS, if
	 * some other thread has got it there. */
...
)
{
	...
	*odattrp = *bfAttr;
	...
	/* If the first extent is being stored in the primary mcell,
	 * initialize the first extent fields. */
