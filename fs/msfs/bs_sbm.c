...
)
{
	...
	ftx_lock_write(&vdp->stgMap_lk, ftxH);
	/*
	...
	*found_blks = 0;
	...
	if (alloc_hint == BS_ALLOC_MIG_RSVD) {
		...
		if ((cur_desc == NULL) ||
			...
			/* Problem is that the cleared space is not free for
			 * some reason, probably because we are running out of
			 * space and the system has unlocked the smartstore
			 * lock range.  We return a NULL if we haven't got the
			 * storage exactly where we wanted it. Caller will
			 * handle the condition of not enough of clusts. */
			return (NULL);
		}
	...
	if (alloc_hint == BS_ALLOC_RSVD) {
		/* For reserved files use the best extent: cur_desc or
		 * freeRsvdStg. */
		if (cur_desc == NULL) {
			if (vdp->freeRsvdStg.num_clust > 0) {
				cur_desc = &vdp->freeRsvdStg;
...
)
{
	...
	if (num_clust == 0) {
		/* normally this would be an error.  however, due to a bug in
		 * V1.0 we really need to just ignore the odd-size free space
		 * chunks on the disk.  they will eventually merge with
...
)
{
	...
	if (clustOffset == stgDesc->start_clust) {
		...
		/* insert stgDesc for the first part of the replacement
		 * descriptor */
		...
		/* insert stgDesc for the second part of the replacement
		 * descriptor */
...
)
{
	...
	uint32_t
	    ...
	    endPageWholeWordCnt,/* number of whole words in last page of
				 * search */
	    endPageBitCnt;	/* number of bits in last word of last page of
				 * search */
	int endPageVisits,	/* # of times we have examined the last page
				 * of search */
	    reqEndPageVisits;	/* # of times we must examine the last page of
				 * search */
	...
	KASSERT(lock_holder(&vdp->stgMap_lk.lock));
	...
	endPageVisits = 0;
	if (endPage == pg) {
		if (end_clust >= curClust) {	/* curClust still starting
						 * clust */
			/*
	...
	while (TRUE) {
		...
		if ((pg == (vdp->bitMapPgs - 1)) && (wholePgs < vdp->bitMapPgs)) {
			...
			/* continue using MIG_RSVD_WORD macro so that if a
			 * cluster is found that is big enough here, it won't
...
)
{
	...
	/* The two lines below needed due to checking overlap in add_cache()
	 * from load_x_cache() */
...
)
{
	...
	uint32_t
	    ...
	    rangeEndPageWholeWordCnt,	/* ending page word cnt for range
					 * entered */
	...
	/* check for not enough space in sbm for range - if not, return
	 * ENO_SPACE */
...
)
{
	...
	/* FIX - UPDATE to CHECK for rsvd tags in the way when version of sbm
	...
	for (pg = startPg; pg <= sbmLastPg;) {
		...
		for (wd = startWd; wd < wordCnt; (wd)++) {
			for (bit = startBit; bit < SBM_BITS_LONG; bit++) {
				...
				if (whoami == SS_PARENT) {
					if (*clustRunCnt != 0) {
						/* Found a valid run. Jump to
						 * the start of the next page */
						wd = wordCnt;
						break;
					} else
						/* no Run ended on this page,
					    running out of space */ if (bmt_rsvd_end > 0) {	/* still some rsvd
												 * space, jump it */

						/* There is a bmt reserved
						 * range in the way. */
						/* Jump to the start of the
						 * first bit after the rsvd
						 * range */
			...
			if (whoami == SS_PARENT) {
				if (bestClustCnt) {
					...
					*blkOffset = savedStartClust * vdp->stgCluster;
					...
					/* not enough space was found, return
					 * ENOSPC */
					*clustRunCnt = 0;
					*blkOffset = 0;
					return (ENOSPC);
				...
				if (free_clust >= reqClustSize) {
					*clustRunCnt = curr_clust_cnt;
					...
					/* not enough space was found, return
					 * EOK if we need to keep going cause
					 * we ran into rsvd, return ENOSPC if
					 * we ran to the end of sbm and don't
					 * have enough */
					*clustRunCnt = 0;
					if (overlaps == TRUE)
						sts = EOK;
