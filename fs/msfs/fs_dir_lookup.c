...
insert_seq(
    ...
    int flag,			/* in - HARD_LINK - entry is being created for
				 * a hard link to an existing file, so BMT the
				 * stats START_FTX - create a subtransation */
...
reuse_entry:
		...
		/* save the old size so that the re-used entry can be split up
		 * later */
		...
		if (IDX_INDEXING_ENABLED(dir_access)) {
			/* We know that the dir is safe since we are holding
			 * the write lock so set up the time stamps to match
			 * and jump back up. */
...
add_stg:
	...
	if ((dir_access->dmnP->dmnVersion >=
		 ...
		 * hasn't dipped back to 1 page. If so the index is already in
		 * place. This may go away if removal of index files below a
		 * threshold is implemented */

		/* The directory can now benefit from an index file so if this
		 * is a newer domain do the conversion to indexing */
	...
	if (IDX_INDEXING_ENABLED(dir_access)) {
		 ...
		 * tree We need to do this prior to pinning any records in
		 * case we fail to put the space into the b-tree. */
	...
	/* reset the buffer pointers and setup for the new entry on the new
	 * page */
	insert_here = (char *) dir_buffer;
	begin_page = (char *) dir_buffer;
...
insert_entry:
	...
	if ((!extended_dir_here) && dirTruncEnabled) {
		...
		if (IDX_INDEXING_ENABLED(dir_access)) {
			/* Since the truncation can fail due to lack of
			 * storage (how odd?) first query the index for the
			 * number of byte to truncate the after the removal is
			 * successful, update the index */
			...
			num_useful_pages = ndp->ni_endoff;
...
)
{
	 ...
	 * calls after. This basically tells us what the entry size will look
	 * like after the glom. We will put this size in the btree. This may
	 * also pick up any space that may have been lost on a previous remove
	...
	if (IDX_INDEXING_ENABLED(dir_access)) {
		...
		if (ret != EOK) {
			if ((ret == ENO_MORE_BLKS) || (ret == ENO_MORE_MCELLS)) {
				...
				/* The insertion to the b-tree caused a node
				 * to split, which in turn needed to obtain
				 * space for which there was none. This is
				 * really bad luck. We do not want to return
				 * to the caller an out of space error when
				 * they are removing a file! So ignore the
				 * error and let the space be lost. We will
				 * flag it on disk as lost so that a later
				 * glom has a chance of picking it up. */

				ret = EOK;

				/* Reset the entry pointer in case glom
				 * changed it */
				 ...
				 * split) for the b-tree. This is because a
				 * glom means the tree only needed to have an
				 * existing entry updated instead of a new
				 * entry inserted (only an insert can cause a
				 * node to split). Unless there is lost space
				 * next to it. Then we could have this
				 * situation. Since we are about to lose this
				 * space turn off any glomming. */
				...
				/* Return the error and let the caller fail
				 * the ftx, */
				return (ret);
			}
		}
...
seq_search(
    ...
    fs_dir_entry ** found_buffer,	/* out - if found:  buffer address of
					 * page on which entry was found */
    rbfPgRefHT * found_page_ref,/* out - if found:  page ref for found_buffer
				 * if under ftx */
    int flag,			/* in - flag: PIN_PAGE - pin pages searched,
				 * REF_PAGE - ref pages searched, USE_OFFSET -
				 * use the offset hint */
    ftxHT ftx_handle		/* in - transaction handle */
)
{
	...
	if (IDX_INDEXING_ENABLED(dir_access)) {
		...
		/* Callers of seq_search expect only 4 return codes. catch and
		 * others here and return EIO instead. Presumably we have
		 * already domain paniced in the index code and are sending up
		 * the error. */
		...
		/* Fall through if the index is out of sync. The directory may
		 * well be out of sync too but give the look up a chance of
		 * maybe seeing the directory without the index. */
	}
...
dir_loop:
	...
	for (j = saved_page; j < last_page; j++) {
		...
		if (numDirPasses == 0 && (saved_page || saved_offset)) {
			...
			/* just in case saved_offset isn't on a block
			 * boundary, set dir_p to the beginning of the
			 * 512-byte dirblock. */
			p = (char *) dir_p;
			...
			if (dir_p->fs_dir_header.fs_dir_bs_tag_num == 0) {
				...
				if ((dir_p->fs_dir_header.fs_dir_namecount ==
					...
					*found_buffer = save_buffer;
					...
					/* count how we found this, i.e. via
					 * numDirPasses */
					seq_search_stats.pass_hit[numDirPasses]++;
					break;
				} else {
					/* Remember the location of this
					 * useful data. We are iteratively
					 * finding the last piece of useful
					 * data in the directory so that we
					 * can potentially truncate empty
					 * pages beyond.  'j' contains current
					 * page number. */
	...
	/* we're at the end of the dir;  so if we're not rescanning i.e.
	 * numDirPasses==2, let's remember the last page. */
	if (numDirPasses < 2 && return_value != I_FILE_EXISTS) {
		...
		if (numDirPasses == 1) {	/* we originally started from
						 * saved */
			++numDirPasses;
			last_page = saved_page + 1;
			/* XXX we could further optimize by only searching up
			 * to our XXX starting saved_offset. */
			saved_page = 0;
			goto dir_loop;	/* rescan from beginning (to
					 * saved_page) */
		}
	}
...
)
{
	 ...
	 * 
	 * The potential danger path is rbf_delete --> chk_blk_quota --> dqsync
	 * --> rbf_add_stg but it's tame here. */
	ret = FTX_START_N(
	    FTA_OSF_REMOVE_V1,
	    &ftx_handle,
	    FtxNilFtxH,
...
)
{
	 ...
	 * rbf_add_stg this is a VERY unlikely scenario: this call would not
	 * normally add storage to the quota files, but this function is
	 * called when verify finds something broken - in which case, the
	...
	if (IDX_INDEXING_ENABLED(dir_access)) {
		...
		if (ret != EOK) {
			if ((ret == ENO_MORE_BLKS) || (ret == ENO_MORE_MCELLS)) {
				/* if there wasn't enough space to insert the
				 * space then ignore the error. It may get
				 * picked up later */
		...
		if (prev_p->fs_dir_header.fs_dir_bs_tag_num == 0) {
				...
				if ((int) tagp->seq == -1) {
					if (*flags & SKIP_LOST_SPACE_GLOM) {
						...
						*flags |= LOST_SPACE_FOUND;
						...
						/* We found some lost space.
						 * See if we are allowed to
						 * reclaim it. */
						/* We need to gather up all
						 * space that can be glommed
						 * This includes unlost free
						 * space as well as more lost
						 * space. Although not very
						 * efficient, the rarity of
						 * this code being excersized
						 * justifies it.
						 * 
						 * We keep looking until we
						 * either hit used space,
						 * unlost space (which we will
						 * pick up) or the start of a
						 * 512 byte boundary. */
						while (1) {
							dir_pp =
			...
			if (!skip_glom) {
				*entry_size += prev_p_size;
				...
				/* if the newly-deleted entry was the last
				 * entry and we just glom'ed it, then inform
				 * the caller we need to re-set the
				 * lastEntry_offset for the page */
	...
	if ((((char *) post_p - dir_buffer) % DIRBLKSIZ) != 0) {
		if (post_p->fs_dir_header.fs_dir_bs_tag_num == 0) {
			...
			if (IDX_INDEXING_ENABLED(dir_access)) {
				tagp = GETTAGP(post_p);
				if ((int) tagp->seq == -1) {
					/* We found some lost space. See if we
					 * are allowed to reclaim it */
					if (*flags & SKIP_LOST_SPACE_GLOM) {
						skip_glom = TRUE;
					} else {
						...
						*flags |= LOST_SPACE_FOUND;
						...
						/* Same deal as for the
						 * previous entries. We need
						 * to check for glommable
						 * entries up until we find
						 * real free space or used
						 * space. */
...
)
{
	...
	if (flags & LOST_SPACE_FOUND) {
		/* If this glommed entry contains some lost space then
		 * unconditionaly reset the hidden tag field just in case the
		 * lost space was tacked on to the end */
    ...
    ftxHT ftx_handle)
{
	...
	contextp = VTOC(dvp);
	KASSERT(lock_holder(&contextp->file_lock));

	/* update the size of the file in the bfAccess */
	old_file_size = bfap->file_size;
	bfap->file_size = file_size_in_bytes;

	/* Call bf_setup_truncation() to start the truncate process. Note: no
	 * one can have any pages to be truncated REF'd or PIN'd! */
	...
	if (delCnt) {
		if (!bfap->dirTruncp) {
			...
			if (!dtinfop) {
				return (delCnt);	/* It'll get deleted at
							 * reboot time */
			}
		} else {
		...
		/* not seizing a mutex since dir is locked and only these 2
		 * routines manipulate this value */
