...
cvT ss_stopped_cv;		/* condition variable used when shutting ss
				 * system down */
...
#define INC_SSTHDCNT( _dmnP )  \
    ...
    mutex_enter( &(_dmnP.mutex)->ssDmnInfo.ssDmnLk );  \
    (_dmnP)->ssDmnInfo.ssDmnSSThdCnt++;  \
    mutex_exit( &(_dmnP.mutex)->ssDmnInfo.ssDmnLk );
...
ss_kern_init()
{
	...
	mutex_init(&ssStoppedMutex.mutex, MUTEX_DEFAULT, IPL_NONE);
...
ss_kern_stop()
{
	...
	if (SS_is_running == TRUE) {
		...
		/* Notify any threads working on files or waiting to work on a
		 * file of shutdown.  */
...
ss_queues_create(void)
{
	...
	/* Create a message queue to send messages to the list worker thread
	 * pool.  */
	...
	/* Create a message queue to handle messages to the worker thread
	 * pool.  */
...
ss_boss_init(void)
{
	...
	mutex_init(&ssListTpool.plock.mutex, MUTEX_DEFAULT, IPL_NONE);
	mutex_init(&ssWorkTpool.plock.mutex, MUTEX_DEFAULT, IPL_NONE);
...
ss_boss_thread(void)
{
	...
	mutex_enter(&ssListTpool.plock.mutex);
	ssListTpool.threadCnt = 0;
	ssListTpool.shutdown = FALSE;
	mutex_exit(&ssListTpool.plock.mutex);
	...
	mutex_enter(&ssWorkTpool.plock.mutex);
	ssWorkTpool.threadCnt = 0;
	ssWorkTpool.shutdown = FALSE;
	mutex_exit(&ssWorkTpool.plock.mutex);
	...
	while (TRUE) {
		...
		case SS_MONITOR_STOPPED:
			...
			if ((SS_is_running == FALSE) &&
				...
				/* notify message sender that shutdown is
				 * completed */
...
ss_monitor_thread(void)
{
	...
	while (TRUE) {
		...
		if (SS_is_running == FALSE) {
			/* send message to boss thread letting it know we are
			 * all done */
			if ((bmsg =
		...
		/* hot list msg queues throttle algorithm - We need to vary
		 * the number of messages produced in order to prevent message
		 * flooding.  The sample period determines how many messages
		 * were dropped as a result of too many messages on q.
		 * Thresholds for producing messages are adjusted here to
		 * either reduce or increase the number of messages for each
		 * domain with vfast enabled. */
		if (msg_rev_cnt++ >= SS_MSG_CHK_REVS) {
			 ...
			 * ssDmnInfo.ssAccessThreshHits to allow message
			 * delivery that does not exceed the global list msgq
			 * depth. Send message to boss thread instead of doing
			 * it here to avoid lock hierarchy issues */
			if ((bmsg =
				(ssBossMsgT *) msgq_alloc_msg(ssBossQH)) != NULL) {
	...
	while (TRUE) {
		...
		if (ssWorkTpool.shutdown == TRUE) {
			...
			if (ssWorkTpool.threadCnt > 0) {
				 ...
				 * we the last thread in the pool, then die */
	...
	while (TRUE) {
		...
		if (ssListTpool.shutdown == TRUE) {
			...
			if (ssListTpool.threadCnt > 0) {
				...
				/* Send message to boss thread letting it know
				 * we are the last thread and we are going to
				 * die. */
				if ((bmsg =
					(ssBossMsgT *) msgq_alloc_msg(ssBossQH)) != NULL) {
					bmsg->msgType = SS_POOL_STOPPED;
		...
		case FRAG_UPDATE_ADD:
			...
			/* Put this fragmented file onto the list so that it
			 * can be defragmented later by vfast. */
...
ss_adj_msgs_flow_rate()
{
	...
	do {
		...
		if ((SS_is_running == TRUE) &&
			...
			/* get the total active domains and total msgs sent
			 * and total messages dropped so we can determine the
			 * average target message rate per domain and adjust
			 * individual domains flow rates based on this average
			 * below. */
			activeDmnCnt++;
	...
	/* Now adjust the hot msgs threshold level on each domain.  This will
	 * cause those domains with fewer than the average to increase their
	...
	do {
		...
		if ((dmnP->ssDmnInfo.ssDmnSmartPlace == TRUE) &&
			...
			/* adjust each active dmn's hot msgs to achieve
			 * average raising hot msgs threshold(hit cnts)
			 * reduces msgs lowerint hot msgs threshold reduces
			 * msgs adjust thresholds no lower than
			 * SS_INIT_FILE_ACC_THRESH */

			if (totListMsgDropCnt > 0) {
				/* list is dropping some - too many msgs */
				if (dmnP->ssDmnInfo.ssDmnListMsgSendCnt >
				    desiredAveMsgCnt) {
					/* reduce the hot msgs on the hottest
					 * domains  by increasing the
					 * threshold */
				...
				/* list is not dropping msgs - balance out msg
				 * flows from domains */
				if ((totListMsgSendCnt > SS_MAX_MSGS_ALLOWED) &&
				    (dmnP->ssDmnInfo.ssDmnListMsgSendCnt >
					desiredAveMsgCnt)) {
					/* reduce the hot msgs 12.5% (raise
					 * threshold) for domains over the ave
					 * -or- whenever a domain exceeds the
					 * per domain max msg rate allowed */
					dmnP->ssDmnInfo.ssAccessThreshHits =
					    dmnP->ssDmnInfo.ssAccessThreshHits +
					    (dmnP->ssDmnInfo.ssAccessThreshHits / 8);
				} else {
					/* Increase the hot msgs 12.5% (lower
					 * theshold) for domains equal to or
					 * under the ave, not less than
					 * SS_MIN_FILE_ACC_THRESH though */
					dmnP->ssDmnInfo.ssAccessThreshHits =
...
ss_do_periodic_tasks(ssPeriodicMsgTypeT task)
{
	...
	do {
		...
		if ((SS_is_running == TRUE) &&
			...
			case SS_HOT_LIST_CHK:
				if ((dmnP->ssDmnInfo.ssDmnHotHdr.ssHotListCnt > 0) &&
					...
					if ((sts == EOK) && (dstVdIndex != -1)) {
						/* Bump the ref count on this
						 * vd so it cant be removed. */
						dvdp = vd_htop_if_valid(dstVdIndex, dmnP, TRUE, FALSE);
						if (dvdp != NULL) {
							vdRefed = TRUE;
						}
						/* else - must be an rmvol
						 * going on - skip on thru */

						/* Check for a hot file to be
						 * migrated on this vol
						 * already. */
						if ((vdRefed == TRUE) &&
						    (BS_BFTAG_EQL
			...
			case SS_LULL_CHK:
				/* Search for inactivity on any vds. If
				 * detected, signal vfast to start work on
				 * that vd. */
				for (vdi = 1;
			...
			case SS_FRAG_CHK:
				...
				if (dmnP->ssDmnInfo.ssDmnDefragment == TRUE) {
					...
					while (ffilep != NULL) {
						...
						/* ignore errors - rechecks
						 * next time in routine */
		...
		if (dmnP != DmnSentinelP) {	/* if we've wrapped - don't
						 * bump */
			++dmnP->dmnAccCnt;	/* add our reference */
		}
		mutex_exit(&DmnTblMutex.mutex);
...
ss_dmn_deactivate(domainT * dmnP, int flag)
{
	 ...
	 * domain open during a vfast migrate. This broadcast is also needed
	 * in vd_remove so that a rmvol forces vfast to release the vol so
	 * that vd_remove may proceed. */
	for (vdi = 1; vdCnt < dmnP->vdCnt && vdi <= BS_MAX_VDI; vdi++) {
		if (!(vdp = vd_htop_if_valid(vdi, dmnP, TRUE, FALSE))) {
			continue;
		}
		mutex_enter(&vdp->ssVolInfo.ssVdMsgLk.mutex);
		vdp->ssVolInfo.ssVdMsgState = SS_STOP;
		mutex_exit(&vdp->ssVolInfo.ssVdMsgLk.mutex);
	...
	/* Wait for any working vfast threads to finish their current work
	 * cycle and notice we are deactivating this domain. */
...
ss_dealloc_dmn(domainT * dmnP)
{
	...
	mutex_destroy(&dmnP->ssDmnInfo.ssDmnHotLk.mutex);
	return;
}
...
{				/* in - pointer to header for list */
	...
	ss_dealloc_frag_list(vdp);
	mutex_destroy(&vdp->ssVolInfo.ssVdMigLk.mutex);
	mutex_destroy(&vdp->ssVolInfo.ssVdMsgLk.mutex);
	mutex_destroy(&vdp->ssVolInfo.ssFragLk.mutex);
	return;
}
	...
	*dmnState = dmnP->ssDmnInfo.ssDmnState;
	...
	case SS_CFS_MOUNT:	/* special cluster mount case */
		if (dmnP->ssDmnInfo.ssDmnState == SS_ACTIVATED) {
			/* Set state to SS_CFS_RELOC only if currently
			 * SS_ACTIVATED. vfast state will be restored to
			 * SS_ACTIVATED in ss_put_rec after domain is
			 * deactivated if case == SS_CFS_RELOC. vfast state
			 * will be restored to SS_ACTIVATED by caller after
			 * fset is done being umounted if case ==
			 * SS_CFS_UMOUNT. If vfast is already in some
			 * deactivated state, nothing to do. */
	...
	case SS_ACTIVATED:
		if (dmnP->ssDmnInfo.ssDmnState != SS_ACTIVATED) {
			/* Not already SS_ACTIVATED, set to SS_ACTIVATED and
			 * return thread states to SS_IDLE DO NOT set
			 * ssVdMigState = SS_IDLE if theres a chance a thread
			 * is already in a volume.  To do so would allow two
			 * threads to work a volume at the same time and that
			 * is a big no-no. */
			dmnP->ssDmnInfo.ssDmnState = state;
			for (vdi = 1; vdCnt < dmnP->vdCnt && vdi <= BS_MAX_VDI; vdi++) {
				if (!(vdp = vd_htop_if_valid(vdi, dmnP, TRUE, FALSE))) {
		...
		break;
	default:
		/* vfast state is either SS_DEACTIVATED or SS_SUSPENDED. Wait
		 * for any working vfast threads to finish their current work
		 * cycle and notice we are deactivating this domain. */
		dmnP->ssDmnInfo.ssDmnState = state;
		ss_dmn_deactivate(dmnP, FALSE);	/* lock released in
						 * ss_dmn_deactivate */
		ssDmnLocked = FALSE;
	}
...
ss_init_dmnInfo(domainT * dmnP)
{
	...
	mutex_init(&dmnP->ssDmnInfo.ssDmnHotLk.mutex, MUTEX_DEFAULT, IPL_NONE);
...
ss_init_vd(vdT * vdp)
{
	...
	mutex_init(&vdp->ssVolInfo.ssFragLk.mutex, MUTEX_DEFAULT, IPL_NONE);
	mutex_enter(&vdp->ssVolInfo.ssVdMigLk.mutex);
...
{				/* out */
	...
	dmnP->ssDmnInfo.ssReserved0 = (uint32_t) ssAttr->ssReserved0;
...
{				/* out */
	...
	/* extras on-disk */
	ssAttr->ssReserved0 = (uint16_t) dmnP->ssDmnInfo.ssReserved0;
...
ss_put_rec(domainT * dmnP)
{
	...
	if (dmnP->ssDmnInfo.ssDmnState == SS_CFS_RELOC) {
		/* don't set update on-disk flag or we would recall this
		 * routine! */
		dmnP->ssDmnInfo.ssDmnState = SS_ACTIVATED;
	}
	...
	for (i = 0; i < xtntMap->validCnt; i++) {
		...
		for (j = 0; j < xtntMap->subXtntMap[i].cnt; j++) {
			...
			if ((prev_blk != XTNT_TERM) && (prev_blk != PERM_HOLE_START)) {
				/* save the pageCnt & blk from this extent to
				 * the previous extent if the previous extent
				 * was not a hole terminator. */
			...
			if (last_pageCnt) {
				...
				/* test for current extent's blocks not
				 * adjacent to last one */
				if (!((prev_blk +
...
)
{
	...
	if (bfap->ssMigrating == SS_STOP) {
		/* vfast closed this file with an error, reset flag to allow
		 * next closer to check for placement on frag list. */
		bfap->ssMigrating = SS_MSG_IDLE;	/* allows to be placed
							 * again */
		RAISE_EXCEPTION(EOK);
	}
	...
	if ((EXTENTS_FRAG_RATIO(allocPageCnt, totXtnts) <
		...
		if ((listmsg =
			...
			mutex_enter(&bfap->dmnP->ssDmnInfo.ssDmnLk.mutex);
			bfap->dmnP->ssDmnInfo.ssDmnListMsgDropCnt++;
			mutex_exit(&bfap->dmnP->ssDmnInfo.ssDmnLk.mutex);
		}
	}
	mutex_enter(&bfap->dmnP->ssDmnInfo.ssDmnLk.mutex);
...
{				/* secondary sort in list */
	...
	if (sts == EOK) {
		...
		/* file set is not accessible probably because domain has been
		 * deactivated or fset is being removed. */
		goto _cleanup;
	}
	...
	while (currp != (ssFragLLT *) fhp) {
		if (fp->ssFragRatio <= currp->ssFragRatio) {
			...
			while ((currp != (ssFragLLT *) fhp) &&
				...
				/* keep going until we find correct place to
				 * put entry */
				currp = currp->ssFragRatFwd;
			}
			if (currp == (ssFragLLT *) fhp)
				break;	/* at list end */
...
{				/* in */
	...
	KASSERT(vdp);
	KASSERT(SLOCK_HOLDER(&vdp->ssVolInfo.ssFragLk.mutex));
	fhp = &vdp->ssVolInfo.ssFragHdr;
	...
	/* search list for file if found, delete it */
	...
	if (fhp->ssFragListCnt < SS_INIT_FRAG_LST_SZ) {
		/* less than maximum entries in list, reset threshhold to
		 * something large */
...
)
{
	...
	/* now update the messages sent counter on the domain.  This counter
...
{				/* in  - entry */
	...
	if (dmnP->ssDmnInfo.ssDmnHotCurrDay != newday) {
		/* if day has flipped to new day, clear new day for all
		 * entries, reorder list */
		while (currp != (ssHotLLT *) hhp) {
			currp->ssHotIOCnt[newday] = 0;
			nextp = currp->ssHotFwd;

			/* if the flip caused there to be a zero count, delete
			 * this entry. */
	...
	/* Search list for a duplicate which may be on a diff vdi. If found,
	 * update it for the new vd and hit counts and then remove it.  List
	 * is ordered by combined counts. */
	hhp = &dmnP->ssDmnInfo.ssDmnHotHdr;
	currp = hhp->ssHotFwd;
	...
	if (hhp->ssHotListCnt >
		...
		/* too many entries in list, delete the one that has the
		 * oldest update time */
		ssHotLLT *delp;
		time_t saved_t;
		saved_t = 0x7fffffff;
...
{				/* in */
	...
	/* search list for file located on volume if found, delete it */
	while (currp != (ssHotLLT *) hhp) {
		nextp = currp->ssHotFwd;
	...
	/* now, if there is still too many entries on list, remove the oldest
	 * entries */
	while (hhp->ssHotListCnt >
	    (SS_INIT_HOT_LST_GROW_SZ * dmnP->vdCnt)) {
		/* too many entries in list, delete the oldest update time
		 * entries */
		hhp = &dmnP->ssDmnInfo.ssDmnHotHdr;
		currp = hhp->ssHotFwd;
		saved_t = 0x7fffffff;
...
)
{
	...
	/*
         ...
         */
	mutex_enter(&bfSetp->dmnP->mutex.mutex);
	if (bfSetp->state == BFS_BUSY) {
		mutex_exit(&bfSetp->dmnP->mutex.mutex);
		RAISE_EXCEPTION(E_NO_SUCH_BF_SET);
	}
...
)
{
	...
	/* bump the access count on the domain so it can't go away while we
	 * are working in it */
	...
	mutex_enter(&svdp->ssVolInfo.ssVdMigLk.mutex);
	svdp->ssVolInfo.ssVdSSThdCnt++;
	mutex_exit(&svdp->ssVolInfo.ssVdMigLk.mutex);
	...
	if ((dmnP->ssDmnInfo.ssDmnSmartPlace) &&
		 ...
		 * command E_WOULD_BLOCK indicates vfast was stopped in middle
		 * of a migrate E_RSVD_FILE_INWAY - vfast unable to pack cause
		 * rsvd files inway. ENOSPC - vfast unable to find or create
		 * enough space E_RANGE_NOT_CLEARED real error, range was not
		 * cleared by mig_pack_vd_range E_PAGE_NOT_MAPPED - file was
		 * truncated, but still defragmented successfully  -or-
		 * During a pack operation, an extent of a file being packed
		 * was truncated and pack aborted. ENO_MORE_MCELLS - File
		 * being packed is on DDL, abort since we cant move it */

		/* Update ssPlaced in hot list. If there is an error moving
		 * file, set ssHotPlaced == -error so we don't try again and
		 ...
		 * processing of the hot list by the monitor thread may see a
	...
	if (dmnP->ssDmnInfo.ssDmnDefragment) {
		...
		mutex_enter(&svdp->ssVolInfo.ssFragLk.mutex);
		 ...
		 * defragment the file on same vol as its been placed by prior
		 * hot list processing. */
...
HANDLE_EXCEPTION:
	...
	if (vdRefed) {
		 ...
		 * threads */
		mutex_enter(&svdp->ssVolInfo.ssVdMsgLk.mutex);
		if (svdp->ssVolInfo.ssVdMsgState != SS_STOP)
			svdp->ssVolInfo.ssVdMsgState = SS_MSG_IDLE;
		mutex_exit(&svdp->ssVolInfo.ssVdMsgLk.mutex);
...
{				/* in - hint for type of allocation */
	 ...
	 * is not enough space on the virtual disk.  Otherwise, it returns, in
	 * "blkCnt", the largest contiguous block that can be allocated.  This
	 * value may or may not be large enough to satisfy the request. */
	...
	if ((blkCnt) &&
		...
		if (((alloc_hint == BS_ALLOC_MIG_SINGLEXTNT) ||
			...
			/* Note: SBM has a range re-locked by sbm_scan_v3_v4.
			 * It will be reduced in size below, after packing it.
			 * RULE 1: if this is a hot file, try to create it
			 * regardless of how much needs to be cleared. We do
			 * this because hot files are accessed frequently and
			 * we want to do the extra work to defragment them.
			 * RULE 2: Never pack a range size that is
			 * SS_PACK_RANGE_PCT_MAX_SIZE percent of the total
			 * volume size. a. more likely to be stolen by system.
			 * b. more likely to encounter a rsvd file. c. becomes
			 * less efficent than combining extents. If we pass
			 * these rules then (1)reset the stgDesc so we fall
			 * into the pack block below. and (2) free the stg
			 * returned from sbm_find_space below AFTER we lock
			 * the new pack range.
			 * 
			 * Note: SSPackRangePctMaxSize is already a reciprocal
			 * percentage. */
			...
			if (sts != EOK) {
				/* E_RSVD_FILE_INWAY: There may have been
				 * enough space but it isn't uninterupted by a
				 * non-movable reserved file. ENO_MORE_MCELLS
				 * - File being packed is on DDL, abort since
				 * we cant move it. ENO_MORE_BLKS indicates
				 * the reserved space was stolen by a needy
				 * user application file. E_PAGE_NOT_MAPPED
				 * indicates that the file range being
				 * migrated was truncated in whole or part
				 * before we could migrate it. */
				sts = ss_find_space(vdp,
				    requestedBlkCnt,
				    &blkOffset,
				    &blkCnt,
			...
			/* range would take too long to pack, return largest
			 * extent */
			sts = ss_find_space(vdp,
			    requestedBlkCnt,
			    &blkOffset,
	...
	if (whoami == SS_PARENT) {
		if (bestExtentCnt) {
			...
			*extentRunCnt = bestExtentCnt;
			...
			/* no extents small enough for request size, return
			 * ENOSPC */
			*startPg = 0;
			*runPageCnt = 0;
			*extentRunCnt = 0;
		...
		if (((reqPageCnt == 0) && (consecutiveExtentCnt > 0)) ||
			...
			*extentRunCnt = consecutiveExtentCnt;
			...
			/* no more than one extent was found from startPg to
			 * end */
			*runPageCnt = 0;
			*xmRunPageCnt = 0;
			*extentRunCnt = 0;
    ...
    vdIndexT * newVdIndex)
{
	...
	*newVdIndex = 0;
	...
	for (vdCnt = 0, vdi = 1;
		...
		/* Now check this vd for enough contig free space, if not
		 * enough try to find a better volume with larger free
		 * extents. */
		sts = x_load_inmem_xtnt_map(bfap, X_LOAD_REFERENCE);
		if (sts != EOK) {
			vd_dec_refcnt(vdp);
...
)
{
	...
	/* shouldn't be needed because check is before its put on lists, but
	 * we'll check anyway. Ignore files under directio control if user
	 * desires. */
	if ((bfap->dmnP->ssDmnInfo.ssDmnDirectIo == FALSE) &&
	    (((bfSetT *) bfap->bfSetp)->bfSetFlags & BFS_IM_DIRECTIO)) {
		/* fset is a directio */
	...
	if ((targVol == -1) &&
	    (alloc_hint != BS_ALLOC_MIG_SINGLEXTNT) &&
	    (alloc_hint != BS_ALLOC_MIG_SAMEVD)) {
		/* If no need to explicitly balance and this is not an
		 * explicit placement for a hotfile, choose the volume with
		 * the most free space. The space may be stolen by system, but
		 * then we would be running out of space anyways. */
		sts = ss_get_vd_most_free(bfap, &newDstVdIndex);
		if (sts == EOK) {
			dstVdIndex = newDstVdIndex;
		} else if (sts == ENO_MORE_BLKS) {
			dstVdIndex = srcVdIndex;	/* try orig cause may
							 * want to pack */
		} else if (sts != EOK) {
			RAISE_EXCEPTION(sts);
		}
	...
	mutex_enter(&dvdp->ssVolInfo.ssVdMigLk.mutex);
	...
	while ((dvdp->ssVolInfo.ssVdMigState == SS_PROCESSING) ||
		...
		/* must be another thread got here first, simply wait for my
		 * turn */
	...
	if ((dvdp->ssVolInfo.ssVdMigState == SS_ABORT) ||
	    (SS_is_running == FALSE) ||
	    (bfap->dmnP->ssDmnInfo.ssDmnState != SS_ACTIVATED)) {
		mutex_exit(&dvdp->ssVolInfo.ssVdMigLk.mutex);
	...
	/* Try to get free space for pages to be migrated. lock it with
	...
	/* May have less than requested here because ss_get_n_lk_free_space
	 * returns as much as it can even if it was unable to find or create
	 * what was requested.  In this case we will combine the most
	 * non-contiguous extents in the file into the free space we did get. */
	srcPageOffset = 0;
	...
	while ((srcPageOffset < xmEndPage) && (pgsMigSoFar < allocPageCnt)) {
		...
		for (page = srcPageOffset; page < xmEndPage;) {
			if (page_is_mapped(bfap, page, &nextpage, FALSE) == TRUE) {
				/* nextpage is the page beyond the current
				 * extent. */
		...
		/* Make sure we figure for holes now filled in by only
		 * migrating pages we have room for. */
		...
		if (srcPageOffset >= xmEndPage) {
			...
			if (wholeFile) {
				mutex_enter(&dvdp->dmnP->ssDmnInfo.ssDmnLk.mutex);
				dvdp->dmnP->ssDmnInfo.ssFilesDefraged++;
				mutex_exit(&dvdp->dmnP->ssDmnInfo.ssDmnLk.mutex);

				if (bfap->primVdIndex != dstVdIndex) {
					/* since we are done, we need to
					 * ensure that the primary mcell is
					 * moved to the destination volume. */
		...
		/* close the fileset and bfap before parking  so that the
		 * fileset can be deleted if needed. We don't want to hold up
		 * the delete while waiting for some idle time to run */
		sts = ss_close_file(bfSetp, bfap, fsetMounted);
		if (sts != EOK) {
			RAISE_EXCEPTION(sts);
		}
...
HANDLE_EXCEPTION:
	...
	if ((closeFileFlag == TRUE) &&
		...
		if (!BS_BFS_EQL(bfSetId, nilBfSetId)) {
			...
			mutex_exit(&dvdp->ssVolInfo.ssFragLk.mutex);
		}
	...
	if (closeFileFlag == TRUE) {
		if (sts == EOK)
			bfap->ssMigrating = SS_MSG_IDLE;	/* allow to be placed
								 * again */
		else
			bfap->ssMigrating = SS_STOP;	/* will be cleared next
							 * ss_chk_fragratio */
...
ss_find_target_vd(bfAccessT * bfap)
{
	...
	/* malloc this structure each time routine is called, otherwise we
	 * would have to reference and dereference the vdp many times in the
	 * following code. */
	...
	for (vdCnt = 0, vdi = 1, validVd = 0;
		...
		if (sts != EOK) {
			/* This volume is not a candidate for our "normal"
			 * file. Skip this volume in our processing. */
			vdCnt++;
			vd_dec_refcnt(vdp);
			continue;
		}
	...
	for (i = 0; i < validVd; i++) {
		...
		if ((abs(volDeviation)) > (SSBalanceGoal * (precision / 100)))
			found_bad_vol = TRUE;	/* at least one of the vols
						 * worse than threshold */
	}

	if (!found_bad_vol) {
		/* if not too out of balance return -1 indicating same vol as
		 * src.is ok */
		targetVdi = -1;
		RAISE_EXCEPTION(EOK);
	}
	...
	for (targ = 0; targ < validVd; targ++) {
		...
		/* Don't consider moving the file to this volume if either the
		 * volume doesn't have enough free space to hold the whole
		 * file (the whole file must be allocated afresh before any of
		 * the old space occupied by the file can be freed, so the
		 * presently occupied space cannot be counted). */
		...
		/* simulate moving file to target volume above, add up all the
		 * other volumes new free space and arrive at a new domain
		 * balance deviation. */
		for (i = 0; i < validVd; i++) {
			...
			/* increase the free size of the volume if file is to
			 * be moved from this volume. */
			...
			/* Calculate delta for this volume from the domain
			 * average free space.  Keep a running total for all
			 * volumes in domain. */
	...
	/* is new best deviation from all volumes above better than the
	 * existing domain deviation? If so, perform a final check. */
	if (bestNewBalanceDeviation < dmnBalanceDeviation) {
		...
		for (i = 0; i < validVd; i++) {
			...
			if (volData[i].vdi == bestTargetVdi)
				continue;
...
{				/* out */
	...
	for (targ = 0; targ < validVd; targ++) {
		...
		/* simulate moving file to target volume above, add up all the
		 * other volumes hot IOs and arrive at a new domain balance
		 * deviation. */
		...
		for (newBalanceDeviation = 0,
			...
			if (i == targ) {
				/* increase the IO count of the volume if file
				 * is to be moved to this volume. */
				newVolIO = volData[i].volHotIO + currHotIOCnt;
			} else
				newVolIO = volData[i].volHotIO;

			/* Calculate delta for this volume from the domain
			 * best hot file balance, keep a running total for all
			 * volumes in domain */
...
{				/* out - selected fset */
	...
	/* malloc this structure each time routine is called, otherwise we
	 * would have to reference and dereference the vdp many times in the
	 * following code to store it on the vdp. malloc the most vds that can
	 * be valid even though we may use a subset of them. */
	...
	/* II. Count up the hottest HOT file IO on each volume. Find first hot
	 * file in top percent that has not been ssHotPlaced on a volume yet. */
	i = 0;
	hp = hhp->ssHotFwd;
	...
	while (hp != (ssHotLLT *) hhp) {
		...
		/* Make sure the entry has been in the list long enough and
		...
		if ((hp->ssHotPlaced == 0) &&
			...
			/* don't move reserved files as part of hot files
			 * balancing */
			for (targ = 0; targ < validVd; targ++) {
				/* Place first file found that will fit on at
				 * least one of the volumes. */
				if ((hp->ssHotFileSize / BS_BLKSIZE) < volData[targ].volFreeBlks) {
					new_hp = hp;	/* found the file to
							 * place! */
					break;
				}
			}
		}
		/* count reserved files where they lay - ignore hot files that
		 * have errored or that have not been placed yet. */
		if ((hp->ssHotPlaced > 0) ||
		    (BS_BFTAG_RSVD(hp->ssHotTag))) {
			for (currHotIOCnt = 0, wday = 0; wday < SS_HOT_TRACKING_DAYS; wday++) {
				currHotIOCnt += hp->ssHotIOCnt[wday];
			}
			/* match correct vd since there can be vdi that are
			 * not consecutive */
			for (targ = 0; targ < validVd; targ++) {
	...
	if ((dmnHotIO == 0) && (new_hp != NULL)) {
		/* No files have been placed yet, keep file on same volume */
		targetVolumeIndex = hp->ssHotVdi;
		RAISE_EXCEPTION(EOK);
	}
	/* III. No new files ready to be placed. Clear placement field for
	 * files that have fallen so the next time they are one of hottest
	 * they can be placed according to the conditions at that time. */
	for (; hp != (ssHotLLT *) hhp; hp = hp->ssHotFwd) {

		/* skip reserved files, files not on list long enough, and
		 * files with move errors */
		if (BS_BFTAG_RSVD(hp->ssHotTag))
			continue;
	...
	if (new_hp == NULL) {
		...
		/* IV. New file ready to be placed. Determine best volume to
		 * place it on. */
