...
typedef struct {
	rbmtExtMsgTypeT msgType;/* msg type (verification; future expansion) */
...
bmtr_scan_mcells(
    ...
    struct vd ** vdp,			/* in/out - vd (of mcell) pointer */
...
bmtr_clone_mcell(
    bsMRT * rp,
    struct vd * srcVdp,		/* Source mcell's disk index */
    bfMCIdT * srcMCId,		/* Source mcell's id */
    struct vd * destVdp,		/* Destination mcell's disk index */
    ftxHT parentFtxH,
    int first
);
...
bmt_upd_mcell_free_list(
    ...
    struct vd * vdp,			/* in - current disk pointer */
...
static int check_xtnts(bsXtntT *, uint16_t, struct bfAccess *, struct vd *);
...
)
{
	...
	while ((rp->type != rtype) && (rp->type != BSR_NIL)) {
		...
		if (rp->bCnt == 0 || rp->bCnt > BSC_R_SZ) {
			if (dmnP == NULL) {
				/* We may be early in the domain activation
				 * process, so that a domain does not yet
				 * exist.  Let the caller handle this case. */
				return NULL;
			}
...
)
{
	...
	while ((rPtr = bmtr_assign_rec(rType, rSize, mcp, pgref)) == NULL) {
		...
		*mcid = mcp->nextMCId;
		 ...
		 * but BSR_XTRA_XTNTS.  If this occurs it is probably due to a
		 * bug where the cell 6(bmt extension cell) mistakenly pointed
		 * to cell 27 when it should have had the mcell chain
		 * terminated within cell 6.
		 * 
	...
	if (rPtr == NULL) {
		 ...
		 * If new code is adding enough records to fill cell 6 and it
		 * needs to allocate another cell it cannot fill cell 27
		 * because the extent chain for the rbmt MUST go there. If
	...
	while ((*bsrp = bmtr_find(mcp, rtype, (*vdp)->dmnP)) == NULL) {
		...
		*mcid = mcp->nextMCId;
		...
		if ((newpage != bmtp->pageId) || newVd) {
			...
			if (!pgRefed) {
				/* BMT is not mapped yet.  Can't access beyond
				 * pg 0 */
...
typedef struct {
	...
	char beforeImage[BS_USABLE_MCELL_SPACE];	/* before image of mcell
							 * rec */
}      bmtrPutRecUndoT;
/*
...
)
{
	...
	if (!(BMT_EXT_REQUEST_OK(numNewPgs, dmnP->ftxLogPgs, vdp->stgCluster))) {	/* log large enough? */
		/* Here: the requested size of bmtXtntPgs is too large for the
		 * current */
		/* log size and could potentially lead to a 'log half full'
		 * panic.      */
		/* Use an ext size that is less than 25% of  (log size *
		 * SBM_ADVFS_PGS(x)) */
...
cleanup_corruption:
	{
		...
		uint32_t new_head_of_free_list;
		 ...
		 * is the first bad page we've found in this call and it
		 * points to itself as the next page with free mcells OR if
		 * this is the second bad page that we've found in this call,
		 * cauterize the list.  In any case, if we're here it's
		 * because some kind of corruption has occurred.  We try to
		 * work around it since it's just a free list but advise the
		 * admin to run fixfdmn, which can rebuild the mcell free
		 * list.  If we find two bad pages with no free mcells, only
		 * print out a warning and post an event for the first one. */
...
)
{
	 ...
	 * metadata is written to the disk, this will be redone on recovery by
	 * the image redo. */
	...
	/* Now initialize 27th mcell which is not initialized by bmt_init_page
	 * for rbmt. */
	...
	/* We need to initialize 27th mcell records too, which are not
	 * initialized by bmt_init_page for rbmt. */
	...
	/* This tx has only a redo operation; force log record to disk when we
...
)
{
	...
	if ((rbmt->nextfreeMCId.cell == (RBMT_RSVD_CELL - 1)) &&
		...
		if (msg) {
			...
			/* Setting this flags is protected by the
			 * rbmt_mcell_lk */
			vd->rbmtFlags |= RBMT_EXTENSION_IN_PROGRESS;
		}
...
)
{
	...
	/* This is similar to the undo routine undo_upd_xtnt_rec. This is the
	 * case where we were extending the BMT and we could not fit the new
	 * extents on the exisiting subxtnt map. We needed to create a new
	 * mcell tfor the new subxtnt map. This mcell is now being unlinked
	 * from our mcell chain on-disk so at this point we need to ditch the
	 * BMT's extent maps and reload them to reflect in memory the fact
	 * that the on-disk extent maps have changed.  */
...
bmtr_clone_recs(
    ...
    uint16_t srcVdIndex,		/* in - Source bf primary mcell's disk index */
    bfMCIdT destMCId,		/* in - Destination bf primary mcell's id */
    uint16_t destVdIndex,	/* in - Destination bf primary mcell's disk
				 * index */
    uint16_t npVdIndex,		/* in - new primary disk index */
    struct domain * dmnP,		/* in - ptr to domain struct */
    ftxHT parentFtxH		/* in - parent ftx handle */
)
{
	struct vd *srcVdp, *parentVdp, *childVdp;
	bfPageRefHT pgRef;
	bsMPgT *srcBmtp;
	bsMCT *srcMcp;
	bfMCIdT mcid;
	...
	do {
		...
		/* get a pointer to the first mcell record in the current
		 * mcell */
...
)
{
	 ...
	 * mcell again. */
try_again:
	/* If there is a corruption in the mcell don't loop forever on a bad
	 * value. One try is enough and  go out . */
	if (retryFlg > 1) {
		return E_BAD_BMT;
	}
	...
	if ((xrp = (bsXtraXtntRT *)
		...
		fxp = xrp->bsXA;
		xCnt = xrp->xCnt;
		/* Difficult to trust that this is still a BSR_XTRA_XTNTS
		 * mcell. so xCnt, as read now, could be bogus. If the value
		 * is not reasonable, try rereading the mcell again. */
		...
		/* Only v4 non-stripe and v3/v4 reserved files have extents in
		 * BSR_XTNTS. */
		fxp = xp->firstXtnt.bsXA;
		xCnt = xp->firstXtnt.xCnt;
		/* If the value is not reasonable for BMT_XTNTS mcell try
		 * rereading the mcell again. */
		if (xCnt > BMT_XTNTS &&
	...
	for (j = 0; j < (xCnt - 1); j++) {
		...
		if (((cRangeBeginBlk >= extentBeginBlk) &&
			...
			if (BS_BFTAG_RSVD(mcp->tag)) {
				/* ran into some reserved file allocated bits
				 * that are in the range to be packed.  Simply
				 * abort since we can't migrate reserved files
				 * with the standard migrate. */
				RAISE_EXCEPTION(E_RSVD_FILE_INWAY);	/* exit routine */
			}
			/* adjust entry for only that part of extent thats in
			 * range */
			...
			/* Allocate a list entry for the extent found in the
			 * clear range */
			...
			*bmt_cnt += xtntp->ssPackPageCnt * (ADVFS_PGSZ_IN_BLKS / vdp->stgCluster);
			...
			/* Put this file's extent range onto the list so that
			 * it can be moved later by caller.
			 * 
			 * add to list - sorted by xtntp->ssPackStartXtBlock asc
			 * Note: Since this list is sorted by the blk number,
			 * any subsequent calls to this routine will get the
			 * next highest block set of files.   Another call to
			 * this routine is required whenever the list has been
			 * maxed out in size. */
			...
			while (currp != (ssPackLLT *) plp) {
				if (xtntp->ssPackStartXtBlock <
					...
					/* insert xtntp at head or middle of
					 * list, before currp */
			...
			if (plp->ssPackListCnt > SS_INIT_PACK_LST_SZ) {
				/* too many entries in list, delete the last
				 * one in list */
				currp = plp->ssPackBwd;
...
)
{
	...
	*another_scan_required = FALSE;
	if (RBMT_THERE(vdp->dmnP)) {
		/* do rbmt first if there. Optimizes because if rbmt has an
		 * extent in the way then we can abort this range since we
		 * cant move reseerved files. */
		doing_bmt = FALSE;
	}
chk_bmt:
	...
	while (1) {
		...
		bmth->curMcell++;	/* bump to the next mcell for next
					 * loop */
...
)
{
	...
	if ((bmth->vdp = vd_htop_if_valid(vdIndex, dmnP, TRUE, FALSE))) {
		...
		lock_write(&bmth->vdp->mcell_lk);
		    sts = bmt_refpg(&bmth->pgRef,
		    (void *) &bmth->pgPtr,
		    bmth->vdp->bmtp,
		    bmth->curPg,
		    BS_SEQ_AHEAD);
		lock_done(&bmth->vdp->mcell_lk);
...
init_bs_bmt_util_opx(void)
{
	...
	/* These two agents are now obsolete but must be kept around incase we
	 * encounter an old domain that needs to have recovery run on it. */
...
*/
static int
test_mcell_ptr(vdIndexT vdi, bfMCIdT mcid, bfTagT tag, struct domain * dmnP)
{
	int sts = EOK;
	struct vd *vdp;
	struct bfAccess *mdap;
...
check_BSR_XTNTS_rec(bsXtntRT * bsXtntRp, struct bfAccess * bfap, struct vd * vdp)
{
	 ...
	 * both V3 and V4 domains, we cannot predict the values that will be
	 * in these fields.  This is stated in a comment inside the
	 * correct_bsr_xtnts_record() in fixfdmn, and I found this to be true
	 * during testing as well. Also, do not try to validate these fields
	 * for non-reserved files in a V3 domain.  V3 records built under the
	 * 4.0 streams will have random values, although the records built
	 * under a 5.x stream will have predetermined, invalid values.  Since
	 * we can't detect how the domain was built, just skip the validation
	 * for V3 normal files. */
	...
	/* Verify V3 metadata and all V4 non-striped files here. The
	 * firstXtnt.xCnt is <= BMT_XTNTS (2) for all reserved files except
	 * the MISC file, where it is 3. */
...
*/
static int
check_xtnts(bsXtntT bsXA[], uint16_t cnt, struct bfAccess * bfap, struct vd * vdp)
{
	int sts = EOK;
	int i;
	uint32_t lastPage;
