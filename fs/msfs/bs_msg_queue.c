...
msgq_create(
    ...
    int allow_q_growth,		/* in - flag TRUE, allow q length to grow;
				 * FALSE no */
...
)
{
	...
	*msgQ = nilMsgQ;
	...
	mutex_init(&msgQ->mutex.mutex, MUTEX_DEFAULT, IPL_NONE);
...
)
{
	...
	mutex_exit(&msgQ->mutex.mutex);
	mutex_destroy(&msgQ->mutex.mutex);
...
)
{
	 ...
	 * that already exist and initialize them.  Be careful since there is
	 * a mutex held by the calling routine, so no sleeping!  Due to the
	 * design of the kernel allocator, it is very likely that the
	...
	/* We will return the address of the user's message area just past the
	 * header info. */
