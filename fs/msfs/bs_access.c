...
_retry:
	...
	while (bfap != (bfAccessT *) (&ClosedAcc)) {
		 ...
		 * remedy the situation.  If the underlying cause of these
		 * invalid structures being on the closed list is fixed, then
		...
		if (state == ACC_INVALID) {
			...
			if (bfap->bfObj) {
				...
				/* VCHR & VBLK type files don't have objects
				 * (see bf_get_l). */
				/* If the state is invalid and there is no
				 * object then the */
				/* saved_stats is the only condition that can
				 * keep the */
		...
		/* Recycle some of the bfacces structs from the ClosedAcc list
		 * to the FreeAcc list.  Do not try to update stats if: 1.
		 * file state is ACC_FTX_TRANS (this is a temporary condition)
		 * 2.  There is no vnode associated with this bfap AND there
		 * are no stats saved from the latest reclaim 3.  There is no
		 * context pointer 4.  The dirty_stats flag is not set.
		 * 
		 ...
		 * closed list, waiting for this opportunity to flush these
		 * stats to disk.  This paragraph is a shortened version of
		 * the one that follows for bfap with no vnode. */
		if ((bfap->bfVp == NULL) && (bfap->saved_stats != NULL)) {
			...
			/* set bit that we are flushing the stats;  a racing
			 * bf_get_l() may see this while we are flushing (the
			 * lock is dropped inside fs_flush_saved_stats) and
			 * load up the stats into a new fsContext, but it will
			 * not try to deallocate bfap->saved_stats out from
			 * under the fs_flush_saved_stats() routine. Then we
			...
			if (ret == EWOULDBLOCK) {
				 ...
				 * of the stats so set it all back to the way
				 * it was, stop processing, and wake waiters */
				bfap->saved_stats->op_flags &= ~SS_FLUSH_IN_PROGRESS;
				if (bfap->saved_stats->op_flags & SS_STATS_COPIED_TO_CONTEXT) {
					/* Special case; we didn't flush the
					 * stats, but they got copied back to
					 * a new fsContext while the lock was
					 * dropped, so latest stats are in the
					 * context.  Get rid of the
					 * out-of-date saved_stats. */
		...
		/* vget() the vnode to prevent it from being reclaimed after
		 * we drop the mutex.  If we can't get it, skip to next access
		 * struct. */
		if (!vget_nowait(vp)) {
			/* Update stats and (hopefully) move to freelist in
			 * DEC_REFCNT() */
			VN_UNLOCK(vp);
			RM_ACC_LIST_NOLOCK(bfap);
			bfap->refCnt++;
		 ...
		 * any more.  In this case, we have lost our way down that
		 * chain and will exit.  We could go back and restart, but I
...
restart:
	...
	while (bfap != (bfAccessT *) (&bfSetp->accessFwd)) {
		...
		/* Check for an access structure that has no vnode but has a
		 * saved_stats structure.  The vnode was reclaimed in
		 * msfs_reclaim, but we did not flush the stats to the disk at
		 * that time.  Therefore, we saved the stats in the access
		 * structure and put it onto the closed list, waiting for this
		 * opportunity to flush these stats to disk.  */
		if ((vp == NULL) && (bfap->saved_stats != NULL)) {
			...
			/* Set stat flush in progress bit;  a racing
			 * bf_get_l() may see this while we are flushing (the
			 * lock is dropped inside fs_flush_saved_stats) and
			 * load up the stats into a new fsContext, but it will
			 * not try to deallocate bfap->saved_stats out from
...
{				/* out - hint indicating an insertion since
	 ...
	 * msfs_reclaim is looking for the bfap of a vnode it is reclaiming.
	 * Since nothing prevents it, a fileset could go away in this time
	 * frame.
	 * 
...
restart:
	...
	do {
		...
		/* We now attempt to find the bsaccess structure in the hash
		 * chain. If the tag.seq is 0 then this is a reserved tag (ie.
		 * logfile, BMT, SBM, roottag dir) and the sequence number is
		 * not incremented so we must therfore check the bfsetid to
		 * insure that it is not an old reserved tag that was left
		 * lying around. */

		/* We will no longer get the bfa lock each time. We will look
		 * at the Setp and tag first and then the lock state.
		 * 
		 * NOTE: If we encounter a BsAccess structure with the same tag
		 * and setp but in ACC_RECYCLE we know that it will soon have
		 * a different tag and setp but currently we need to wait for
		 * it because any i/o associated with it must be flushed
		 * before we can access the file.
		 * 
		 * Some day we will be able to skip the wait since there should
		 * never be anything to flush since this access structure was
		 * just removed from the free list. */
		...
		if ((BS_BFTAG_EQL(findBfap->tag, tag)) &&
		                 ...
		                 */
			{
				...
				/* This is a reserved tag. Need further
				 * verification This situation can arise when
				 * many filesets are opened causing the
				 * metadata file tags to wrap around. Since
				 * the tag.seq is not incremented we can't
				 * tell so we must check the bfSetId itself. */

				/* There's a chance the fileset could have
				 * gone away. We wont find it anyway so break
				 * out */
				...
				if (BS_BFS_EQL(bfSetp->bfSetId,
					...
					/* We found the one we want; return it
					 * locked */
					goto return_it;
				}
			}
...
return_it:
	KASSERT(findBfap ? mutex_owned(&findBfap->bfaLock.mutex) : 1);
...
start:
	...
	for (bfap = bfSetp->accessFwd;
		 ...
		 * by another thread, wait for it since the recycling process
		 * will take it off this fileset's list. This avoids a race
		 * with bfs_invalidate destroying the accessChainLock before
		...
		if (bfap->hashlinks.dh_links.dh_next != NULL) {
			/* We can't make this visible yet. First we must take
			 * it off the hash table but we can't do that until we
			 * give up the accessChainLock so we'll defer this
			 * till then. */
			bfap->setFwd = bfap_remove_from_hash_list;
			bfap_remove_from_hash_list = bfap;
		} else {
			/* Since the hashtable relies on the bfSetp for its
			 * hash value we can not change this until the access
			 * structure is no longer in the hash table */
	...
	while (nextbfap != NULL) {
		...
		/* Since the hashtable relies on the bfSetp for its hash value
		 * we can not change this until the access structure is no
		 * longer on the hash table */
...
)
{
	 ...
	 * hanging around because CFS doesn't immediately reclaim bfap's. The
	 * vgone() below will force CFS to reclaim it though. If the usecount
	 * is too high, someone still has it opened so we should error out of
	 * here with a E_TOO_MANY_ACCESSORS. The open can occur on one of the
	 * reserved files either through mssh utility or a fopen() of a file
	 * like "/mnt/.tags/M-13" */
	...
	if ((bfap != NULL) && (bfap->refCnt > 1)) {
		 ...
		 * Do this since if we used the fs list  directly a lock
		 * hierarchy violation would occur below because namei()
		 * accesses the fsContext.file_lock. Violation would be
		 * between fsContext.file_lock and FilesetLock. */
		FILESET_READ_LOCK(&FilesetLock);
		for (fsp = FilesetHead; fsp != NULL; fsp = fsp->fsNext) {
			if (BS_UID_EQL(domainId, fsp->dmnP->domainId)) {
				/* create a list of the mounted filesets in
				 * the same domain as volume being removed is
				 * in. */
		...
		for (fsMntp = fsMntHeadp; fsMntp != NULL; fsMntp = fsMntp->fsNext) {
			/* check the mounted filesets in the same domain as
			 * volume being removed is in. */
			sprintf(fnamep, "%s/.tags/M%d",
			    (char *) fsMntp->mountpt,
			    (int) bfap->tag.num);	/* build a path to this
							 * reserved file */

			ndp->ni_nameiop = LOOKUP | FOLLOW;
			ndp->ni_segflg = UIO_SYSSPACE;	/* retrieve as from
							 * kernel */
			ndp->ni_dirp = fnamep;
			if (!namei(ndp)) {

				/* on ENOENT, continue on to next mounted
				 * filesystem in this domain since this
				 * fileset is not the one holding it opened. -
				 * otherwise - found it!, get the vnode and
				 * check the usecount the usecount tells us
				 * that if its 1, we are the only current
				 * accessor and its on the free list because
				 * it hasn't been reclaimed - and may not be
				 * for quite a while. In this case we force
				 * the reclaim with a vgone.
				 * 
				...
				VN_LOCK(vp);
				if (vp->v_usecount == 1) {
					/* free the vnode/bfap with vgone()
					 * because it is just the namei above
					 * that has the v_usecount at 1. In
					 * the process of namei we increment
					 * the access struct and if the vnode
					 * is on the free list (but not
					 * currently accessed) we obtain the
					 * vnode.  This is the case of a
					 * cluster file system -CFS
					 * i.e.hanging onto the access struct
					 * by way of its vnode cache. */
					...
					/* v_usecount indicates someone from
					 * user mode is holding it */
					VN_UNLOCK(vp);
					vrele(vp);
	...
	}			/* end if refCnt is too high, someone has it
				 * opened or has recently closed it */
do_cleanup:
...
/*    KASSERT(lk_get_state(&bfap->stateLk) == ACC_INIT_TRANS);
	...
	if (bfap->dmnP->state != BFD_ACTIVATED) {
		...
		/* Once the domain is activated we can do a full check of the
		 * mcell. */
...
retry_clu_clone_access:
	...
	if (bfaccState == ACC_VALID) {
		...
		if (BS_BFTAG_PSEUDO(tag)) {
			...
			/* During CFS failover, check the DDL for unlinked
			 * files */
			...
			if (vdIndex == 0) {
				/* this normally only happens if a user opens
				 * .tags/-2 */
				sts = ENO_SUCH_TAG;
				goto err_vrele;
			}
	...
	if (bfap->bfState == BSRA_VALID) {
		 ...
		 * 
		 * The bitfile is neither VALID nor DELETING, therefore it must
		 * be in one of the ftx transition states. */
	...
	if (!vp || got_clu_clone_vnode) {
		...
		/* Note: if got_clu_clone_vnode is TRUE here, it means we
		 * preallocated a vnode but didn't attach it to the access
		 * structure.  We still might or we might release it. */
...
err_setinvalid:
	 ...
	 * grab_bsacc waiters who were waiting for the state to transition out
	 * of ACC_INIT_TRANS. Since the state is set to ACC_INVALID here,
	 * those waiters would all unknowingly race on resetting the entry. */
	if (bfap->hashlinks.dh_links.dh_next != NULL) {
		BS_BFAH_REMOVE(bfap, FALSE);
	} else {
		BS_BFAH_UNLOCK(bfap->hashlinks.dh_key);
...
lookup:
	/* Check if access struct with this tag is already in hash table. If
	 * so, return it with its bfaLock seized. */
...
found:
		...
		if ((lk_get_state(&bfap->stateLk) == ACC_INVALID) &&
			...
			/* Retry the access entry search if an undesired state
			...
			/* Return to the caller holding the bfaLock.  The
			 * caller must test for states other than INIT_TRANS
		...
		/* Not in the hash table, so get a new bfAccess struct
		 * (locked) */
	...
	/* Initialize the new/recycled bfAccess struct. If the bfAccess is
	 * being recycled, its last vnode will not be reused so it should be
	 * vgone'd to put it at the front of the vnode freelist. The following
	 * object cleanup duplicates some of the processing in msfs_reclaim(),
	 * but a call to vgone() at this point would wait for ACC_RECYCLE in
	 * find_bfap().  Once the bfAccess is removed from the hash chain for
	 * its previous bitfile and bfap->bfaLock is released for a call to
	 * vgone(), a new access to the previous bitfile may be found when
	 * msfs_reclaim calls find_bfap() which would prevent it from cleaning
	 * up the object.  Therefore, the code duplication is necessary. */
	vp = bfap->bfVp;
	if (vp) {
		VN_LOCK(vp);
		/* Hold off deallocation via CACHE_LOOKUP_REF instead of using
		 * a vget call, as the corresponding vrele could trigger a
		 * hierarchy panic in a CFS/Advfs scenario */
		CACHE_LOOKUP_REF(vp);
		bnp = (struct bfNode *) & vp->v_data[0];
		saved_v_id = vp->v_id;
		saved_tag = bnp->tag;
	 ...
	 * bfap call but we noted the insert count at that time. If nothing
	 * has been inserted on that chain since then, we can assume find_bfap
	 * will fail again. If on the other hand an insertion has taken place
	 * to this chain then we better make sure we aren't racing another
	 * grab_bsacc with the same tag and bfsetp. In either case the hash
	 * chain lock will protect us from an identical access structure going
	 * in (we'll be holding it after find_bfap). */

	if (insert_count_after != insert_count_before) {
		BS_BFAH_UNLOCK(hash_key);

		/* Check to see if this access struct may have been added by
		 * another thread while we were messing around in the above
		 * code; if so, use it. Otherwise, set up our bfap to use.
	...
	/* If this bfap was last used by a directory with an index or an index
	 * itself, then free up the associated structure */
...
)
{
	 ...
	 * internal closes and shouldn't cause the file to be fragged. By only
	 * allowing the last external close to frag the file we insure that
	 * the vnode is available, internal opens may not have a vnode on the
	 * file. */

	if (options & MSFS_INACTIVE_CALL) {
		/* Notify vfast with a FILE_FRAGRATIO_UPDATE that file is
		 * being closed by the last non-kernel closer. Update the
		 * file's fragmentation stats now. We avoid a hierarchy lock
		 * problem between bfaLock and xtntMap_lk used in
		 * ss_chk_fragratio by taking the bfaLock after
		 * ss_chk_fragratio. */
	 ...
	 * because the other bumps of the access count are internal opens of
	 * the orignal from a clone.  The last close from a clone may not have
	 * a vnode for the original and the call to chk_blk_quota will panic.
	 * It is safe to frag and trunc now because :
	 * 
	 * 1) Clones are read-only and can not cause the file to be fragged or
	 * truncated. 2) Fragging at this point means the last page of the
	 * file was modified which caused it to COW over to the clone. 3)
	 * Truncation at this point cause the pages to be cowed over to the
	 * clone.
	 * 
	 * An internal open may also be done by clear_bmt() when processing an
	 * rmvol.  In this case, the RMVOL_TRUNC_LOCK serialization prior to
	 * decrementing bfap->accessCnt allows safe fragging or truncating
	...
	if (FTX_EQ(&parentFtxH, &FtxNilFtxH) && !is_clone) {
		 ...
		 * the quota files at file creation, chown or explicit quota
		 * setting. The quota entry already exists in the quota file
		 * so the chk_blk_quota will NOT add storage. The potential
		 * danger path would be chk_blk_quota --> dqsync -->
	...
	if (bfap->bfState == BSRA_DELETING) {
		...
		if (deleteIt && (options & MSFS_INACTIVE_CALL)) {
			...
			if ((bfap->bfVp) && (VTOC(bfap->bfVp))) {
				...
				/* Use bfattrp->state as a flag during
				 * recovery. */
				/* If it is BSRA_DELETING then we crashed
				 * before we could */
				/* set the quotas. If it is BSRA_INVALID we
				 * got the quotas */
				/* set but we may not have deleted the storage
				 * yet. */
		...
		if (bfap->accessCnt == 0) {
			...
			if (deleteIt) {
				...
				/* Defer the stg deallocation until after all
				 * locks are released.  */

				delMCId = bfap->primMCId;
				delVdp = VD_HTOP(bfap->primVdIndex, bfap->dmnP);

				/* If we are deleting the file, we don't need
				 * to worry about flushing the stats to disk.
				 * Also we don't want this access structure to
				 * get on the ClosedAcc list. It may be even
				 * better to test for BSRA_INVALID in
				 * free_acc_struct if that guarantees that the
				 * access structure is no longer being used. */
		...
		if (bfap->trunc) {
			...
			if (delCnt == 0 && madeFrag) {
				/* We made a frag but could not truncate the
				 * last page. Delete the frag and leave the
				 * last page of the file. Note quota was not
				 * increased so we don't decrease. */
	...
	/* Update vfast frag list when a file has been truncated or deleted. */
	if ((BS_BFTAG_REG(bfap->tag)) &&
	    (bfap->xtnts.allocPageCnt > 1) &&
	    (bfap->ssMigrating != SS_POSTED) &&
	    !(options & MSFS_SS_NOCALL)) {
...
_close_it:
	...
	if (bfap->accessCnt == 0) {
		...
		/* To make certain that access structs do not lie around on
		 * the closed list.  Clear dkResult if we are not CFS, NFS or
		 * type VREG */
	...
	/* if we should release the vnode lets check to make sure there is
	 * one.  It is possible to have an access structure with out a vnode. */
...
)
{
	...
	if (!added_bfap_to_list) {
		if (lk_get_state(&bfap->stateLk) == ACC_FTX_TRANS) {
			...
			vm_object_lock(bfap->bfObj);	/* lock while derefing
							 * object flds */
			if (bfap->bfObj->vu_dirtypl == NULL &&
			    bfap->bfObj->vu_dirtywpl == NULL &&
			    bfap->bfObj->vu_putpages == 0) {
				CHECK_ACC_CLEAN(bfap);
				/* unlock to avoid lock violation with
				 * BfAccessFreeLock */
...
check_mv_bfap_to_free(bfAccessT * bfap)
{
	 ...
	 * associated with this access struct 3. The stats are dirty; must
	 * stay on the closed list 4. dkResult != EOK (possibly unreported
	 * error) */
...
bs_dealloc_access(bfAccessT * bfap)
{
	...
	/*
         * Mutex locks.
         */
	mutex_destroy(&bfap->bfaLock.mutex);
	mutex_destroy(&bfap->bfIoLock.mutex);
	mutex_destroy(&bfap->actRangeLock.mutex);
		...
		while (listp != (actRangeT *) actListHead) {
			if (arp->arStartBlock <= listp->arEndBlock) {
				if (arp->arEndBlock >= listp->arStartBlock) {
					/* Range conflict. Return NULL, and
					 * load address of conflicting range
					 * if requested. */
					if (conflictp)
						*conflictp = listp;
    ...
    struct fsContext * contextp)
{
	 ...
	 * actRanges on the wait list since they are not kept in sorted order.
	 * Also, known scenarios such as migrate and file truncation tend to
	 * take out large active ranges that will likely conflict with most
	 * other ranges.  Well-behaved directIO applications will avoid
	 * interthread range conflicts so this should not be a problem for
	 * them. */
	if (bfap->actRangeList.arWaitCount)
		goto put_onto_waitlist;

	/* No waiters in front of us, so let's see if we can get onto the
	 * active range list. */
...
put_onto_waitlist:
	/* Enqueue our actRange at the end of the wait list and go to sleep.
	 * When the conflicting range has been removed, the thread that
	 * removes the range will put our range onto the active list and wake
	 * us up. */
	...
	if (contextp) {
		/* Don't hold file_lock while sleeping; first, check to see
		 * whehter it is held exclusively or shared so we can reseize
		 * it in similar mode when we wake up. */
    ...
    actRangeT * arp)
{
	...
	/* If any waiters, wake them up.  This involves walking the queue of
	 * waiters.  For each entry, if it can now be enqueued, do so and wake
	 * the waiting thread.  If it cannot be enqueued because of a
	 * conflicting active range, bump arWaiters on the active range and
	...
	if (arp->arWaiters) {
		...
		while (waitp != (actRangeT *) (&bfap->actRangeList)) {
			...
			if (!actp) {
				/* Can't move this one to active; our work
				 * here is done! First, bump wait count on
				 * range in conflict. */
				carp->arWaiters++;
				break;
			} else {

				/* remove waitp from wait queue, and insert
				 * before actp. */
				...
				/* only one thread ever sleeps on a given
				 * range */
...
)
{
	...
	while (listp != (actRangeT *) & bfap->actRangeList) {
		/* check if the block at the beginning of the page is in the
		 * active range. */
		if (listp->arEndBlock >= block_bop &&
		    listp->arStartBlock <= block_bop) {
			return listp;
		}
		/* check if the block at the end of the page is in the active
		 * range. */
