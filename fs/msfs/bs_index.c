...
)
{
	...
	/* Put the creation of the index under a subftx so we can have an undo
	 * routine clean up bogus MCELL records. */
	...
	/* Allocate 2 pages of storage for the index file starting at page 0.
	 * Charge the space against the directory since the index context area
	 * is not maintained. */
	...
	/* Get memory for the index parameters that hang off the directory's
	 * access structure. The memory for this will be released in
	...
	/* Get memory for the parameters that hang off the index file's access
	 * structure. The memory for this will also be released in grab_bsacc
	 * when the access structure is recycled.  */
	...
	/* This deserves some explaination. The bmtr_put_rec routine does not
	 * supply and undo for the creation of a record. Since it itself
	 * starts a subftx, the record could get out to disk at the done of
	 * that subftx. Thus the undoing of this record must actually be setup
	 * prior to putting it out. In this way the undo routine can attempt
	 * to read the record and if successful put in a Nil tag value. If it
	 * is not sucessful then the undoing is skipped.
	 * 
	 * If we were to put the record out prior to the done then a small window
	 * exits between putting the record out and doing the done. This could
	 * lead to the record reaching the disk without the undo routine
	 * running. */
...
)
{
	...
	/* Check to see if the directory access structure passed in still has
	 * the index file parameters cached.  */
	...
	if ((bsDirIdxRecT *) dir_bfap->idx_params == NULL) {
		...
		if ((sts != EOK) || (BS_BFTAG_EQL(index_tag, NilBfTag))) {
			 ...
			 * Indicate to future opens that this is a non-indexed
			 * dir. This will change only when a index is created
			 * or the access strucutre is recycled. */
			/* Return an EOK status since no index file is not an
			 * error condition. The callers can check the
			 * idx_params field to find out if an index was found
			 * or not. */
	 ...
	 * idx_bfap indicating that the index must be re-openned. If it is not
	 * NULL then it must already be openned and the association setup. */
	...
	if (idx_bfap == NULL) {
		 ...
		 * in bs_access_one where the index file is in the process of
		 * being created and the code doesn't realize we were the
		...
		if (sts != EOK) {
			...
			/* The index file is deleted before the directory so
			 * we may have caught it in that state.  The cluster
			 * must be allowed to access the dir it had open or it
			 * will crash. */
		...
		/* Check and see if the index file's access has been recycled
		 * and if so re-read in the index file parameters.  */
	}
...
)
{
	 ...
	 * and .. entries must be removed even if the dir is still open. This
	 * is the case where you cd into the dir and then remove it. The . and
	 * .. are not removed from the index so we don't want any lookups
	 * finding them. */
	switch (action) {
	...
	case IDX_NORMAL_CLOSE:
		/* Indicate to any future dir openners to also open the index
		 * file */
		if (idx_bfap->idx_params != NULL)
	...
	case IDX_REMOVING_DIR:
		...
		/* Currently the only way to remove and index is to also
		 * remove the associated directory. Thus we will account for
		 * the quota's at the last close of the directory. This is
		 * because of the difficulties of the index using the
		 * directories context structure for the call to
		 * chk_blk_quota. Since the directory is being removed we can
		 * not guarantee its existence at the last close of the index
		 * (ie the index is held open via .tags). Thus we will include
		 * the quota blocks in the dir's access structure so it can
		 * update the quota's at last close. */
...
idx_bsearch_int(
    ...
    int last_node		/* flag indicating if last node of an internal
				 * level */
)
{
	int start;
...
)
{
	...
	if (count > IDX_MAX_BTREE_LEVELS) {
		 ...
		 * requeset means we want to split the tree past the largest
		 * allowable size. This restricition exists becasue of nummber
		 * of pinned pages per sub ftx not due to b-tree limitaions.
		 * It also means the caller has over 4 billion files in this
		 * directory! */

		return (ENO_MORE_BLKS);
	}
	/* Normally one should call x_load_imm_xtnt_map here but that is only
	 * necessary in case the file was truncated. Otherwise the maps were
	 * brought in via bs_map_bf. Since index files can not be truncated we
	 * only need to get the xtntMap lock.
	 * 
	 * It is not necesary to get the XTNTMAP_LOCK_READ here because all
	 * callers of this function currently hold the MIGTRUNC lock
	...
	/* Put the pages in the array for pinning after all the storage has
	 * been obtained. This is necessary in case of an error because we can
	 * not remove a page that has been pinned so we most postpone pinning
	 * until after we are sure we wont fail.  */
...
)
{
	...
	/* Hash the filename to get the search key to insert into the index
	 * b-tree.  */
	 ...
	 * instead of the root, thus avoiding having to traverse the tree.
	 * 
	 * There is no point using the hint if the tree has only one node (level
	 * =0). Furthermore, this takes care of initizing the hint. A rename
	 * can do 2 nameis on a non-indexed directory and then the insert
	 * causes the dir to get an index The remove will then be called with
	 * an uninitialized hint. By not usinging this early we'll get a
	 * chance to init it. */
	...
	split_count = -1;	/* indicates to the recursive routine that we
	...
	if (insert) {
		 ...
		 * pinned for us.
		 * 
		 * Increment level to indicate that the b-tree has increased in
		 * size.  Place the new elements in to the new root node and
		 * pin the record.  It is not necessary to update the search
		 * key of the last element of a root node since this contains
		 * all values greater than the previous element.  */
	...
	/* We have reached a point now where it is safe to stop all undo's
	 * below us. The b-tree is in a consistent state and does not need to
	 * be undone if this ftx is failed. We will provide our own undo
	 * routine to remove the filename only NOTE: This will not disable the
	 * undo for this current ftx. */
...
)
{
	 ...
	 * the page is going to be modified due to a split, just pin it
	 * instead of reffing it. If we never do an rbf_pin_record then the
	 * page will not be marked dirty.  */
	 ...
	 * split_count.  Recursively call insert again if this is not a leaf
	 * node.  */
	...
	if (level > 0) {
		...
		if (node_ptr->total_elements == IDX_MAX_ELEMENTS) {
			 ...
			 * we may need to split then ask for an extra page
			 * since our caller will need to create a new root
			 * node.  */
		 ...
		 * 
		 * The work around is to bump the max allowed pinned pages per
		 * sub-transaction from 7 to 9 or rework my
		 * idx_index_get_free_pgs_int to not pin the pages and start a
		 * sub-transaction for each recursive call.  */
		 ...
		 * (index) will be rippled up with the new offset and the new
		 * slot gets the passed back search_key. The split caused the
		 * existing node to now have a new search key and the new node
		 * to have the original search key.  */
		...
		*insert = TRUE;	/* we know at least the leaf must perform an
				 * insertion */
	...
	/* Check if an insertion needs to be done. Split the node if it needs
	 * it and insert the new element.  */
	...
	if (*insert) {
		if (node_ptr->total_elements == IDX_MAX_ELEMENTS) {
			...
			/* Update the return arguments so the caller can
			 * insert our new node into its search array. The
			 * offset refers to the new node while the search_key
			 * refers to the old node.  */

			*offset = pptr[level].pgno;
			*search_key =
			    node_ptr->data[node_ptr->total_elements - 1].search_key;

			/* We maintain a singly linked list amongst the nodes
			 * of the tree.  Set up the new split nodes to point
			 * right. The leftmost bit will always be FALSE since
			...
			/* Indicate to the caller that an insertion must be
			 * done.  */
...
)
{
	...
	/* Hash the filename to get the search key to remove from the index
	 * b-tree. */
	...
	do {
		...
		if (!found) {
			if (!using_hint) {
				/* Normally this would be a domain panic
				 * situation but this could be called from a
				 * specialty routine trying to "fix" the
				 * domain. It will be the callers
				 * responsibilty to deal with the error */
	...
	/* The root done routine is our pruning routine. Lead it to the node
	 * that needs pruning via the search_key.  */
	...
	if (((bsIdxRecT *) idx_bfap->idx_params)->prune_key_fname) {
		/* Don't bother checking for status. Let the caller fail the
...
)
{
	...
	if (level > 0) {
		 ...
		 * continue.
		 * 
		 * Since collisions could have existed and then were removed we
		 * may be pointing at the end of a node. This is not wrong
		...
		while ((index == node_ptr->total_elements) ||
			...
			/* If we're at the end of the node then pin the next
			 * page and start with it.  */
			...
			if (index == node_ptr->total_elements) {
				...
				/* A special case exists in which a rename
				 * first does the insertion of the new
				 * filename, which can cause the node to
				 * split. This may invalidate the remove hint
				 * if the filename to be removed split over to
				 * a different node. In this case we will
				 * return FALSE and cause the b-tree to be
				 * searched.  */
			...
			/* This is the one to be removed so remove it by
			 * rippling down the elements and pining.  */
			...
			if (node_ptr->data[index].loc.dir_offset == offset) {
				...
				/* Determine if a the potential of a tree
				 * pruning exists and indicate so in the index
				 * access strucuture */
		...
		/* A special case exists in which a rename first does the
		 * insertion of the new filename, which can cause the node to
		 * split. This may invalidate the remove hint if the filename
		 * to be removed split over to a different node. In this case
		 * we will return FALSE and cause the b-tree to be searched. */
...
)
{
	...
	while (!found) {
		if (index == node_ptr->total_elements) {
			/* We are at the end of the node. We must look at the
			 * next node because of potential collisions.  */
			...
			if (sts != EOK) {
				/* If we run out of space attempting to clone
				 * the directory then we will not be able to
				 * read the direectory. Don't bring down the
				 * domain, just return an EIO error to the
				 * caller which will not allow access to the
				 * directory */
				if (sts != E_OUT_OF_SYNC_CLONE) {
					/* Ok we've got problems bring the
					 * domain down */
		...
		if (search_key == node_ptr->data[index].search_key) {
			...
			/* We have matching hash values. This does not mean
			 * the filename strings match so we must look at the
			 * string in the directory file that this hash value
			 * corresponds to. */
			...
			if (flag & PIN_PAGE) {
				...
				if (sts != EOK) {
					/* If we run out of space attempting
					 * to clone the directory then we will
					 * not be able to read the direectory.
					 * Don't bring down the domain, just
					 * return to the caller which will not
					 * allow access to the directory */
					if (sts != E_OUT_OF_SYNC_CLONE) {
						/* Ok we've got problems bring
						 * the domain down */
			...
			/* Filename's did not match see if more collisions
			 * exist */
...
)
{
	...
	if (sts != EOK) {
		 ...
		 * down the domain, just return to the caller which will not
		 * allow access to the directory */
...
)
{
	...
	/* Get the idx_file's access structure and setup for recursive call.
	 * Here the search key is the address of the free space.  */
	...
	/* This may seem confusing but the leaf nodes have the address of the
	 * storage as their search keys and the size as the size. The
	...
	if (sts == INSERT) {
		 ...
		 * size.  Place the new elements in to the new root node and
		 * pin the record.  It is not necessary to update the search
		 * key of the last element of a node since this contains all
	...
	/* We have reach a point now where it is save to stop all undo's below
	 * us. The b-tree is in a consistent state and does not need to be
	 * undone if this ftx is failed. We will provide our own undo routine
	 * to remove the space only */
	...
	if (((bsIdxRecT *) idx_bfap->idx_params)->prune_key_ffree) {
		...
		if ((sts == ENO_MORE_BLKS) || (sts == ENO_MORE_MCELLS)) {
			/* We need do distinguish errors here for callers
			 * above us. Since we could have failed attempting to
			 * insert space due to a node split requesting blocks.
...
)
{
	 ...
	 * If this is the first caller (split_count < 0) and we may need to
	 * split then ask for an extra page since our caller will need to
	 * create a new root node. Recursively call insert again if this is
	 * not a leaf node.  */
	...
	if (level > 0) {
		...
		if (ret == UPDATE) {
			/* This is a special case. The search_key that sends
			 * us to our child is too great. A glomming caused our
			 * search_key to now partially reside on the node next
			 * to our child. We must update the range. */
		...
		if (ret == UPDATE) {
			 ...
			 * us to our child is too great. A glomming caused our
			 * search_key to now partially reside on the node next
			 * to our child. We must inform our parent */
		...
		if (ret != INSERT) {
			...
			/* We were able to glom an entry together. No insert
			 * is needed. Return our status up.  */

			return (ret);
		}
		/* Get any resources that will be needed to be passed back up,
		 * before we pin any records and are still able to fail.
		 * 
		 * If we are unable to get storage for the split then return this
		 * to our caller since we must act like the insert was
		 * successful.  */
	 ...
	 * 
	 * Check if an insertion needs to be done. Split the node if it needs it
	 * and insert new element. rbf_pin_record any ranges that were
	 * modified.  */
	...
	if (ret == INSERT) {
		if (node_ptr->total_elements == IDX_MAX_ELEMENTS) {
			...
			/* Update the return arguments so the caller can
			 * insert our new node into its search array. The size
			 * (range) refers to the new node while the search_key
			 * refers to the old node.  */
...
)
{
	...
	if (found_size > 0) {
		...
		/* We need to finish the transaction here in case we end up
		 * calling idx_directory_insert_space. Since
		 * idx_directory_insert_space could fail we want our caller to
		 * be able to fail us.. */
		...
		if (found_size == 0) {
			 ...
			 * directory No undo routine is necessary just finish
			 * the ftx */

			ftx_done_n(ftxH, FTA_IDX_UNDO_V1);
			return (TRUE);
		} else {
			/* Looks like we took an error and domain paniced send
			 * the status up */
			ftx_fail(ftxH);
...
)
{
	...
	/* We are at the leaf node so locate the first element that satisfies
	...
	while (TRUE) {
		...
		if (node_ptr->data[index].loc.free_size >= requested_size) {
			/* We think we found space that meets our
			 * requirements.  Since the directory file deals in
			 * 512 byte blocks we must only hand back space that
			 * is within a 512 byte block. It can not cross the
			 * 512 boundary ( This restriction is artifical. It is
			 * a UFSism that we do not need to maintain because
			 * the library routines do not handle filenames
			 * crossing a 512 byte boundary when returned for
			 * getdirents)
			 * 
			 * The free space tree glomms together as much space it
			 * can with no regard for block or page sizes. Thus we
			 * must only hand out space up to the nearest 512 byte
			 * block and if that happens to not be enough then see
			 * if the second half is enough and if thats not
			 * enough keep looking.  */
			...
			/* Because we have to carry around this cookie at the
			 * end of each directory page, the dir_record we must
			 * account for it in our calculation but hand it out
			 * as free space */
			...
			if (required_size <= (rounded_free_size - dirRec_adj)) {
				*file_offset = start_addr;
				...
				/* We have the space to satisfy our request.
				 * Now try to save work by peeling out the
				 * exact amount we need (this may be more that
				 * the requested size).
				 * 
				 * We have certain restrictions that will force
				 * us to return a large chunk of free space
				 * and make the calling routine re-insert the
				 * smaller pieces. This is because we search
				 * the leaf nodes linearly and can not return
				 * to the proper parent. So any modifications
				 * done now must not result in the parent node
				 * needing a modification. If it does then a
				 * formal insert must be done.
				 * 
				 * No shortcut can be performed if any of the
				 * following hold true.
				 * 
				 * 1) The leaf search_key will change and we are
				 * the last entry in the node.
				 * 
				 * 2) An insertion to this node must take place
				 * and the node is already full.  */

				/* We can not cross the 512 byte boundary, so
				 * we will peel out what we need and return
				 * the rest back to the free tree unless we
				 * are the last element in the node in which
				 * case we'll return the whole thing. Except
				 * (*sigh* so many exceptions) when the node
				 * happens to be the last node (or only node)
				 * in the tree then we can safely alter the
				 * last entry in the node. This is because the
				 * last node represents everything greater
				 * than the previous node and we will only be
				 * increasing the starting address */
				...
				if ((rounded_free_size - required_size - dirRec_adj) <
					...
					/* This may be more space than
					 * requested, but not enough to break
					 * into two. So we'll give it all
					 * back. */
				...
				if (required_size < free_size) {
					if ((index < node_ptr->total_elements - 1) ||
					    (node_ptr->page_right < 0)) {
						/* Take a short cut here and
						 * do the insertion ourselves.  */

						/* We now know exactly how
						 * much space we will now
						 * check to see if we can save
						 * work by adjusting the
						 * existing chunk of free
						 * space if it is larger that
						 * what we need.  */
						...
						/* Return the exact size to
						 * the caller indicating that
				...
				/* We need to pull the whole thing out of the
				 * tree. This is because either it exactly
				 * meets our needs or we were unable to adjust
				 * the existing entry and a re-insertion is
				 * required. */
				...
				/* Update the requested size in case it needed
				 * to encompass space that is less that the
				 * min dir size or that last record on a page.
				 * This will inform the initial caller to
				 * account that the returned space is slightly
			...
			/* The first half of the space did not satisfy the
			 * request so look at the second half (if there even
			 * is one) */
			 ...
			 * end of each directory page, the dir_record we must
			 * account for it in our calculation but hand it out
			 * as free space NOTE since we are dealing with the
			 * second half we know we will never have to include
			 * the dirRec since we are at the beginning of a 512
			 * boundary and the filename length MAX is
			 * FS_DIR_MAX_NAME = 255. I will keep the dirRec_adj
			 * in for completeness but it is unecessary */
			...
			if ((rounded_free_size != 0) &&
				...
				*file_offset = start_addr;
				...
				/* The second half will satisfy the request.
				 * So at least get that ready. */
				 ...
				 * effecting our parent. This will be the case
				 * as long as we do not change the last slot
				 * in the node or we do not have a right
				 * sibling.
				 * 
				 * Since we ripple the elements up one slot the
				 * current index can not be the last slot. */
				...
				if ((rounded_free_size - required_size - dirRec_adj) <
					 ...
					 * returned size every thing taken out
					 * of the tree (this may include
					 * anything less than
					 * MIN_DIR_ENTRY_SIZE) so the undo
					 * record knows exactly what to put
					 * back !  */
...
)
{
	...
	/* Calculate the number of pages that can be truncated and remove them
	 * from the free space b-tree.  Start a subtransaction so we can
	 * supply an undo function. We will allow the stg_remove_stg to put
	 * the storage back if it is failed and we will do likewise.  */
	...
	if (operation == TRUNC_QUERY) {
		if (bytes_to_trunc == 0) {
			...
			/* Here's the deal. A prior filename deletion noticed
			 * that the   directory   could   be   truncated  and
			 * set  the IDX_TRUNCATE flag.
			 * 
			 * A few things could have taken place before us,
			 * multiple deletions (which will  not change the need
			 * to truncate, a failed truncation (we  could fail to
			 * remove  storage because we  couldn't get an  mcell)
			 * in  which case  the flag will still be set and the
			 * truncation still needed, an insert into the page
			 * that was about to be truncated, or an insert
			 * anywhere but the truncation region.
			 * 
			 * If  the  truncation  were to fail  because we could
			 * not remove  storage, we  will not  fail the
			 * insertion that triggered the  truncation.  This
			 * means if we are tight on  space,  many  insertions
			 * can  take  place  without truncation   succeeding.
			 * But   we   will   keep   the IDX_TRUNCATE flag set
			 * in hopes of eventually succeeding the truncation.
			 * 
			 * We  must make  sure that the  first attempt to
			 * truncate which  detects that  the  directory  can
			 * no  longer be truncated  ( because we inserted
			 * into  the last page ) clears the flag.
			 * 
			 * The  danger  here is a  full  page at  the  end  of
			 * the directory could mistakenly be  truncated off.
			 * If  there was an empty page  proceeded by a
			 * completely full page we would  think the  directory
			 * could  be truncated. By clearing the flag at the
			 * first insert into that page we can avoid this.  */
...
)
{
	...
	if (level > 0) {
		...
		if (node_ptr->total_elements == 0) {
			 ...
			 * elements due to our pruning policy. Bring in the
		...
		if ((num_bytes & (ADVFS_PGSZ - 1)) > 0) {
			...
			if (operation == TRUNC_UPDATE) {
				...
				/* Help out the undo routine by telling it the
				 * last offset in the directory of where this
				 * should be returned if we are failed. It
				 * should be glommed onto this entry  */
			...
			if (operation == TRUNC_UPDATE) {
				/* This is exactly a page multiple. Remove the
				 * entry from the node by decrementing the
				 * total_elements; */
				...
				/* Help out the undo routine by telling it the
				 * last offset in the directory of where this
				 * should be returned if we are failed. The
				 * entire entry should be put back  */
...
)
{
	...
	/* Start by glomming with the entry to our left. It may be necessary
	 * to bring in an entire node.  */

	dir_bfap = ((bsIdxRecT *) idx_bfap->idx_params)->dir_bfap;

	/* The passed in index is really the right node that we must check for
	 * glomming, except for the case below. */
	...
	if (index == 0) {	/* Our left neighbor lies on another node ? */
		if (node_ptr->sib.page_left >= 0) {	/* yep */
			...
			/* If the previous entry is on another node then bring
			 * it in and check the last entry in that node. Note
			 * as discussed above we will check for greater than
			 * or equal to in case of overlap.  */
			...
			left_node_ptr = NULL;	/* We have no left sibling */
	} else {
	...
	/* The following tests if the entry being glommed overlaps with the
	 * previous entry. If it does then it should either totally overlap or
	 * straddle a 512 byte boundary */
	...
	if (index == node_ptr->total_elements) {
		...
		if (node_ptr->page_right >= 0) {
			...
			if (right_node_ptr->total_elements == 0)
				right_node_ptr = NULL;
			right_index = 0;
			update_warning = TRUE;
		} else
			right_node_ptr = NULL;	/* We have no right neighbor */
	} else {
	...
	if (reuse_ptr != NULL) {
		 ...
		 * than or equal which indicates that we either are adjacent
		 * or overlap in which case glomming can be done */
		...
		if ((right_node_ptr != NULL) &&
			...
			if ((right_node_ptr->total_elements <= IDX_COMPRESS_ME) &&
				...
				/* We glommed entry from the right node over
				 * to the left node. In the process we notice
				 * that the right node dropped below the
				 * compression threshold and needs to be
				 * prunned. Since the original search brought
				 * us to the left node we need to force the
				 * prune to land on the right node so we must
				 * use a search key from the right node. */
		 ...
		 * This test used to be done because the directory was
		 * sequentially walked. Now we must sense it. We will cause a
		 * directory truncation if the glommed entry is a page or more
		 * and is the last entry in the b-tree (and therefore the last
		 * page of the directory file).  */
	...
	if (right_node_ptr->data[right_index].search_key - start_addr <= size) {
		...
		if ((right_index == 0) && (update_warning)) {
			/* Oops we spanned two nodes. We must inform our
			 * parent to update its range. */
			return (UPDATE);
		}
		return (DONE);
	}
...
)
{
	...
	/* Initialize each field in the root node. The NULL value for a page
	 * is -1.  These are unsigned so it corresponds to the largest page
	 * number which should never be reached in the index file.  */
	...
	/* The leftmost flag will only be set TRUE in a root node. When a new
	 * root is added this node will always be the leftmost since pruning
	 * and splitting always take place to the right.  */
	...
	/* Loop through each entry in the directory. If it is a filename place
	 * it's hash value in the filename node, if it is free than place it
	 * in the free space node. We know here that we do not have to deal
	 * with glomming 512 byte chunks since this routine is only called
	 * when a second page is needed for the directory and that would not
	 * be the case if there were two 512 byte chunks that needed glomming.  */
	...
	while (dir_p <= last_p) {
		...
		if (dir_p->fs_dir_header.fs_dir_bs_tag_num == 0) {
			 ...
			 * that the previous free space entry was not the last
			 * piece of a 512 byte boundary in which case we can
			 * glom these two together.  */
			...
			if ((free_idx > 0) &&
				...
				/* If its the last entry in the page then add
				 * the entire remaining space (including the
				 * last record) and get out */
		...
		if (FS_DIR_SIZE_OK(n, offset)) {
			/* there has been a minor data corruption, inform and
			 * error out */
	...
	/* The free space node is already sorted but the filename node needs
	 * to be sorted so do so now ... */
	 ...
	 * needs to be set up noe because the insertion that is causing this
	 * routine to run did not do a lookup via the index file but will do
	 * an insertion into the index file and check the hint if the hint is
	 * not initialized here bad things could happen. */
...
)
{
	...
	for (i = FNAME; i <= FFREE; i++) {
		...
		if (search_key > 0) {
			...
			if (sts < 0) {
				...
				/* We double duty status. If it is less than
				 * zero then we have and error condition and
				 * need to fail out */
			...
			if (new_root != old_root) {
				/* We can tell which btree by comparing the
				 * root page to the passed in page since the
				 * root nodes can not occupy the same page. */
				...
				/* Push the new root page out to the MCELL
				 * record. */
			...
			/* As explained above, disable the undoing of this sub
			 * ftx upon completion */
	...
	finish_prune:
			 ...
			 * to call stg_remove_stg_finish. Normally this must
			 * be done after the root ftx but in this case
			 * disabling the undos is about the same thing. */
...
)
{
	 ...
	 * compression will be into the left node from the right node, except
	...
	if (level > 0) {
		...
		do {
			adjust_range =
			    ...
			    ftxH);
			if (adjust_range < 0) {
				/* We are in trouble. The value in
				 * adjust_range is an error status. Pass it
				 * back up */
				return (adjust_range);
			}
	...
	if ((level == root_level) &&
		...
		if (sts != EOK) {
			/* We can get an ENO_MORE_MEMORY here so just and pass
			 * the status up. */

			/* If the root has two elements and we've successfully
			 * pruned one of its children, we need to reduce the
			 * root's total_elements to remove any reference to
			 * the now-removed child. */
	...
	/* Check to see if this node can be compressed. If its size is less
	 * than 1/4 full then compression will take place. If it is a leaf
	 * node and we are above the threshold then we probably have have a
	 * collision spanning two nodes in which case the search key actually
	 * is for the right sibling */
	...
	if ((dst_node_ptr->page_right >= 0) &&
		 ...
		 * and we are at the last slot of the node. This is because
		 * the slot that needs to be removed actually lives on our
		 * sibling. by removing it from our sibling we could cause it
		 * to drop below the prune threshold. By forcing a compression
		 * we will insure the sibling never drops below. */
		...
		/* The only way we can be a non-leaf node that needs to be
		 * compressed, is if a child below us was removed causing this
		 * node to need an entry removed (at index). Incorporate this
		...
		if (adjust_range == REMOVE) {
			/* If the node below us that was removed had its
			 * parent as our sibling then we will catch it here */
			...
			if (index_to_remove <= dst_node_ptr->total_elements - 1) {
				...
				/* The index to remove lies on the other node.
				 * This is yet another special case situation.
				 * We need to be careful here not to allow the
				 * sibling node to drop below the prune
				 * threshold. If we did then eventually it
				 * could reach zero which can not be tolerated
				 * 
				 * Since this is a relatively rare case we will
				 * force a pruning or redistribution
				 * regardless of wether the current node needs
				 * it. This avoids a lot of special caseing.
				 * 
				 * We must perform the redistribution here if the
				...
				if ((redistribute) &&
					...
					right_node_ptr->total_elements)) {
					/* We will first update the search
					 * key. Since we allow the copy_cnt to
					 * be zero we need to pin this node
					 * record. */
					 ...
					 * down by the numer we will copy in.
					 * Since we are removing the first
					 * slot, We will not both moving it
					 * down. */
					...
					/* The left sibling can now be copied
					 * over. */
				...
				copy_index = 1;	/* The index to remove lies on
						 * the other node */
			}
		}
		if (redistribute) {
			/* We can only redistribute between these nodes. So
			 * set up the proper copy counts */
		...
		if (redistribute) {
			 ...
			 * amongst two nodes Now we must ripple the source
			 * nodes elments down */
		 ...
		 * done. For now it is here to avoid multiple pin records of
		 * the same field */
		...
		if (status == REMOVE) {
			...
			/* To cut down on the number of concurrent pins, we
			 * will save any info we need and unpin the page that
			...
			if (level == 0) {
				/* This is a bit confusing but, removing
				 * storage is a difficult matter, we know that
				 * if the leaf node removes storage, parent
				 * nodes may also and if we don't they wont.
				 * Anyway, Get an array of storage for placing
				 * the messages for the cleanup thread for
				 * sending later outside of a root ftx.
				 * 
				 * We will place this in the index access
				 * structure piggy backing the existing
				 * dirTruncp mechanism already in place. If a
				 * previous Trunc package is already present
				 * just reuse it. */
				if (idx_bfap->dirTruncp == NULL) {
					idx_bfap->dirTruncp =
					...
					if (idx_bfap->dirTruncp != NULL) {
						 ...
						 * to retore the page. Just
						 * put the count back, since
						 * our right sibling is
						 * already correct. */

						RBF_PIN_FIELD(dst_pgref, dst_node_ptr->total_elements);
						dst_node_ptr->total_elements -= copy_cnt;

						/* If we can't get memory skip
						 * the truncation */
						return (ENO_MORE_MEMORY);
					}
				}
			...
			sts = idx_remove_page_of_storage_int(idx_bfap,
			    ...
			    ftxH);
			if (sts != EOK) {
				/* This is royally confusing. If we fail to
				 * remove the storage we want to get out of
				 * here without panicing. In order to do this
				 * we must not fail any transactions, since we
				 * have already pinned pages. We will put back
				 * together the page we just tried to remove.
				 * Then we will return an error up. Any pages
				 * below us that may have been removed can
				 * stay that way since we haven't really
				 * changed the logic of the b-tree.
				 * 
				 * If the level below us was able to remove
				 * storage but we are not, it is ok since we
				 * have already update our node to reflect
				 * this. Our parent does not have anything to
				 * do unless we ourselves were to compress. */

				/* This is all we need to do to retore the
				 * page. Just put the count back, since our
				 * right sibling is already correct. */
			...
			if (tree == FFREE) {
				/* In this tree we maintain back pointers so
				 * we must update our right's siblings right
				 * sibling (which is now our right sibling */
		...
		*search_key =
		...
		if ((level == 0) &&
		    (dst_node_ptr->total_elements > IDX_COMPRESS_ME) &&
		    (dst_node_ptr->page_right >= 0)) {
			/* This case should only be hit when we have
			 * collisions that wrap across nodes. Since we only
			 * pass the search key we land on the leftmost node
			 * containing collisions. If the node to be prunned
			 * actually is our right sibling we must return to our
			 * parent to bring it in. */
			rbf_deref_page(dst_pgref, BS_NIL);
			return (CONTINUE);
		}
	}
	...
	if (adjust_range == REMOVE) {
		/* This node is not a candidate for compression but we do need
		 * to remove the range for our child that was removed.
		 * 
		 * If the index is at the end of the node then the child that was
		...
		/* Ripple down the slots into the removed slot and pin the
		 * changes */
	...
	if (adjust_range == MODIFY) {
		...
		if (index == dst_node_ptr->total_elements - 1) {
			/* We just updated (increased - an important
			 * distinction) the the last entry in a node. This
			 * means that our parent MUST update its entry for us
			 * since we now contain elements greater than the
			 * search key our parent has for us ! */
...
)
{
	...
	/* Don't let stg_remove_stg_start release the quota's (relquota
	 * argument is FALSE) since we need to use the directory's bfap */
	...
	if (sts != EOK) {
		/* An interesting case here. We can actually run out of space
		 * trying to give it back. Pass this up to the caller after
		 * failing this ftx. */
	 ...
	 * complete the storage deallocation in a struct hanging off the
	 * bfAccess.  The calling routine will invoke idx_prune_finish() once
	 * this entire transaction has been committed. */
...
)
{
	...
	/* We know that :
	 * 
	 * 1) If this is a result of recovery being run, the index file has not
	 * yet been opened. 2) If this is a call to ftx_fail, the index file
	 * MUST already be open.
	 * 
	 * Given the above facts, The index will only be opened and closed by
	 * this routine if this is during recovery. The directory will be
	 * opened and closed regardless. The call to idx_open_index_file will
	 * only open the index if it isn't already.
	 * 
	 * This needs to be done explicitly because the close of the directory
	 * will not trigger a close of the index file. This will only be
	 * triggered via the close of an external open of the directory. */
	...
	if (dir_bfap->idx_params == (void *) -1 && recp.action == IDX_CREATE_UNDO) {
		/* The BMT record did not get out to disk. The putting of the
		 * BMT record is purposefully outside of the index creation
	...
	case IDX_INSERT_FILENAME_UNDO:
		 ...
		 * subtransaction. We can ignore the pruning status return
		 * since the state of the node could not have changed since
	...
	case IDX_REMOVE_FILENAME_UNDO:
		 ...
		 * calling the internal routine. We can not start a
		 * subtransaction. We can ignore the split parameters since we
		 * are inserting an entry that we just removed which
		 * guarantees that space is available and no split will
		 * happen. */
	...
	case IDX_DIRECTORY_GET_SPACE_UNDO:
		...
		/* Insert the space to the b-tree by directly calling the
		 * internal routine. We can not start a subtransaction. We can
		 * ignore the pruning status return since the state of the
		 * node could not have changed since the insertion took place. */
	...
	case IDX_CREATE_UNDO:
		...
		/* To reach here we must have failed creating the index file.
		 * Probably because we are out of space. At this point the the
		 * directory's access structure has the idx_params set up to
		 * associate it with the index. We must remove this
		 * association as well as indicate on disk that there is no
		 * index file. Unfortunately there is no way to just remove a
		 * record from an MCELL without removing the entire MCELL. So,
		 * we will put out a NIL tag value and check it whenever
		 * openning the index file.
		 * 
...
)
{
	...
	/* Remove the entry from the b-tree by directly calling the internal
	 * routine. We can not start a subtransaction. We can ignore the
	 * pruning status return since the state of the node could not have
	 * changed since the insertion took place. */
	...
	/* Since performance is not an issue, we will remove the entry and if
	 * it was glommed insert any neighboring pieces back in. We should be
	 * safe here since the insertions will only be necessary if the
	 * transaction that is being undone caused the glom to happen in which
	 * case we are only undoing what we did. */

	if ((index == node_ptr->total_elements) ||
	    (node_ptr->data[index].search_key > start_addr)) {
		/* A glom occured to our left so we will need to peel our
		 * piece out */
	...
	if (entry_sa < start_addr) {
		...
		if (entry_sz > size) {
			/* We also glommed to our right so we must now do an
			 * insertion (we are safe here of not splitting a node
			 * because we know the node was condensed in this
			 * transaction.) */

			if (right_node_ptr != NULL) {
				/* The insertion actually must take place on
				 * the node to our right. So use that instead */
	 ...
	 * parent to update its ranges. This does not need to be undone. the
	 * update ranges will be correct even if we remove this insertion */
