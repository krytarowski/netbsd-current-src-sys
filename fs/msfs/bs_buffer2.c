...
bs_get_bsbuf(int rad_id, int wait)
{
	...
	/* Malloc zeroes the memory. A null bp return means no memory is
	 * available. */
	if (wait)
	...
	if (bp) {
		mutex_init(&bp->bufLock.mutex, MUTEX_DEFAULT, IPL_NONE);
...
)
{
	...
	lk_destroy(&bp->lock);
	mutex_destroy(&bp->bufLock.mutex);
    ...
    struct uucred * cred)
{
	...
	if (res != EOK) {
		...
		if (res == E_PAGE_NOT_MAPPED) {
			if (bp->lock.state & WRITING) {
				...
				/* Even though fs_write() should have arranged
				 * to add this storage before we reached this
				 * point, we could have been racing with a
				 * truncate when we dropped the file lock to
				 * obtain the active range for this write. In
				 * that case a truncate could have removed our
				 * storage, so we need to have code right here
				 * to add it back. */
				...
				res = fs_write_add_stg(bfap,
				    ...
				    NULL);
				if (res == EOK) {
					/* Set this so storage added in a hole
					 * will still trigger the stats to get
					 * updated in fs_write(). */
				...
				/* Try one more time to map the page if
				 * fs_write_add_stg() succeeded. */
	...
	/* DirectIO policy is to permit IO request sizes up to the maximum IO
	 * transfer byte size for an AdvFS volume that the underlying driver
	...
	while (bytes_left) {
		...
		if (res != EOK) {
			/* If we need to add storage and we are writing,  do
			 * it now.  */
			if (res == E_PAGE_NOT_MAPPED) {
				if (bp->lock.state & WRITING) {
					...
					res = fs_write_add_stg(bfap,
					    ...
					    NULL);
					if (res == EOK) {
						/* Set this so storage added
						 * in a hole will still
						 * trigger the stats to get
						 * updated in fs_write(). */
						contextp->dirty_alloc = TRUE;
					}
					...
					/* If E_PAGE_NOT_MAPPED and reading
					 * then this page has never been
					 * written so mark it
					 * data_never_written. */
					res = EOK;
					bp->ioDesc.data_never_written = TRUE;
		...
		/* Is this page contiguous and less than or equal to the max
		 * IO transfer size? If not, add another ioDesc to our array
		 * and fill it in.  It's contiguous if it is the same virtual
		 * drive, next_block matches the previous page and the
		 * page_is_mapped status from x_page_to_iolist() matches.  The
		 * vdIndex for an unmapped pages is 0 so comparing them checks
		 * data_never_written. */
		...
		/* Increment the current byte tranfer count. Reset if greater
		 * than maximum IO transfer size. */
...
)
{
	 ...
	 * detected by (sts == E_PAGE_NOT_MAPPED), which probably isn't a
	 * great idea, but it is what it is.
	 * 
	 * TO DO: test this code as an improvement here
	 * 
	 * if (bsPage && bsPage >= bfap->nextPage) return(E_PAGE_NOT_MAPPED); */
...
)
{
	...
	if (sts != EOK) {
		...
		if (sts == E_PAGE_NOT_MAPPED && flag.getflag) {
			/* See if we need to do frag processing (it might be a
			 * hole) */
			if (bfap->fragState == FS_FRAG_VALID &&
				...
				sts = copy_frag_into_vm_page(bfap->bfObj,
				    ...
				    pp);
				if (sts) {
					/* set error result into buffer and
					 * call pseudo I/O completion. Set the
					 * UNMAPPED buffer flag and zero the
					 * page since the the page is not
					 * mapped to storage. */
					ubc_page_zero(pp, 0, ADVFS_PGSZ);
					mutex_enter(&bp->bufLock.mutex);
					bp->lock.state |= UNMAPPED;
					bp->result = sts;
					KASSERT(bp->lsnFwd == NULL);
					KASSERT(bp->directIO == 0);
					/* have bs_io_complete() drop UBC
					 * pg_hold reference */
					bp->ubc_flags |= B_BUSY;
					bs_io_complete(bp, &s);
					*bfPageRefH = NULL;
				 ...
				 * 
				 * The getpage faulted on a hole in the file or
				 * the page is at an offset greater than the
				 * frag page offset. Getpage already checks
				 * for reads starting past the end of file.
				 * However for NFS, a prior NFS setattr
				 * request may have grown the file size but
				 * the file may not have been written up to
				 * that EOF yet. So the frag page offset may
			...
			sts = EOK;
			bs_io_complete(bp, &s);
			/* UBC page (pg_hold counter) reference remains until
			 * bs_derefpg. */
			*bfPageRefH = (bfPageRefHT) bp;
			*bfPageAddr = (unsigned char *) ubc_load(bp->vmpage, 0, 0);
			return sts;
		} else {
			/* Set the UNMAPPED buffer flag and zero the page
			 * since the page is not mapped to storage. */
			ubc_page_zero(pp, 0, ADVFS_PGSZ);
			mutex_enter(&bp->bufLock.mutex);
			bp->lock.state |= UNMAPPED;
	...
	/* Significant that we test buffer result and load final return status
	 * from it when no i/o was attempted, as it deliberately diverged from
	 * the local sts above. */
...
)
{
	...
	if ((flags & B_NOCACHE) == 0) {
		...
		/* This is an early exit on detection of I/O error on this
		 * buffer */
		...
		if (bp->lock.state & UNMAPPED) {
			 ...
			 * internal callers can not reference a page with no
			 * storage, specifically this is needed because the
			 * bmt_read paths are really ugly and scan until they
			 * hit the E_PAGE_NOT_MAPPED error. besides, this
			 * makes the result identical for the four cases of
			 * ref-new, ref-found, pin-new, pin-found. */
			if (!getflag) {
				ubc_page_release(pp, ubc_flags);
		...
		*bfPageAddr = (void *) ubc_load(bp->vmpage, 0, 0);
		...
		/* Page was not found in the UBC cache. A cache miss. Setup a
		 * new buffer page. */
		...
		if (SS_is_running &&
			...
			/* send cache miss message to smartstore if enough
			 * have occurred */
			TIME_READ(new_time);
	...
	*number_read = 0;
	...
	for (i = firstPage; i < (firstPage + npages); i++) {
		...
		if (sts == E_PAGE_NOT_MAPPED) {
			/* This is in the cache but it is not part of the
			 * file's extent map, likely because it has been ref'd
			 * but no storage has been associated with it yet.
			 * blkmap_direct() will mark the page
			 * data_never_written and because there is nothing to
			 * read, this routine will treat the page as a sparse
			 * hole and provide zeros back to the user.  */
			sts = EOK;
			clear_bufs = 1;
			continue;
		} else if (sts != EOK)
		...
		if (bfPageAddr == (char *) NULL) {
			/* The buffer was not in the UBC. Remember to flush
			 * any buffers which may be found after this one so we
			 * can do one, hopefully consolidated, I/O to retrieve
			 * the data. */
			clear_bufs = 1;
			continue;
		}
		 ...
		 * disk synchronously.  (We will later get the data in a read
		 * from the disk with the rest of the page range). If the page
		 * is clean, no need to flush, so just unpin it. Note that
		 * since we hold the range lock for this part of the file, it
		 * is perfectly safe to read the data from disk knowing that
		 * the in-memory copy will not change because a writer will
...
on_error:
	 ...
	 * is not a clone.  In the case of clones, we have already wired these
	 * pages in fs_read() to avoid a lock hierarchy violation here.  We
	 * don't expect an AIO call to come here with a seg_flag of
	 * UIO_USERSPACE, so check for this in testing, but handle it in the
	 * field. */
	...
	for (i = 0; i < bp->ioList.maxCnt; i++) {
		...
		if (bp->ioList.ioDesc[i].data_never_written) {
			...
			/* We don't pass vmpage, we pass the user's mapped
			 * address */
	...
	*number_read += nbytes;
	...
	if (list_cnt) {
		...
		/* Set a pointer to the actRange in the buffer, and increment
		 * the # of IosOutstanding for this active Range before
		 * enqueuing this ioDescriptor. Seizing of the actRangeLock is
		 * only necessary because arIosOutstanding will be decremented
		 * in bs_osf_complete(), and we never know what thread will be
		 * doing that.  There can be many buffers per active range. */
		bp->actRangep = arp;
		bp->ln = SET_LINE_AND_THREAD(__LINE__);
		mutex_enter(&bp->bfAccess->actRangeLock.mutex);
		arp->arIosOutstanding++;
		mutex_exit(&bp->bfAccess->actRangeLock.mutex);
...
retry_lookup:
	...
	/* Now that the vm_page_t is pinned, decrement the associated bsBuf's
	 * write counter since no bs_unpinpg() is called by getpage callers.
	 * Getpage callers eventually call ubc_page_release() directly and not
	 * through AdvFS. */
	...
	if (sts) {
		*pp = NULL;
		...
		/* Wait for IO_TRANS state to clear and then set it. This
		 * allows us to coordinate with set_block_map() in the migrate
		 * path.  While IO_TRANS is set here, set_block_map() is
		 * prevented from examining the DIRTY flag.  And while we wait
		 * for IO_TRANS to clear, we are prevented from examining the
		 * REMAP flag. IO_TRANS also protects us from another mmapper
		 * comming in while we are flushing below and thinking it
		 * needs to flush. */
		...
		if ((--bp->writeRef == 0) && (bp->accFwd)) {
			...
			/* must mark it now so if we fail and end up in
			 * bs_pinpg_put, bs_invalidate_pages or bs_io_complete
			 * without doing I/O we can handle it correctly since
			 * it is not on any queue. dirtyBufList removal
			 * requires WRITING be set. */
			bp->lock.state |= (GETPAGE_WRITE | DIRTY | WRITING);

			if (bp->rflList ||
			    SEQ_LTE(bp->flushSeq, bp->bfAccess->hiWaitLsn)) {

				/* Setup the UBC page for IO.  Note we keep
				 * the page on the ubc dirty list to prevent
				 * races.  We expect to always be able to make
				 * the page busy, but handle an unexpected
				 * failure by restarting a new lookup. We hope
				 * this gives ubc a chance to clean up
				 * whatever is wrong so we don't just keep
				 * looping, which is still probably better
				...
				/* Wait for the IO completion since the
				 * getpage caller will be modifying the page.
				 * bs_io_complete() removes the bsBuf from the
				 * dirtyBufList but does not clear
				 * GETPAGE_WRITE and DIRTY because we set
				 * KEEPDIRTY. */
				ubc_page_wait(bp->vmpage);
				mutex_enter(&bp->bufLock.mutex);
				bp->ln = SET_LINE_AND_THREAD(__LINE__);
			} else {
				/* we leave here with the bsBuf on the
				 * dirtyBufList */
				bp->ln = SET_LINE_AND_THREAD(__LINE__);
				clear_state(bp, IO_TRANS);
			}
		} else {
			/* Here the bsBuf may be on the dirtyBufList if
			 * writeRef > 0 or not on the dirtyBufList if it
			 * wasn't dirty before. */
...
{				/* in */
	...
	for (curpg = plp, pageCount = 0;
		...
		if (bp->accFwd) {
			...
			/* The bsBuf is on the dirtyBufList for the access
			 * structure. Pull any ioDesc's belonging to it off
			 * the lazy queues. */
			...
			if (listLen) {
				...
				/* As of bigpages, bs_pinpg_get will leave any
				 * previously-dirty bsBuf on the dirtyBufList
				 * so unless another unpinner/flusher
				 * intervenes, this is where we will
				 * eventually write the data once all mmap
				 * holds have been released and the page ages
				 * sufficiently to be sync'd or recycled.
				 * 
				...
				/* Prevent unidentifiable corruption.  The
				 * only legal way we can be in this path is
				 * because of a domain_panic that left a dirty
				 * buffer off the queues.  Force crash in
				 * development or domain_panic and I/O
				 * completion. must release lock to get live
				 * dump then retake it */
				int s;
			...
			/* This page was on the dirty list for the UBC object
			 * but not on the bfap->dirtyBufList.  That usually
			 * means it was being modified by an mmapper and we
			 * should have the bsBuf marked as such.  UBC moves
			 * the page to the clean list before the call to
			 * putpage.
			 * 
			 * Pin requests to sparse hole pages will exist on the
			 * object dirty list.  Don't flush, just call UBC to
			 * clear the pg_busy.
			 * 
			 * We can also have a page which could not be read on the
			 * object dirty list.  This is because ubc marks a
			 * page dirty on the lookup for write.  It remains
			 * dirty since we don't signal the error to ubc
			 * because we don't want to keep retrying.  In that
			 * case we have a page of trash and we don't want to
			 * write it since the write can succeed and then we
			 * have on-disk corruption (unreadable is better than
			 * unknown updates). */
			...
			if ((bp->lock.state & UNMAPPED) || bp->result) {
				...
				/* The mmap path marked the bsbuf
				 * GETPAGE_WRITE so put it on the
				 * bfap->dirtyBuflist for AdvFS I/O. */
				...
				/* Prevent unidentifiable corruption.  The
				 * only legal way we can be in this path is
				 * because of a domain_panic (probably in
				 * unpin) that left a dirty buffer off the
				 * dirtyBufList.  Force crash in development
				 * or domain_panic and page release. must
		...
		/* If we have ioDescs to add to the growing master list, add
	...
	/* Send the master list of buffers to disk using the dedicated ubcreq
	 * queue. */
	if (listLenMaster) {
...
)
{
	...
	/* If this is the first buffer on the dirty list, we can wake up any
	 * threads waiting for this LSN to be flushed. */
	if (bp == bfap->dirtyBufList.accFwd) {
		...
		while ((curFlushWaiter != (flushWaiterT *) & bfap->flushWaiterQ) &&
			...
			/* Deque the current waiter from the flushWaiter
			 * queue. */
...
)
{
	...
	if ((uflags & B_NOCACHE) == 0) {
		 ...
		 * will be removed, and we found a clean page in the cache, we
		 * want to invalidate it and return the status of 'cache miss'
		 * to the caller.  If the page is dirty (ubc_lookup set
		 * B_DIRTY in uflags if the page is dirty), we fall into
		 * bs_pinpg_found() to handle the page flushing. */
		if ((flag & PINPG_DIO_REMOVE) && !(uflags & B_DIRTY)) {
			vm_offset_t offset = pp->pg_offset;

			/* release it the same way we got it to keep counts
			 * correct */
			ubc_page_release(pp, ubc_flags);
		...
		/* Page is not in the UBC cache. A cache miss. Setup a new
		 * buffer page. */
...
)
{
	...
	/* If this is a directIO check of whether the page is in the cache,
	 * and this page is being removed from the cache by another directIO
	 * thread, then merely wait for that page to be removed by the other
	 * thread. Otherwise, mark it so that other threads know that we will
	 * remove it. */
	if (flag & PINPG_DIRECTIO) {
		/* The order of the next if..else stmt is significant! It is
		 * legitimate for several threads to call here with
		 * PINPG_DIO_REMOVE for the same page if their active ranges
		 * overlap on that page.  In this case, the first thread does
		 * the invalidation and the subsequent threads must wait and
		 * return a NULL bfPageAddr. */
		if (bp->lock.state & DIO_REMOVING) {
			/* Wait for DIO_REMOVING to be cleared. It will only
			 * be cleared after this buffer has been flushed to
			 * disk. */
			wait = 0;
			...
			mutex_exit(&bp->bufLock.mutex);
			/* Drop UBC page (pg_hold counter) reference. Note
			 * that even though this thread may have caused buffer
			 * to be placed onto the object's dirty list when
			 * ubc_lookup() was called, the thread that
			 * invalidates the page is actually responsible for
			 * moving it back to the clean list.  This happens in
			 * bs_unpinpg() when the page is staged for I/O. */
			ubc_page_release(pp, ubc_flags);
			*bfPageRefH = NULL;
			*bfPageAddr = NULL;
			return sts;
	 ...
	 * setting flag. Setting the FLUSH flag allows migrate to setup a page
	 * remap or copy while temporarily blocking other routines wanting to
	 * flush the page. Migrate will explicitly clear this state prior to
	 * unpinning page. However, this pinpg must clear the FLUSH flag state
	 * and wakeup waiters when handling an error. */
	if (flag & PINPG_SET_FLUSH) {
		set_state(bp, __LINE__, FLUSH);
	}
	/* Set the REMOVE_FROM_IOQ bit so that any IO descriptors that are on
	 * the lazy queues now, will not be moved onto the device queue. This
	 * prevents having to wait for IO to complete for this buffer below.
	 * Any ioDesc structs already on the blocking queue will get moved to
	 * the device queue and have IO done anyway. */
	bp->lock.state |= REMOVE_FROM_IOQ;
	 ...
	 * in UBC without associated storage, and then a filesystem write is
	 * done which allocates the missing storage and calls us - we still
	 * need to do the blkMap for this case */
	if (bp->lock.state & UNMAPPED) {
		mutex_exit(&bp->bufLock.mutex);
		sts = blkMap(bp, bfap);
		if (sts != EOK) {
	...
	if ((sts = bp->result) != EOK && !(sts >= MSFS_FIRST_ERR)) {
		...
		if (flag & PINPG_DIO_REMOVE) {
			/* we set this above; clear it now and wake any
			 * waiters */
	...
	if (bfap->dmnP->logAccessp == bfap) {
		mutex_enter(&bfap->bfIoLock.mutex);
...
)
{
	...
	bp->ln = SET_LINE_AND_THREAD(__LINE__);
	/* Set the FLUSH flag at migrate routine's request. Migrate will
	 * explicitly clear this flag state prior to unpinning page. If we
	 * error out of this code, we must be sure to clear the FLUSH flag if
	 * we set it. */
	...
	/* Map bf page to vd block.  We need to do this before we put any
	 * pages on the dirtybuflist since the iolist is not initialized in
	 * the bsbuf until the blkmap. */
	sts = blkMap(bp, bfap);
	...
	if (sts != EOK) {
		/* Mark buffer as UNMAPPED and zero the page since the page is
	...
	if (refHint == BS_OVERWRITE) {
		 ...
		 * This must be executed for any condition that will not
		 * attempt I/O, otherwise the BUSY condition would not be
		 * cleared and waiters awoken. */
		mutex_enter(&bp->bufLock.mutex);
		bp->result = EOK;
		bs_io_complete(bp, &s);
		 ...
		 * readers wait on new overwriting flag on buffer that is set
		 * here, and cleared in bs_unpinpg() (they would wait till
		 * after the uiomove() was done). This is problematic only for
		 * the overwriting case. */
	...
	/* Perform the I/O;  ioList is protected by IO_TRANS state. */
	{
		...
		listLen++;
		if (listLen > 1) {
			/* Add local ioDesc into the list created by
			 * seq_ahead. */
			ioListp->bwd->fwd = ioDescp;
			ioDescp->bwd = ioListp->bwd;
			ioDescp->fwd = ioListp;
	...
	/* If there is an IO error, drop the writeRef count and release the
	 * page. */
	if (bp->result != EOK) {
		mutex_enter(&bp->bufLock.mutex);
		bp->writeRef--;
...
{				/* in */
	...
	*number_written = 0;
	...
	/* Do not do any of this if we are coming from fs_zero_fill_pages. We
	 * are coming from there if arp = NULL. */
	if (arp != NULL) {

		/* Check our calculated range is within the actRanges
		 * currently held */
		KASSERT(bsBlock >= arp->arStartBlock &&
		    bsBlock <= arp->arEndBlock);
		KASSERT((bsBlock + total_blocks - 1) >= arp->arStartBlock &&
		...
		for (i = firstPage; i < (firstPage + npages); i++) {
			...
			if (sts == E_PAGE_NOT_MAPPED) {
				/* Just invalidate this page. It is in the
				 * cache but it is not mapped, likely because
			...
			/* Unpin and invalidate the page.  Any threads waiting
			 * on DIO_REMOVING in bs_pinpg_found() will be
			 * awakened: 1) in the unpin code if the page is
			 * clean, or 2) upon IO completion if the page is
			 * being flushed. In the latter case, the now-clean
			 * page will be in the ubc briefly after the IO has
			 * completed but before bs_invalidate_pages() is
			 * called.  New pinners of this page may race with
			 * this thread to invalidate the page in
			 * bs_pinpg_one_int(), but this race is atypical and
			 * benign (the first thread will invalidate the page
			 * and the others will merely find that there is no
			 * longer a page to invalidate). */
	...
	/* If the blkmap_direct() caused the frag page to be copied in for the
	 * file, then we must get rid of that page from the buffer cache since
	 * we are bypassing it on the write. */
	...
	if (list_cnt) {
		...
		if (aio_bp && aio_bp->b_driver_un_1.longvalue != 3) {
			...
			if (!resid && !aio_bp->b_driver_un_1.longvalue) {
				/* This write and any previous trailing
				 * unaligned write were both successful: no
				 * b_resid to report. */
				aio_bp->b_resid = 0;
			} else {
...
)
{
	...
	if (SS_is_running &&
		...
		/* send a page write message to smartstore if enough have
		 * occurred */
		TIME_READ(new_time);
		GETCURRWDAY(new_time.tv_sec, wday);
	...
	case BS_MOD_DIRECT:
		 ...
		 * page is wired by an application so we can not change
		 * protection to make the page clean.  In that case just write
		 * the page while keeping it dirty.  Any other failure should
		 * be impossible so just prevent further damage and crash it
		 * in development. */
		if ((sts = advfs_page_get(bp, UBC_GET_BUSY | UBC_GET_HAVEHOLD))) {
			...
			if (sts) {
				...
				/* nothing more caller can do so report
				 * success */
				return (0);
			}
		} else {
		...
		/* Since pg_busy and BUSY flag are set, other pinners will
		 * wait for these to clear, so it should be safe to release
		 * this lock before checking currentLogRec.  Must release it
		 * prior to calling call_logflush(). */
		bp->writeRef--;
		mutex_exit(&bp->bufLock.mutex);
		...
		if (!LSN_EQ_NIL(bp->currentLogRec.lsn)) {
			...
			mutex_enter(&logbfap->bfIoLock.mutex);
			...
			KASSERT(LSN_GTE(logbfap->hiFlushLsn,
				bp->currentLogRec.lsn));
			mutex_exit(&logbfap->bfIoLock.mutex);
		}
		bs_q_list(&bp->ioList.ioDesc[bp->ioList.write],
		    bp->ioList.writeCnt,
		    bs_q_blocking);
	...
	case BS_MOD_LAZY:
		...
		if (--bp->writeRef == 0) {
			...
			if (q_fn == bs_q_blocking || q_fn == bs_q_flushq) {
				...
				/* Setup UBC page for IO.  We can fail here
				 * because an mmapped page is wired by an
				 * application so we can not change protection
				 * to make the page clean.  In that case just
				 * write the page while keeping it dirty.  Any
				 * other failure should be impossible so just
				 * prevent further damage and crash it in
				 * development. */
				if ((sts = advfs_page_get(bp,
					...
					if (sts) {
						...
						mutex_exit(&bp->bufLock.mutex);
						ubc_page_release(bp->vmpage, B_WRITE);
						/* nothing caller can do,
						 * already a domain panic */
		...
		if (--bp->writeRef == 0) {
			if (bp->accFwd) {
				...
				if (q_fn == bs_q_blocking) {
					...
					/* Setup UBC page for IO.  We can fail
					 * here because an mmapped page is
					 * wired by an application so we can
					 * not change protection to make the
					 * page clean.  In that case just
					 * write the page while keeping it
					 * dirty.  Any other failure should be
					 * impossible so just prevent further
				...
				/* Change UBC page to be clean without doing
				 * IO.  We can fail here because an mmapped
				 * page is wired by an application so we can
				 * not change protection to make the page
				 * clean.  In that case ignore the failure.
				 * Any other failure should be impossible and
				 * it may be we could also ignore it here. But
				 * since we are not certain, just prevent
				 * further damage and crash it in development.
				 * 
				 * If the DIO_REMOVING flag was set in
				 * bs_pinpg_one_int(), the thread that set it
				 * may come through this path if the page was
				 * not modified.  In this case, be sure that
				 * this flag is cleared and any waiters are
				 * awakened.  The waking will occur below in
				 * clear_state() for IO_TRANS. */
	...
	case BS_LOG_PAGE:
		...
		if (cacheHint == BS_LOG_NOMOD) {
			...
			/* If the page is not previously modified, then tell
			 * UBC to move the page from the UBC object dirty to
			 * clean list. */
			if (!(bp->lock.state & DIRTY)) {
				...
				if (bp->lock.state & ACC_DIRTY) {
					...
					/* Change UBC page to be clean without
					 * doing IO. Failure should be
					 * impossible and it may be we could
					 * ignore a failure here. But since we
					 * are not certain, prevent further
					 * damage and crash it in development. */
					if ((sts = advfs_page_get(bp, UBC_GET_MAKECLEAN))) {
						/* release lock to get live
						 * dump then retake it */
		...
		if (LSN_EQ_NIL(bp->flushSeq)) {
			...
			KASSERT(LSN_GT(bp->flushSeq, wrtAhdLogAddr.lsn));

		/* Log pages are placed on the dirtybuf list at pin time in
		 * order to keep track of lsn's. So there is no need to put it
		 * on the list here. */
		KASSERT(bp->accFwd);
		...
		/* We get here with a dirty log page. Check if a flush is
		 * underway and if we are part of this range. If so then put
		 * this page on the blocking queue now. This allows log
		 * flushes to have pinned pages in the range since they will
		 * now be flushed at unpin time. */
		/* We need to be careful here that the flush lsn may live on
		 * this log page. If so we need to peak in the log page on
		 * grab the first LSN on the page. */
		...
		if ((SEQ_LTE(bp->flushSeq, bp->bfAccess->logWriteTargetLsn)) ||
			...
			/* Setup UBC page for IO. Assumes only log code can
			 * start IO since UBC cannot initiate IO on metadata.
			 * Prevent any further damage and crash it in
			 * development. */
			if ((sts = advfs_page_get(bp, UBC_GET_BUSY | UBC_GET_HAVEHOLD))) {
				/* release lock to get live dump then retake
				 * it */
				...
				mutex_exit(&bp->bufLock.mutex);
				ubc_page_release(bp->vmpage, B_WRITE);
				/* nothing caller can do, already a domain
				 * panic */
				return (0);	/* so just say it was ok */
			}
			bp->lock.state |= BUSY;
...
)
{
	...
	error = ubc_lookup(
	    ...
	    &pp,
	    &rflags,
	    0);			/* only checking existence so default
				 * vm_policy */
...
)
{
	 ...
	 * trigger page, so do as little processing as possible when we are
	 * not on a trigger page. */
	if (bsPage == 0)
		return;
	mutex_enter(&bfap->bfIoLock.mutex);
	...
	/* Seize bufLock here to prevent reading the values from the buffer's
	 * ioDesc.blkDesc while it is being modified in buf_remap(). */
	mutex_enter(&bp->bufLock.mutex);
	ioDescp = &bp->ioList.ioDesc[bp->ioList.read];
	...
	if (bsPage != 0 && bsPage == bfap->raHitPage) {
		 ...
		 * pages.  Start at page indicated in raStartPage and read the
		 * number of pages that can be read in a predetermined number
		 * of consolidated I/Os (default = 1).  Be sure to scale this
		 * number back to exceed neither the number of pages remaining
		...
		numBlocks = nPages * ADVFS_PGSZ_IN_BLKS;	/* # 512-byte blocks to
								 * read */
...
)
{
	 ...
	 * in a predefined number of consolidated I/Os for this disk.  Here we
	 * start twice the number of I/Os as will be started each time we hit
	 * a trigger page in seq_ahead_cont(). */
	...
	/* Set # pages to the lesser of the pages to the end of the file or
	 * the maximum that can be read. */
	if (nPages < maxPages) {
		/* Read-ahead limited by file size; raHitPage points to last
		 * page */
		bfap->raHitPage = bsPage + nPages;
		bfap->raStartPage = bfap->raHitPage + 1;
	} else {
		/* Normal path; raHitPage is set to the middle of the range
		 * currently being read as a trigger page to start the next
		 * read-ahead; raStartPage is set to the next page to
		 * read-ahead in seq_ahead_cont. */
		nPages = maxPages;
...
)
{
	...
	*ioListp = NULL;
	...
	while (numBlks < maxBlks && bsPage <= endPage) {
		 ...
		 * This is a racy check, since the object is locked only
		 * within ubc_incore(). UBC might expel the page from the
		 * cache just after returning from this call. For read ahead
		 * and prefetch, the worst case is that a thread may need to
		 * wait on IO, but this is expected to be rare.  At any rate,
		 * continue on to look at the rest of the pages in the
		 * requested range. */
		...
		/* Must not use callers ubc_flags here. There might be some
		 * flags that need extracted later, but for now just do
		 * B_READ.  We definitely do not want B_WRITE since that makes
		 * all the prefetched pages dirty even though the caller may
		 * never actually touch them.  If they later pin any
		 * prefetched page, then that ubc_lookup will dirty it. */
		...
		/* Fill in fields of new buffer descriptor. The bufLock is not
		 * held since other threads are blocked on the vm_page's
		 * pg_busy flag until this thread's page IO completes. */
		bp->lock.state = IO_TRANS | BUSY | READING;
		bp->ln = SET_LINE_AND_THREAD(__LINE__);
		bp->bfPgNum = bsPage;
		bp->bfAccess = bfap;
		 ...
		 * decrement the pg_hold counter. The page's pg_busy flag
		 * remains set to block other references until the IO
		 * completion. */
		ubc_page_release(pp, ubc_flags);
	}			/* end while */
...
)
{
	...
	/* Since we could spend a long time in the loop purging pages setup a
	 * max cpu hold time so we preempt and threads bound to our cpu won't
	 * time out.  The arbitrary limit is set close to 20 milliseconds
	 * while allowing a shift instead of divide. With rt_preempt_enabled,
	 * we preempt on simple_unlock if the locks held drops to 0 so no need
	 * to do it ourselves. */
	if (!rt_preempt_enabled) {
		max_cpu_ticks = 20 * hz / 1024;
		th = current_thread();
		/* give cpu to any waiting lightweight contexts before we
		 * start */
		if (lwc_pending())
			thread_preempt(th, FALSE);
		/* anytime we block in thread_preempt() we can change
		 * processors */
...
startover:
	...
	for (bp = bfap->dirtyBufList.accFwd;
		 ...
		 * state can change to true while we don't have the buffer
		 * lock, but can not change to false while we hold the chain
		 * lock. So we save time by not locking when it is already
		 * true. */
		if (bp->lock.state & BUSY) {	/* true for all non-lazy
						 * queues */
			continue;
		}
		/* Try to lock the buffer in out-of-hierarchy order since if
		 * it succeeds, this is the simplest thing to do.  If it
		 * fails, try the next buffer but remember we need to start
		 * again. */
		if (!mutex_tryenter(&bp->bufLock.mutex.mutex)) {
			skipped_buffers++;
			continue;
		}
		/* Drop chain lock now so we don't hold it too long.  There is
		 * some timeout risk if there are many buffers on the dirty
		 * list before we get here, but no easy solution to fix it. In
		 * reality, if simple_locks are enabled we are at risk even
		 * though the lock is dropped.  The problem is we may retake
		 * it after only a few instructions and another thread never
		 * gets the chance unless it is spinning on the lock since the
		 * miss code makes it less likely to get the lock. */
		mutex_exit(&bfap->bfIoLock.mutex);

		/* Lost the race - the buffer is at the device layer and we
		 * can't remove it so move on to the next buffer.  Re-get the
		 * chain lock while we still have the buffer locked so the
		 * chain remains valid since both are needed to remove this
		 * buffer.  We don't care that other threads could have
		 * removed or inserted other buffers on the list, all we need
		 * is our own buffer to stay on the list so bp->accFwd is
		 * valid once we relock the bfIoLock. */
		if (bp->lock.state & BUSY) {
			mutex_enter(&bfap->bfIoLock.mutex);
			mutex_exit(&bp->bufLock.mutex);
			continue;
		}
		/* Don't wait for IO_TRANS to clear since we don't known what
		 * the thread that has it is doing.  If the buffer is going
		 * onto one of the blocking queues, IO_TRANS remains set until
		 * I/O is complete.  On a future scan of the chain, either the
		 * buffer should transition to BUSY or be gone or have
		 * IO_TRANS clear. */
		if (bp->lock.state & IO_TRANS) {
			mutex_enter(&bfap->bfIoLock.mutex);
			mutex_exit(&bp->bufLock.mutex);
		...
		/* Transition vm_page to busy state so it will block UBC from
		 * freeing our bsBuf after we drop locks.  The busy state also
		 * keeps UBC from starting a putpage or doing a lookup.  Since
		 * we set busy we don't need to hold the vm_page so we get
		 * much better performance (see below). And we need to change
		 * the page to busy anyway before calling bs_io_complete.  We
		 * also count on the buffer state set below to keep advfs
		 * threads from stealing it or sending it to the device.  The
		 * page can only be busy here if UBC is initiating a putpage,
		 * in which case we just continue processing the chain. */
		...
		/* Set the REMOVE_FROM_IOQ bit so the IO descriptors won't be
		 * moved onto the device queue when we drop the lock to do the
		 * rm_ioq.  Since this buffer can't already be on a blocking
		 * type queue, IO_TRANS keeps other threads away. */
		 ...
		 * have to restart at the beginning and waste time when we are
		 * truncating part of a file with many dirty buffers. Note we
		 * only marked the vm_page busy, we didn't hold it. So we
		 * don't release it here, which saves time and cuts the vm
		 * object locking in half.  Since the page and bsBuf can
		 * disappear as soon as bs_io_complete calls ubc_page_release,
		 * we must not touch either again. */
		if (purge_buffer) {
			mutex_enter(&purge_buffer->bufLock.mutex);
	 ...
	 * dropping the chain lock if there were buffers we skipped due to the
	 * buffer being locked or being in transition. */
	mutex_exit(&bfap->bfIoLock.mutex);
	if (purge_buffer) {
		mutex_enter(&purge_buffer->bufLock.mutex);
		KASSERT(purge_buffer->lock.state & WRITING);
	...
	/* If we need to restart the loop, other threads are probably waiting
	 * for the bfIoLock, but we have not dropped the lock for enough time
	 * to let them get it.  Block for a tick so they can do their thing,
	 * which also releases our cpu. */
	if (skipped_buffers) {
	 ...
	 * I/O so let ubc_invalidate() wait to remove all busy pages from the
	 * dirty and clean list and wait for wired pages to be unwired. */
    ...
    int fiflags)
{
	...
	/* It has been shown that CFS can call here with a migrate running in
	 * the kernel so we need to lock them out to prevent corruption. Seize
	 * an active range for the whole file.  It is really only the
	 * invalidation that is critical to coordinate with migrate, but we
	 * might as well prevent any unneccessary flushing too.
	 * 
	 * We only want the range if CFS is the caller since advfs callers take
	 * the migTrunkLk and will deadlock due to different locking order in
	 * some paths.  Advfs callers protect without the range.
	 * 
	 * CFS invalidation is the exclusive user of INVALIDATE_UNWIRED, and
	 * always uses it or NO_INVALIDATE which is safe with no range. */
	 ...
	 * (directory and data logged files), bfflush() must be called to
	 * flush the log prior to the file's transaction controlled dirty data
	 * to maintain the write-ahead log rule.  For all other files, we must
	 * flush to make sure busy pages are pushed out to disk (in addition
	 * to dirty pages). */
	...
	if (!(bfap->bfObj->vu_flags & UBC_NOFLUSH)) {
		/* Don't call ubc_flush_dirty if file is set to noflush, advfs
		 * will take care of the flushing if this is the case. Files
		 * marked noflush are not mmapped anyway because they are
	...
	if (fiflags & (INVALIDATE_UNWIRED | INVALIDATE_ALL)) {
		...
		/* pass in B_DONE to flush any dirty pages before invalidating
		 * pages */
