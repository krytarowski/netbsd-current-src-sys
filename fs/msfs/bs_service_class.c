...
)
{
	...
	for (tblPos = 0; tblPos < scTbl->numClasses; tblPos++) {
		...
		if (SC_SUBSET(reqServices, scTbl->classes[tblPos].serviceClass)) {
			...
			/* Count number of optional services that this class
			 * provides */
			bits = 0;
			...
			if (bestEntPos < 0) {
				/* First time we find a class we need to init
				 * bestEntPos */
				bestEntPos = curPos;
			}
			if (bestEntBits < bits) {
				/* This class provides more of the optional
				 * services */
				bestEntBits = bits;
				bestEntPos = curPos;
			}
...
)
{
	...
	/* search the class' vd list for a free slot (contains a zero) and put
	 * the new vd's id into that slot */
...
)
{
	...
	for (i = 0; i < scTbl->numClasses; i++) {
		if (i == tblPos) {
			...
			/* Found a class that uses the requested class as an
			 * alt class */
	...
	while (!done) {
		...
		for (i = 0; !done && (i < BS_MAX_LIST_SEG_VDS); i++) {
			if (vdLstSeg->vdi[i] == vdIndex) {
				...
				if (scEntry->numVds == 1) {
					/* This is the last vd in the service
					 * class */
					...
					/* Move the last vd index into the
					 * slot of the deleted vd. */
		...
		if (!done) {
			vdLstSeg = vdLstSeg->nextSeg;	/* Go to the next
							 * segment. */
		}
	}
...
{
	int i;			/* general index */
	int vd;			/* used to determine if scan has gone full
				 * circle */
	int sts;
	vdLstSgmntT *vdLstSeg;
	scEntryT *scEntry;
	...
	do {
		...
		while (!done && (i < vdLstSeg->numVds)) {
			...
			/* Check to see if this is the best volume for the new
			 * Mcell.  If it is then we are done. */
			...
			if (vd == scEntry->nextVd) {
				...
				/* If we cycle through all the volumes and we
				 * have an OK volume (bestVdp is not NULL),
				 * pick this volume. */
				if (bestVdp != NULL) {
					/* already have one, not optimal, but
					 * will work */
					scEntry->nextVd = NEXT_VD(bestVd);
					/* Return found volume with ref count
					 * bumped. */
	...
	do {
		...
		while (!done && (i < vdLstSeg->numVds)) {
			if (vdLstSeg->vdi[i] != 0 && (vd_skip_list == NULL ||
				...
				/* use this disk if it has > 6.25 % free space
				 * and enough space to cover the desired
				 * number of blocks and at least 50% more
				 * space than the preferred disk (at this
				 * stage 50% means only 3% of preferred disk) */
				if (desiredBlks <= curVdFreeBlks &&
				    (curVdFreeBlks > vdp->vdSize / 16 &&
					curVdFreeBlks > preferVdBlks + preferVdBlks / 2)) {

					/* advance nextVd when we didn't have
					 * a preferred disk or preferred was
					 * already the nextVd */
					...
					*vdpa = vd_htop_already_valid(vdLstSeg->vdi[i], dmnP, TRUE);
					...
					/* save preferred info in case we
					 * don't find any disk that has > 6.25
					 * % to do second stage check, we
					 * check it first so it begins as the
					 * best too */
...
)
{
	 ...
	 * BMT so also check if we have the space to extend the BMT. If not
	 * then reject this one. */
	...
	/* At this point the volume is good but may not be the best. so set
	 * the return volume to vd.  If we can't find a best volume then this
	 * one will do. */
	*vdi = vd;

	/* Is there at least 1/8 (12.5%) of the volume free? if not then
	 * reject it. */
