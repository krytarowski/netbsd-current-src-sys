...
int AdvfsDomainPanicLevel = 1;
 /* Domain Panic Levels (what to do in domain_panic): 0: do not call live_dump
...
getosfbuf(void)
{
	...
	/* We do not lock_setup(b_lock), this is not used in our paths. A
	 * driver could check state is not valid and do the init. */
	return (bp);
}
/*
 * freeosfbuf
...
{
	/* this is paranoia just because we have seen some corruptions in our
	 * iodesc structs.  clear the important buf struct fields that we set
	 * just so if there is a mistake made by the next allocator, they
	 * don't have pointers into our space. */
	bp->b_proc = NULL;
	bp->b_dev = NULL;
	bp->b_iodone = NULL;
...
)
{
	...
	extern long lbolt;
	extern int hz;
	extern int bio_sample_shift;	/* 2**bio_sample_shift is sampling
					 * period */
	...
	if ((bp->b_flags & B_ERROR) == 0) {
		...
		switch ((bp->b_eei & 0x0000ff00)) {
			/* EEI_BBR_DONE          -- Informational message
			 * indicating that Bad Block Replacement was performed */
			/* EEI_DEVSOFT_FAILURE   -- Informational message
			 * indicating that a soft error occurred, but the
			 * device or the driver took care of it */
		case EEI_CNTBUSY_FAILURE:	/* HBA/Controller Busy
						 * (retriable) */
		case EEI_DEVBUSY_FAILURE:	/* Device is busy (retriable) */
		case EEI_DEVBIO_FAILURE:	/* Device/bus I/O error
						 * (retriable) */
		case EEI_ABORTIO_FAILURE:	/* Abort/Timed out I/O
						 * occurred */
		case EEI_CNTRLR_FAILURE:	/* HBA/Controller failure
						 * (retriable) */
		case EEI_RESERVE_PENDING:	/* Shared Device Re-reserve
						 * pending it might be a
						 * transient cluster condition */
			/* EEI_DEVPATH_FAILURE:  -- Device not there (powered
			 * off or maybe the brick was pulled from the rack and
			 * even put back in the rack?) this breaks the path in
			 * the driver to the device and until an outside force
			 * does something like an open/close on the device, it
			 * will continue to return this error.  One day, if
			 * the device drivers do not provide support for hot
			 * swapping devices, then we could consider making
			 * this retry logic more complex by doing an
			 * open/close on the device, but for now, that is
			 * beyond the scope of this project. */
		case EEI_DEVPATH_RESET:	/* Device/bus reset occurred */
		case EEI_DEVPATH_CONFLICT:	/* Device access denied (path
						 * conflict) it might be a
						 * transient cluster condition */
		case EEI_DEVHARD_FAILURE:	/* Device hardware error (hard
						 * error) Some hard failures
						 * are transient conditions */
			error_maybe_retriable = TRUE;	/* used by error
							 * messages */
	 ...
	 * which used to be done right here, but now for easier debugging it
	...
	/* If the device driver is supplying feedback on I/O statistics, use
	 * it.  Otherwise, we must do our own sampling to determine how fast
	 * to feed I/Os to the device. */
	if (active_time && Advfs_allow_driver_io_feedback) {
		if (waiting_time <
			...
			/* Our wait time is excessive; cut down the size of
			 * the device queue. */
			if (b_iolen <= vdp->devQ.lenLimit) {
	...
	/* since the vm page can change between IO operations, we want to make
	 * sure the data address isn't reused.  originally I did this clearing
	 * in call_disk, but now it is here for debugging. only the first
	 * targetAddr is set on cache IO using ubc pages. The check for vmpage
	 * (only set for ubc) excludes raw/directIO just for debug ability
	 * since they don't use the ubc cache. Up to here we could:
	 * KASSERT(taddr == iop->targetAddr); */
	if (iop->bsBuf->vmpage)
		iop->targetAddr = NULL;

	/* Remove ioDesc from devQ; if this is a consolidated I/O, all are
	 * removed here and left as a local chain no longer on the devQ for
	 * processing after the vdIoLock is dropped. */
	 ...
	 * some more going. Do this under the devQ.ioQLock. */
	...
	/* Now, pull a consolidated I/O apart and call bs_io_complete() for
	 * each buffer to wake any waiters. */
	while (len--) {
		next = iop->fwd;
		 ...
		 * are done. That routine will release the bufLock and make
		 * the buffer elible for immediate memory deallocation. So,
		 * never reference the buffer after returning from
		 * bs_io_complete. */
		if (--iop->ioCount == 0) {
			mutex_enter(&iop->bsBuf->bufLock.mutex);
			if (--iop->bsBuf->ioCount == 0) {
				/* IO done integrity checking is done here
				 * instead of bs_io_complete() since we don't
				 * want to check on aborted/fake completions.
				 * The clearing of the bsBuf->metaCheck is in
				 * bs_io_complete() for the opposite reason.
				 * Checking after a read or write is
				 * controlled by the thread staging the IO. */
				if (iop->bsBuf->metaCheck)
					bs_io_metacheck(iop->bsBuf);
				bs_io_complete(iop->bsBuf, &resetfirst);
			} else {
				mutex_exit(&iop->bsBuf->bufLock.mutex);
			}
		}
		iop = next;
	}
	 ...
	 * log entry just got written. We can calculate a new oldest log
	 * entry. Since the dirtyBufLa value is maintained on a domain-wide
	 * basis, seize the dmnP->lsnLock to be sure that bs_io_complete()
	 * running for another vd doesn't try to modify this at the same time
	 * we try to read it.  Two threads coming thru this code at the same
	 * time for different vd's in the same domain may race to reset this
    ...
    vm_offset_t starting_virtual_address)
{
	...
	while (len--) {
		...
		/* Map buffer; this code is modeled on bp_map(). By this time
		 * we must have ref'd the vm page and set the bsbuf->vmpage
		 * pointer in prep for I/O. */
...
)
{
	...
	KASSERT(SLOCK_HOLDER(&qhdr->ioQLock.mutex));
...
retry:
		...
		if (len == 0) {
			*ioList = NULL;
			...
			mutex_enter(&start->bsBuf->bufLock.mutex);
			if (start->bsBuf->lock.state & REMOVE_FROM_IOQ) {
				/* Whoops, got set while we were locking */
				mutex_exit(&start->bsBuf->bufLock.mutex);
				start = start->fwd;
				len--;
				goto retry;
			}
			/* Setup the UBC page for IO. This obtains the UBC
			 * object lock. Skip this buffer IO if the UBC page's
			 * pg_busy flag is already set. Most likely, we lost a
			 * race with UBC staging the page for IO prior to
			 * calling putpage. We also skip the bsbuf if the
			 * vm_page is in an exchange transition, but that is
			 * ok since eventually we will see it again on a
			 * future pass. RAWRW requests never go on the
			 * consolQ. */
	...
	/* walk thru remaining ioDesc on this queue checking for
	 * appropriateness of consolidating their IOs.  Compare the values of
	 * each ioDesc with that of the previous buffer. */
	while (len-- &&		/* more ioDesc's on incoming queue */
		...
		mutex_enter(&this->bsBuf->bufLock.mutex);
		...
		if ((((this->bsBuf->lock.state & REMOVE_FROM_IOQ) == 0) ||
			...
			if (qhdr == &vdp->consolQ) {
				/* For consolq queue buffers, setup the UBC
				 * page for IO. But skip the buffer IO if
				 * these flags are already set. That means
				 * either a racing UBC routine setup the page
				 * for IO but has yet to call the filesystem
				 * putpage or some thread (flush, pin) already
				 * set the IO_TRANS flag. We also skip this
				 * buffer if the page is in an exchange
				 * transition, but that is ok since eventually
				 * we will see it again on a future pass. */
				if (advfs_page_get(this->bsBuf, UBC_GET_BUSY)) {
					/* Next buffer will not be block
					 * contiguous so break. */
					mutex_exit(&this->bsBuf->bufLock.mutex);
					break;
				}
			}
	...
	if (count > 1) {
		...
		/* Call bp_map_alloc() to get n pages of contiguous virtual
		 * memory from the vm subsystem. If either the allocation or
		 * mapping fail, then we have to return one unconsolidated I/O
		 * and put the rest of the ioDesc struct onto the blockingQ. */
		...
		if (virtual_mem_start) {
			...
			/* Map the buffers contiguously starting at
			 * virtual_mem_start */
			map_status = list_map(start, virtual_mem_start);
		}
		if (map_status < 0) {

			/* Record the consolidation failure in domain
			 * statistics */
			start->bsBuf->bfAccess->dmnP->bcStat.consolAbort++;

			/* If the incoming queue is not the blocking queue
			 * then we need to drop the incoming locks so as to
			 * get the blocking queue lock in order. Note: This is
			 * safe because the ioDescTs in the chain at start are
			 * protected from the removal routines because they
			 * are marked as being on the devQ and the removal
			 * routines do not operate on the device queue. The
			 * chain at start has already been removed from the
			 * incoming queue so it cannot be moved to another
			 * queue while the qhdr->ioQLock is dropped. Racing
			 * pins and refs will pend on the BUSY buffer
			 * associated with each ioDescT. */
			if (qhdr != &vdp->blockingQ) {
				mutex_exit(&qhdr->ioQLock.mutex);
				locks_held[qindex] = 0;
				mutex_exit(&vdp->devQ.ioQLock.mutex);
			...
			/* Pass virtual_mem_start back for call_disk only when
			 * we actually combined multiple buffers for one IO. */
...
)
{
	...
	if (ioList->bsBuf->directIO) {
		 ...
		 * requests passed down to a Prestoserve enabled PCI NVRAM
		 * board.  Prestoserve will reject DirectIO requests and send
		 * them directly to the underlying disk driver. A preferred
		 * solution is for Prestoserve to convert the user address
		 * into a physical KSEG address before sending the request to
		 * the PCI NVRAM board. The hardware only deals with physical
		 * addresses. This B_RAW flag can be removed when Prestoserve
		 * address conversion is supported.
		 * 
		 * We set B_PHYS for drivers that use the b_proc field.
		 * 
		 * Check to be sure that we do full-sector transfers on directIO
		 * transfers. */
	...
	bp->b_iostats.devqlen = vdp->devQ.ioQLen;	/* # outstanding
							 * bufferss */
...
get_locks(ioDescHdrT * qhdr, vdT * vdp, qtypeT qindex, int *locks_held)
{
	...
	KASSERT(SLOCK_HOLDER(&vdp->devQ.ioQLock.mutex));
	if (!mutex_tryenter(&qhdr->ioQLock.mutex.mutex)) {

		mutex_exit(&vdp->devQ.ioQLock.mutex);
		mutex_enter(&qhdr->ioQLock.mutex);
		mutex_enter(&vdp->devQ.ioQLock.mutex);

		KASSERT(vdp->devQ.lenLimit > 0);
		if (vdp->devQ.ioQLen >= vdp->devQ.lenLimit) {	/* no room */
			/* Note: There is no danger that our I/O will be
			 * stranded if we bail out here, because the
			 * background I/O thread will be sent a message by I/O
...
)
{
	 ...
	 * are slightly different. IO_FLUSH:     Ideally FILL the devQ by
	 * taking one-half of the number of entries available from the
	 * blockingQ, one-half of the now remaining entries from the ubcReqQ,
	 * half of the remaining entries from the flushq, and the rest from
	 * the lazy queues.  If this calculation would still not fill the
	 * device queue,  fill in the remainder by taking as many as possible
	 * from the blocking, ubcReq, and flush queues, in that order.
	 * IO_SOMEFLUSH: Same as IO_FLUSH, except that we limit the number
	 * taken from the lazy queues to a percentage of those on the consolQ.
	 * This is done in an attempt to keep the lazy I/Os draining, but not
	 * do it so fast that these buffers cannot be repinned or that we
	 * actually take up room on the devQ with buffers that do not actually
	 * need to get flushed now. IO_NOFLUSH:  1. Flush as many as possible
	 * from the blockingQ. 2. If there is room left, flush as many as
	 * possible from the ubcReqQ. 3. If there is still room left, flush as
	...
	if (flushFlag == IO_FLUSH || flushFlag == IO_SOMEFLUSH) {
		...
		if (devQ_room) {
			if (flushFlag == IO_SOMEFLUSH) {
				/* Pick up items from consolQ even if ioQLen
				 * is small */
	...
	/* Start more I/Os so long as: 1.  The # of buffers on the deviceQ is
	 * less than its limit.  AND 2.  There are still more buffers to be
	 * moved according to our calculation above. */
	while (vdp->devQ.ioQLen < vdp->devQ.lenLimit) {
		...
		if (vdp->blockingQ.ioQLen && blocking_cnt > 0) {
			...
			/* get out if we moved the # of buffers calculated
			 * above, even though there is still room on the
			 * device queue. */
			break;
		}
		...
		if (vdp->consolidate &&
			...
			/* Call consolidation routine to group I/Os where
			 * possible. Avoid calling this routine if there is
			 * only one buffer to be added. */
			...
			/*
	                 ...
	                 */
			ioList = qhdr->fwd;
			mutex_enter(&ioList->bsBuf->bufLock.mutex);

			if (qhdr == &vdp->consolQ) {
				/* Skip the buffer if it is coming off the
				 * consolQ and is marked REMOVE_FROM_IOQ
				 * because it is being removed from the lazy
				 * queues for reuse. Allow ioDesc with this
				 * bit set on blockingQ to go thru. */
		loop:
				while (ioList->bsBuf->lock.state & REMOVE_FROM_IOQ) {
					ioDescT *next = ioList->fwd;
					mutex_exit(&ioList->bsBuf->bufLock.mutex);
					ioList = next;
				...
				/* Setup the UBC page for IO. Raw and Direct
				 * I/O do not put buffers on the consolq. */
				KASSERT(!(ioList->bsBuf->lock.state & RAWRW));
				if (advfs_page_get(ioList->bsBuf, UBC_GET_BUSY)) {
					/* If the UBC page is already setup
					 * for IO (the pg_busy flag is on),
					 * then skip this buffer and try the
					 * next one. UBC might be initiating
					 * the IO and has not yet called
					 * AdvFS's putpage. We also skip this
					 * buffer if the page is in an
		...
		if (ioList->desCnt > 0) {
			...
			iop = ioList->bwd;	/* iop is the last ioDesc on
						 * list */
		...
		/* Zero the retry counter.  This will be used to limit the
...
read_raw_bmt_page(
    ...
    struct bsMPg * dpg)
{				/* in - bitfile metadata table page ptr */
...
write_raw_bmt_page(
    ...
    lbnT vdblk,			/* in - starting disk block number */
    struct bsMPg * dpg)
{				/* in - bitfile metadata table pg ptr */
