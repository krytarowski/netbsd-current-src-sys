...
enum mlDeFlags {
	ML_DEF_NONE = 0, ML_DEF_META = 1, ML_DEF_DATA = 2,
...
typedef enum {
	...
	XMT_MIRROR_UNSUPPORTED,	/* mirrored/shadowed file; currently
				 * unsupported */
...
typedef struct _opEvent {
	int action;		/* in - flag: 0 == clear event, 1 == post
				 * event */
}        opEvent;
...
typedef struct {
	...
	u32T bmtPreallocPgs;	/* number of pages to be preallocated for the
				 * BMT */
...
typedef struct {
	...
	u32T bmtPreallocPgs;	/* in - number of pages to be preallocated for
				 * the BMT */
...
typedef enum {
	...
	ML_BSR_VD_ZOMBIE	/* vd is being removed but is only
				 * mostly-dead. */
}    mlBsVdStatesT;
/*
