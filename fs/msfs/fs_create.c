...
{				/* in - nameidata pointer */
	...
	if ((mode & S_IFMT) == S_IFDIR) {
		...
		/* Set the value of effective_LINK_MAX: DVN3 uses signed short
		 * st_nlink DVN4 (or later) uses unsigned short */
	 ...
	 * (which is mapped in the quota file already), except in the case of
	 * svid III compatibility, with the set-gid bit on the parent not set. */
	...
	for (qtype = 0; qtype < MAXQUOTAS; qtype++) {
		...
		/* if the type is user and the page is not mapped, take the
		 * lock. if the type is group and svid3 compatibility mode is
		 * on and the setgid bit on the parent is not set AND the page
		 * is not mapped, take the lock. see svid III compatibility
		 * below. */
		if (((qtype == USRQUOTA) ||
	...
	if (create_directory) {
		...
		/* Directories start off with no index regardless of domain
		 * versioning. Set a -1 in idx_params to indicate that it is
		 * unnecessary to attempt to open the index */
	...
	if (create_symlink) {
		...
		if (len <= bmtr_max_rec_size()) {
			 ...
			 * could not take it in this case. Instead, migrate
			 * will wait on the bitfile state */
	...
	mutex_enter(&dir_cp->fsContext_mutex.mutex);
	dir_cp->fs_flag |= (MOD_MTIME | MOD_CTIME);
	dir_cp->dirty_stats = TRUE;
	dir_cp->dirstamp++;
	mutex_exit(&dir_cp->fsContext_mutex.mutex);
	 ...
	 * insert_seq for the just inserted-into  directory. */
...
)
{
	 ...
	 * fscontex structure. Any other routine that calls this would have to
	 * coordinate in the same way if it uses the same mutex in the same
	 * way. */
...
)
{
	 ...
	 * bmtr_update_rec() returns a 'cant start a transaction' error. In
	 * this case, the whole operation will be retried again at a later
	 * time. */
	return (ret);
}
