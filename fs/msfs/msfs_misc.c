...
{
	mutex_init(&fscp->fsContext_mutex.mutex, MUTEX_DEFAULT, IPL_NONE);
...
bf_get(
    ...
    int flag,			/* in - flag - META_OPEN, metadata bf being
				 * opened */
...
bf_get_l(
 ...
 /* - DONT_UNLOCK, don't unlock dir */
    struct mount * mp,		/* in - the mount structure pointer */
    struct vnode ** vp,		/* out - vnode corresponding to the accessed
				 * bf */
...
)
{
	...
	if (((GETBFSETP(mp))->cloneId != BS_BFSET_ORIG) &&
		...
		if (sts != EOK) {
			/* Be sure this lock gets released unless requested
			 * not to! */
			if (!(flag & DONT_UNLOCK)) {
	...
	*vp = nvp;
	...
	if (file_context->initialized) {
		...
		if ((file_context->dir_stats.st_mode & S_IFMT) == S_IFDIR) {
			/* don't get the lock if we already obtained it in
			 * lookup */
	...
	/* If there is a bfap->saved_stats structure, then the latest file
	 * stats were never flushed to disk before the vnode was reclaimed, so
	...
	if (bfap->saved_stats) {
		...
		if (!flush_in_progress)
			...
			/* mark so that cleanup thread knows that this has
			 * been copied back to a context structure.  It uses
			 * this info during error recovery when attempting to
			 * flush stats to disk. */
...
)
{
	...
	if (tempbfap) {
		 ...
		 * closed and if so, then also close any associated index
		 * file. */
		if (IDX_INDEXING_ENABLED(tempbfap) &&
		    IDX_FILE_IS_DIRECTORY(tempbfap)) {
			/* Close the associated index file and deal with any
			 * clone processing. */
			IDX_CLOSE_INDEX(tempbfap);
		}
		/* take a ref on the bfap to prevent it from moving to the
		 * freelist in bs_close(). This is needed until bfNode.accessp
		 * is cleared below. */
		mutex_enter(&tempbfap->bfaLock.mutex);
		tempbfap->refCnt++;
		mutex_exit(&tempbfap->bfaLock.mutex);
 ...
 */
u_long msfs_reclaim_cleanup_failures[2] = {
	0L, 0L
};
...
)
{
	 ...
	 * reached this point because it called vgone and we do not need to do
	 * anymore work. Grab_bsacc has taken care of disassociating the vnode
	 * and access structure already. We mainly are avoiding the call to
	 * bs_bfs_lookup_desc which can be costly if there are a lot of
	 * mounted filesets.  */
	...
	if (BFSET_VALID(bfSetp)) {
		...
		/*
	         * RACE CONDITION
	         *
	         * Between the call to bs_bfs_lookup_desc() above and the mutex_enter(.mutex)
		...
		if (bfap && (vp == bfap->bfVp)) {
			...
			if ((bfap->bfState == BSRA_INVALID) && vop && vop->vu_dirtypl) {
				/* invalid access struct can't have any dirty
				 * ubc pages */
			...
			/* Remove this access struct from the free list (if on
			 * it). Up the refCnt on the access struct to prevent
			 * it from being recycled if we sleep on getting a
			 * buffer in bs_pinpg. */
			...
			if (vop) {
				 ...
				 * read-only. The only way there could be
				 * dirty pages is on a COW, but that will
				 * flush them out. Leaving pages on the clean
				 * list will get picked up in grab_bsacc at
				 * recycle time.
				 * 
				 * We don't get vnodes for internal opens of frag
				 * and index files. If we are in this path, it
				 * must have been an open via .tags. Don't
				 * invalidate and flush pages here - internal
				 * close will take care of that. */
			...
			if (VTOC(vp) && VTOC(vp)->dirty_stats) {
				/* We are going to lose the context structure
				 * in a moment, but there are file statistics
				 * that need to be flushed to disk.  The
				 * problem is that we may have been called
				 * indirectly from an advfs routine where a
				 * transaction has already been started, but
				 * we do not have the transaction handle to
				 * start a subtransaction.  And incorrectly
				 * starting a root transaction is death.  So,
				 * we will save the file statistics from the
				 * fsContext into a structure that will hang
				 * off the access structure until: a) the file
				 * is accessed again when we will copy the
				 * stats back into the new fsContext struct,
				 * or b) the cleanup thread processes the
				 * acccess structure which will be on the
				 * closed list.  It can easily start a root
				 * transaction, flush the stats to disk, and
				 * then move the access struct to the free
				 * list.
				 * 
				 * Taking a clue from fs_update_stats(), we only
				 * save the stats if this is NOT a readonly
				 * file and the file was not opened via the
				 * .tags directory. */
				...
				if (!(mp->m_flag & M_RDONLY) &&
					...
					/* Because we are lazy about flushing
					 * these stats out to disk, it is
					 * possible we are reclaiming a vnode
					 * that still has saved_stats from the
					 * previous reclaim.  Not to worry; we
					 * will just update the saved stats
					 * that have already been allocated. */
					if (!bfap->saved_stats) {
						/* This should be the typical
						 * path */
						mutex_exit(&bfap->bfaLock.mutex);
				...
				if (bfap->saved_stats) {
					/* Not sure this ever happens, but
					 * this is the correct thing to do:
					 * We are reclaiming a vnode that has
					 * no dirty_stats but there is an old
					 * saved_stats structure still hanging
					 * around.  This is obviously
					 * out-of-date, so just deallocate it.
					 * It's presence makes the access
					 * structure go onto the closed list
					 * rather than the free list, and we
...
msfs_refer(vm_ubc_object_t vop)
{
	...
	mutex_enter(&vop->vu_ap->bfaLock.mutex);
	vop->vu_ap->mmapCnt++;
	mutex_exit(&vop->vu_ap->bfaLock.mutex);
    ...
    struct uucred * cred)
{
	...
	for (; ;) {
		...
		/* Temporary race condition check, previously we purged the
		 * page if ((pp->pg_reserved & VPP_INVALID) && !(flags &
		 * B_INVAL)) // We want to skip the write if we are called
		 * from / ubc and the page is being invalidated. Contrarily,
		 * we / also don't want to skip the write if ubc_invalidate()
		 * / is calling us to write the page (B_INVAL flag is set). /
		 * The latter condition is for vclean path. but now we should
		 * never be here with VPP_INVALID from other paths.  It should
		 * be "no harm, no foul" to either throw the page away or
		 * write it, but maybe there is a path I missed... so */
    ...
    struct uucred * cred)
{
	...
	*pl = VM_PAGE_NULL;
	...
	/* It is an error to read/write beyond the file's last page, we need
	 * to check because some callers don't have the lock. Because of the
	 * way nfs works and the problems of racing truncation, we only report
	 * an error for vm, other callers get success and fewer (maybe 0)
	 * pages. */
	fsize = bfap->file_size;

	if (offset >= fsize) {
		/* for mmappers this is an error that becomes sigbus. for nfs
		 * this is success and 0 pages returned. */
		if (protp)
	...
	for (; pageCnt; pageCnt--, page++) {
		...
		if (!writing) {
			...
			error = bs_pinpg_get(bfap, page, refHint,
			    pl, vmp, offset, len, ubc_flags);
			if (error) {
				/* we could be writing a hole or a frag.  in
				 * most cases on the first unmapped page the
				 * call we make below to fs_write_add_stg()
				 * will allocate multiple (we hope all) pages.
				 * if fs_write_add_stg() didn't allocate all
				 * the pages for any reason, such as a bigpage
				 * request larger than MAX_ALLOC_PAGE_CNT, we
				 * will try again when we hit the next
				 * unmapped page. */
				if (error == E_PAGE_NOT_MAPPED) {
					if (!have_write_lock) {
						...
						/* someone could have
						 * truncated the file or added
						 * the storage if we slept
						 * getting the write lock. no
						 * special code here for nfs,
						 * they do read only. */
						fsize = bfap->file_size;
						if (offset >= fsize ||
					...
					/* add storage for a sparse write or
					 * the frag page */
					...
					/* tell sync writers to flush the
					 * extent map.  fsync will also flush
					 * if we set the dirty_stats below.
					 * having the write lock is the only
					 * protection needed. */
					contextp->dirty_alloc = TRUE;

			have_storage:
					/* we only retry 1 time and bail out
					 * if it fails */
					error = bs_pinpg_get(bfap, page, refHint,
					    pl, vmp, offset, len, ubc_flags);
					if (error)
...
int
msfs_fs_cleanup(vm_offset_t opfs)
{				/* In:Copy of vm_page's fs private field. */
	struct bsBuf *bp;
...
int
msfs_fs_replicate(vm_page_t pp)
{				/* In:Copy of vm_page's fs private field. */
	...
	/* TO DO:  decide if we really need this, or should we always just set
	 * up for the 1 ioDesc that is inside the bsBuf.  we really should not
	 * need to malloc for the multiple destination case since that only
	 * applies to migrate and that should not be here. */
	if (bp->ioList.maxCnt > 1) {
		if ((bp->ioList.ioDesc = (ioDescT *) ms_rad_malloc_no_wait(
			...
			bp->ioList.maxCnt = 0;	/* can't free what we don't
						 * have */
			bs_free_bsbuf(bp);
			return (-1);
		}
	 ...
	 * word-tear since there is nobody stopping other threads, but the
	 * original should be read-only and unlikely to be undergoing a
	 * transition that would affect this field */
...
restart:
	...
	if (bfap->dataSafety == BFD_FTX_AGENT) {
		...
		/* Increment the mmapCnt under the bfaLock. u_vp_create()
		 * below will increment the mmapCnt also, but no matter. We'll
		 * decrement our increment on mmapCnt after the call to
		 * u_vp_create(). */
		mutex_enter(&bfap->bfaLock.mutex);
		bfap->mmapCnt++;
		mutex_exit(&bfap->bfaLock.mutex);
		...
		if (!(vp->v_flag & VMMAPPED)) {
			/*
	                 ...
	                 */
			if (!(vp->v_flag & VDIRECTIO)) {
				/* DirectIO is not already in effect, just
				 * throw the bit. It won't be modified with
				 * the file_lock held by us. */
				VN_LOCK(vp);
				vp->v_flag |= VMMAPPED;
				VN_UNLOCK(vp);
			} else {
		 ...
		 * field indicating that msfs_sync_mmap() should flush dirty
		 * pages for this file every smsync_age seconds. */
		if (bfap->mmapCnt) {
			bfap->mmapFlush = 1;
...
/* Revision 1*/
	/* Flush the domain's log to disk before flushing metadata. Flush
	 * modified metadata pages up to the domain log's highest LSN flushed.
	 * This skips flushing pinned metadata pages. */
/*
...
/* Revision 2 */
	...
	/* Flush the passed in object. This will catch case were the metadata
	 * file was migrated and the smoothsync queues are full of noflush
	 * buffers. These will not be on the domains LSN list so the above
	 * flush wouldn't find them.  */
	...
	if (bfap->dataSafety == BFD_NO_NWR) {
		/* A new log may be in the process of being setup (switchlog
		 * or mkfdmn) lets just be paranoid and not deref NULL. */
