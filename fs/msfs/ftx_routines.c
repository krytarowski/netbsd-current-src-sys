...
)
{
	...
	if (parentFtxH.hndl == 0) {
		...
		if (FtxStats) {
			...
			/* These slots can't be used even if they are
			 * FTX_SLOT_AVAIL */
		...
		if ((ftxTDp->ftxWaiters) ||
			...
			/* Don't allow someone to sneak in if there are
			 * already waiters; make them get in line like
			 * everyone else.  The conditional wait is fifo.  This
			 * is the ftx bouncer algorithm. */
			if (ftxTDp->ftxWaiters) {
				if (AdvfsLockStats) {
			...
			/* These waiters will be awakened when the next slot
			 * becomes available.  The wakeup will occur when
			 * either a) the next slot rrNextSlot is incremented,
			 * or b) an ftx_free changes a slot to AVAIL status. */
		...
		if (xid == 0) {
			ftxTDp->lastFtxId += 1;
			/* if high-order bit set after incr, roll over to
			 * FtxId == 1 */
		...
		/* Now wait on the trimCv.  We will be awakened whenever the
		 * oldest lsn is reset. */
		wait = 0;
		...
		if (!(flag & FTX_EXC)) {
			/* Advance round robin pointer to the next slot for
			 * the next ftx */
		...
		if (!(flag & FTX_EXC)) {
			/* Since we've bumped the next slot, wake a waiter
			...
			if ((ftxTDp->ftxWaiters > 0) &&
				...
				/* If we just trimmed the log making lots of
				 * slots available, then broadcast to the
				 * waiters in hopes of many getting to run. */
				if (trimwait) {
...
)
{
	...
	if (lvl != 0) {
		 ...
		 * This if statement is a place holder for the handling of
		 * that return value. */
	...
	/* log_donerec_nunpin() now returns int instead of void. This if
	 * statement is a place holder for the handling of that return value. */
	if ((sts = log_donerec_nunpin(ftxp, dmnP, lrvecp)) != EOK);
	...
	if (ftxp->nextRDoff) {
		...
		do {
			...
			/* log_donerec_nunpin() now returns int instead of
			 * void. This if statement is a place holder for the
			 * handling of that return value. */
			if ((sts = log_donerec_nunpin(ftxp, dmnP, lrvecp)) != EOK);
	...
	for (pli = clvlp->lastPinS; pli >= 0; pli -= 1) {
		...
		if (plpp->numXtnts == 0) {
			...
			if (bs_unpinpg(fpp->pgH, logNilRecord,
				...
				release_ftx_locks(clvlp);	/* release any locks
								 * held by this ftx */
				sts = E_DOMAIN_PANIC;
			}
	...
	while (!LSN_EQ_NIL(ftxp->undoBackLink.lsn)) {
		...
		if ((lrsts != EOK) && (lrsts < MSFS_FIRST_ERR)) {
			...
			do {
				...
				if ((lrsts != EOK) && (lrsts < MSFS_FIRST_ERR)) {
					...
					/* Give back the buffer handed to us
					 * by lgr_read */
					ms_free(segptr);
					release_ftx_locks(clvlp);	/* release any locks
									 * held by this ftx */
					lgr_read_close(logrh);	/* close the log read
								 * stream */
					ms_free(ftxDLRp);
					goto free_ftx;
				}
				...
				/* Give back the buffer handed to us by a
				 * successful lgr_read */
		...
		if (cntRead < recwordcnt) {
			...
			if (lrsts != W_LAST_LOG_REC) {
				...
				/* Give back the buffer handed to us by
				 * lgr_read */
				ms_free(ftxDLRp);
		...
		if (ftxDLRp->contOrUndoRBcnt) {
			...
			/* ftxId and bfDmnId will already be correct in the
			 * ftx */
...
free_ftx:
	if (atomicRPass || !lvl) {
		...
		/* Now free the ftx struct allocated to the newly-available
		 * slot */
		ftx_free_2(ftxp);
...
)
{
	...
	for (recxi = 0; recxi < plpp->numXtnts; recxi += 1) {
		...
		if ((recendoff <= endoff) && (recendoff >= boff)) {
			/* end within existing, already know it's not
			 * contained */
			recendoff = endoff;
...
)
{
	 ...
	 * 
	 * If ASYNC logging was used we can't be sure that the transaction is
	 * persistent at this point.  The only thing we can guarantee is that
	 * the log will be written before the real bitfile pages are written,
	 * that is, the write ahead log protocol is always enforced. */
...
ftx_init(void)
{
	...
	mutex_init(&FtxMutex.mutex, MUTEX_DEFAULT, IPL_NONE);
	 ...
	 * UBC's maximum cache pages. The intent appears to be avoiding
	 * waiting for buffers after starting a transacton. Since the number
...
)
{
	 ...
	 * actually be freed later in ftx_free_2().  The fields in that struct
	 * may be used in reset_oldest_lsn() after this routine returns. */
	...
	if (FtxStats) {
		if (ftxTDp->oldestSlot == ftxSlot) {
			...
			/* The oldest slot is being freed. Find the new oldest
			 * slot. */
...
)
{
	...
	/* Decrement the # ftx structs in use; allows us to honor an upper
	 * limit so that we do not start more transactions than the buffer
	 * cache can support.  This is protected by the FtxMutex under
	 * non-recovery conditions. */
	FtxDynAlloc.currAllocated--;

	/* If threads are waiting for the number of allocated structs to fall
	 * below the threshold, and we are below the threshold, then wake up a
	 * single thread. */
...
)
{
	 ...
	 * with ftx_set_recovery_logaddr to explain the following. */
...
{
	/* The dmnP->lsnLock must be held by the caller. No ASSERT since this
	 * can also be called during recovery when this will not be necessary. */
...
)
{
	...
	KASSERT(mutex_owned(&FtxMutex.mutex));
	if ((dmnP->state < BFD_RECOVER_CONTINUATIONS) &&
	    (!(dmnP->dmnFlag & BFD_DEACTIVATE_IN_PROGRESS))) {
		return;
	}
