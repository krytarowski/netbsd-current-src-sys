...
#define DIO_REMOVING    0x20000	/* Directio is in process of removing this
				 * buffer from the AdvFS cache. */
...
#define ACC_DIRTY       0x1000000	/* this buffer is on the access dirty
					 * list */
...
#define REMAP           0x80000000	/* The REMAP flag indicates that this
					 * file is being migrated.
					 * Furthermore, the bsBuf with this
					 * flag set has had its ioDesc
					 * remapped so that it points to only
					 * the destination extent map of the
					 * migration.  The migrate code sets
					 * this when copying the file's
					 * contents to the destination extent
					 * map.  If the REMAP flag is set and
					 * another (non-migrate) thread pins
					 * the page, modifies it, and unpins
					 * it, the unpin code (bs_unpinpg)
					 * will realize that the bsBuf needs
					 * to be remapped once again so that
					 * its ioDesc(s) will point to both
					 * the original extent map for the
					 * file and also the migration
					 * destination extent map.  This is
					 * necessary in case the migration
					 * fails. If the migration fails, the
					 * destination storage will be
					 * released and the file will revert
					 * to using only the original extent
					 * map.  If the unpin code did not
					 * remap the bsBuf and later the
...
struct bsBuf *
bs_get_bsbuf(int rad_id,	/* in - preferred rad for struct */
 ...
 */
int
advfs_page_get(struct bsBuf * bp,	/* in - buffer to get current vm_page
					 * struct */
