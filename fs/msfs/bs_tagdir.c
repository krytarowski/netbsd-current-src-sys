...
)
{
	...
	/* Initialize the page header and the subordinate free list. Note that
	 * some the tag page header is initialized via bzero. */
	...
	/* Page 0 is special in that the first tagmap struct is used to store
...
getfreeslot:
	...
	--slot;			/* this cleverly makes unsigned slot >
				 * BS_TD_TAGS_PG if 0 */
...
badfreelist:
	...
	/* free list is corrupt on this page.  kill the free list. goto to the
	 * next tag dir page in the "page free list". */
...
)
{
	...
	if (sts != EOK) {
		if (sts == E_NO_SUCH_BF_SET) {
			/* The bitfile set may have been created and removed
			 * before the log was rolled forward. Just ignore the
			 * error and let recovery continue */
...
)
{
	...
	if (bfAccess != NULL) {
		...
		mutex_exit(&bfAccess->bfaLock.mutex);
		bfAccess = NULL;
	}
	/*
	...
	if (tagPg != bfSetp->tagFrLst - 1) {
		...
		while (!prevFound) {
			...
			if (prevTdpgp->tpNextFreePage == 0) {
				...
				if (oldPrevPg != 0) {	/* don't reref page 0,
							 * needed below */
...
)
{
	...
	if (tp->seq != 0) {
		/* A non-zero sequence number is used when a match on sequence
		...
		/* A zero sequence number is used when a wild card lookup is
		 * requested.  The current sequence number is returned in the
		 * tag. */
