...
)
{
	...
	if (bfap->cloneId == 0) {
		...
		if (bfap->bfSetp->cloneId != BS_BFSET_ORIG) {
			...
			/* This is a clone that has not had any metadata (or
			 * file pages) COWed. At open time the original's
			 * extents were loaded into it's (the clone's) in
			 * memory extent map.  This is basically a bug (to be
			 * addressed at some future time). We can not trust
			 * these extents since the original could have been
			 * migrated (which does not cause a COW).  So we need
			...
			if (clu_is_ready()) {
				 ...
				 * Migrate will get the clu_clonextnt_lk on
				 * the clone when migrating the original. So
				 * this will protect us here. the extent maps
...
)
{
	...
	while (!RECCUR_ISNIL(*cur)) {
		...
		/* Check for the status of the function call.  If there was
		 * some error, then break out of the loop and return the
		 * error. */
		if (sts != EOK)
			break;
...
)
{
	...
	while (uiop->uio_resid >= SIZEOF_PROPLIST_HEAD) {
		...
		if (!strcmp(proplist_headp->pl_name, "DEC_ADVFS_OPERATION")) {
			/* ensure that the op was correctly given; if no
			 * pl_value was supplied, then this pointer remains
			 * null */
			if (!prop_data) {
				error = EINVAL;
				goto out;
			...
			case ADVFS_SYNC_WRITE:
				...
				/* Make sure the caller is authorized to do
				 * this. */
...
)
{
	...
	char *name_buf = NULL;
	int valuelen, entry_size, large, cell_size, page, name_resid, data_resid;
	...
	if (got_name) {
		...
		if (RECCUR_ISNIL(hdr)) {
			tmp_hdr.bmtp = NULL;	/* page is no longer
						 * referenced */
		}
		hdr = tmp_hdr;
...
out:
	...
	if (ftx_started) {
		...
		if ((error != EOK) && (mcells_alloced)) {
			/* failure - mcells_alloced, which comes only from
			 * findhead_setdata, indicates that some mcells were
			 * allocated, and must now be deallocated.
			 * 
			 * Don't execute the root done tx - don't want to delete
			 * the existing duplicate mcell set since we failed
			 * this transaction.
			 * 
			 * Clean up newly allocated mcells explicitly with the
			 * subtx's rootdone, which has already been logged by
			 * FTA_MSFS_ALLOC_MCELL's successful completion. */
	...
	if (cow_read_locked == TRUE) {
		lock_read_done_recursive(&(bfAccess->origAccp->cow_lk));
...
)
{
	...
	*mcells_alloced = 0;
	 ...
	 * Make sure all allocations have been done prior to this.  No
	 * ftx_fail calls (or error returns from this routine) allowed once
	...
	for (i = 0; i < nmcell; i++) {
		...
		if (sts != EOK) {
			if (i) {
				...
				/* didn't succeed on first pass, nothing to
				 * dealloc */
...
)
{
	...
	*min_buf_size = 0;
	...
	if (!just_sizing && !error) {
		...
		uiop->uio_resid = uioLcl.uio_resid;	/* upper layer uses
							 * uio_resid */
	}
	if (bbuf != NULL) {
		ms_free(bbuf);
...
)
{
	...
	if (error) {
		sec_info->sec_type = NULL;	/* Failed, set type to be NULL
						 * so we do not attempt any
						 * security operations using
						 * this data. */
	}
	/*
         * Verify that if the attribute is a security
	...
	do {			/* while (!RECCUR_ISNIL(hdr)) */
		...
		while (1) {
			...
			if ((FLAGS_READ(rdflags, hdr_rec->flags) & BSR_PL_DELETED) == 0) {
				...
				/* save header location in case we need to
				 * delete it */
				rdr.hdr = *((bsOdRecCurT *) & hdr);
		...
		if (!RECCUR_ISNIL(hdr)) {
			...
			sec_info.sec_type = NULL;	/* Set to NULL so we do
							 * not perform any
							 * security operarions
							 * the initial retriving
							 * of data. */
			/*
		         * advance hdr to beyond end of current entry,
		         * new entry goes after
