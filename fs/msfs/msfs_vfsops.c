	...
	struct vnode **vpp;
{
	...
	if (!(flag & M_FAILOVER)) {
		...
		if (sts == EOK) {
			 ...
			 * msfs_sync typically does this, but we'll do it if
			 * we can mount successfully too. */
			now_time = time.tv_sec;
			sts = bmtr_put_rec(root_accessp, BMTR_FS_TIME,
			    &now_time, sizeof(now_time),
			    FtxNilFtxH);
			if (sts != EOK) {
				/* likely i/o error; ignore it; update not
				 * critical */
...
{				/* in */
	...
	/* For V5.0 ship, make the -o directIO option hidden and dependent on
	 * the Advfs_enable_dio_mount being non-null. */
	...
	if (mp->m_flag & M_UPDATE) {
		 ...
		 * EXCEPTION:  When booting the machine the root filesystem
		 * may have a temporary mntfromname.  The temp name differs
		 * depending if the machine is in a cluster. Therefore, allow
		 * the change of the mntfromname with this exception.
		 * ASSUMPTION: Only allow differences in mntfromname and
		 * args.spec to go through if the mntfromname is either
		 * default_rootname ("root_device") OR default_bootname
		 * ("local_root"). */
		...
		if (mp == pmp) {
			/* ms_malloc never fails, it doesn't return until
			 * success */
			...
			/* Check to see if "root_device" or "local_root" is
			 * the name, if so, we need to let it mount R/W. The
			 * mount command may have found a missing entry for
			 * "/" or the fstab was missing. */
			...
			if (((strlen(mp->m_stat.f_mntonname) == strlen(rootmpname)) &&
				...
				/* Get vnode of each of the devices and
				 * compare rdev values */
				for (vdi = 0; vdi < dmntbl->vdCount; vdi++) {
					...
					/* Scan through the domain's vdp table
					 * looking for this device. Note that
					 * we have to look through the whole
					 * table as it can be sparse, and the
					 * order is not necessarily the same
					 * as the one in dmntbl. */
					rdev_match = FALSE;
			...
			/* TODO NOTE: The flush/sync needs a mechanism to
			 * track the IO's we flush and sync on. Other IO's can
			 * be started after the flush (by bootstrap link
			 * kernel) causing sync to return a nonzero count. For
			 * now, we ignore the return. */
			(void) bs_bfdmn_flush_sync(dmnP);

			MountWriteDelay = 0;	/* Write delay is only for
						 * root */
...
cleanup:
	if (error == 0) {
		 ...
		 * only does this for rootfs, but during install this should
		 * be updated too;  otherwise if this becomes the new rootfs
		 * the time appears zero. */
		root_accessp = GETROOTACCESS(mp);
		now_time = time.tv_sec;
		sts = bmtr_put_rec(root_accessp, BMTR_FS_TIME,
		    &now_time, sizeof(now_time),
		...
		/* If this is a readonly, dual mount,ensure we flush the log
		 * record onto disk */
...
advfs_mountfs(struct mount * mp)
{
	...
	/* activate the bfset (pass in the advfs mount flags) + special flag
	 * used only in cluster */
	...
	mutex_enter(&dn->dmnP->ssDmnInfo.ssDmnLk.mutex);
	...
	mutex_exit(&dn->dmnP->ssDmnInfo.ssDmnLk.mutex);
	...
	mutex_init(&dn->filesetMutex.mutex, MUTEX_DEFAULT, IPL_NONE);
	...
	if (setp->dmnP->ssDmnInfo.ssDmnState == SS_ACTIVATED) {
		...
		/* Wait for all vfast threads to finish before mounting the
		 * set. */
...
cleanup:
	...
	/* if we got to the point of setting up a fileSetNode pointer we have
	 * more work to do */
		...
		if (sts != 0) {
			...
			if ((vdp->vdSize == actual_vd_size) ||
		                 ...
		                 */
				{
					...
					long page_to_read,	/* Number of page to
								 * read */
...
)
{
	...
	*fsnp->fsPrev = fsnp->fsNext;
	...
	/* reset the vfast first mount time if no other mounted filesets
	 * writable */
	for (fsp = FilesetHead; fsp != NULL; fsp = fsp->fsNext) {
		/* check for non readonly, mounted fset already in this domain */
    ...
    caddr_t arg)
{
	...
	dnp->mountp = mp;	/* XXX should this be initialized in a more
				 * general place?  Is a lock needed? */
...
msfs_statfs(struct mount * mp)
{
	...
	if ((dn->quotaStatus & QSTS_QUOTA_ON) &&
		...
		if (dn->blkSLimit > 0 && dn->blkHLimit > 0) {
			...
			totalBlocks = dmnp->totalBlks;	/* no fset limit; use
							 * dmn's */
		}
	...
	/* Note: UFS would allow availBlocks to go negative because UFS has
	 * the */
...
)
{
	...
	if (smsync_period == 0 || waitfor == MNT_WAIT) {
		...
		/* Move this here from above msfs_sync_todr() to guard against
		 * a dangling log record write to disk from the
...
)
{
	...
	for (vp = mp->m_mounth; vp; vp = nvp) {
		...
		if (bfap = VTOA(vp)) {
			...
			if (bfap) {
				/* Take the access struct off the free list
				 * and bump its refCnt. */
				RM_ACC_LIST_COND(bfap);
				bfap->refCnt++;
				mutex_exit(&bfap->bfaLock.mutex);
		...
		if (AdvfsSyncMmapPages && bfap && (vp == bfap->bfVp) &&
			...
			/* Check to see if we can clear the mmapFlush flag;
			 * only do this if the file is no longer mmapped.
			 * 
			 * Note that bfap->mmapCnt and bfap->mmapFlush are not
			 * maintained in a cluster.  In a cluster, a file
			 * originally mmap'ed on the server of its fileset
			 * will be flushed by this routine (if dirty) as long
			 * as VMMAPPED is set (cleared at cfs_inactive()
			 * time). I.e., VMMAPPED may still be set even after
			 * the file has been unmapped, but only until
			 * cfs_inactive(). */
...
getnext:
		/* Before proceeding on to the next vnode, decrement the
		 * refCnt on the bfap that we are working with, if any, and
		 * vrele its associated vnode.  This order is done explicitly
...
)
{
	...
	for (vols_found = 0, ind = 1;
		...
		if (vdp = vd_htop_if_valid(ind, dmnP, FALSE, FALSE)) {
			...
			/* Check to see if any on the higher I/O queues that
			 * must also be output.  If so, be sure that they get
			 * moved down to the devQ. */
			...
			numout += num_this_vol;	/* Include these in count for
						 * caller */
...
chk_set_fsdev(
    ...
    int fsId,			/* In: fsdev to use. Set to 0 if check_flag
				 * false. */
    int check_flag		/* In: TRUE, verify and create new fsdev, if
				 * needed. */
...
loop:
	if (clu_is_ready()) {
		...
		for (fsp = FilesetHead; fsp != NULL; fsp = fsp->fsNext) {
			/* if setp is Nil then it is under construction (being
			 * mounted) */
			bfSetp = fsp->bfSetp;
	...
	u_int smsync_flag;	/* perform non-smooth'ed operations */
{
	...
	/* Assist UBC in making UBC cache pages available for recycling. If
	 * UBC's dirty metadata page count exceeds the threshold, then issue a
	 * domain log and metadata flush. */
    ...
    int timeout)
{
	...
	msg->frzmMP = mp;
	msg->frzmStatus = 0;
	mutex_enter(&AdvfsFreezeMsgsLock.mutex);
	...
	mutex_exit(&AdvfsFreezeMsgsLock.mutex);
	thread_block();
