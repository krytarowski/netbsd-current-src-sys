...
strtoul(const char *nptr, char **endptr, int base)
{
	...
	if (base >= 0 || base <= MBASE) {
		...
		/* Check for overflow.  If the current value (before adding
		 * current xx digit) is already greater than ULONG_MAX / base,
		 * we know that another digit will not fit.  Also if after the
		 * current digit is added, if the new value is less than the
		 * old value, we know that overflow will occur. */
...
msfs_lookup(
    struct vnode * vp,		/* in - vnode of the directory in which to
				 * look */
    register struct nameidata * ndp	/* in - namei data - contains the file
					 * name to lookup */
)
{
	int error;
	...
	if (BS_BFTAG_EQL(bnp->tag, fsn->tagsTag)) {
		...
		if ((ndp->ni_namelen == 1 && ndp->ni_dent.d_name[0] == '.')) {
			...
			if ((ndp->ni_namelen == 2 && ndp->ni_dent.d_name[0] ==
				...
				set_nocross = 1;	/* set NOCROSSMOUNT if
							 * this entry is a dir */
	...
	if (error = cache_lookup(ndp)) {
		 ...
		 * vnode pointed to by the namei cache entry.  Getting the
		 * wrong, but still valid tag, will be caught by the v_id
		 * test.  Getting a now invalid tag will cause bf_get to fail. */
		VN_LOCK(nvp);
		 ...
		 * be bogus.  Instead call seq_search() to search the
		 * directory. */
		if (nvp->v_id != vpid) {
			vrele(nvp);
			ndp->ni_vp = NULL;
...
lookup:
	...
	if (ret == I_INSERT_HERE) {
		...
		} else {	/* all other cases of not found, including
				 * ni_next != 0 or deleting file... make
				 * negative entry */
			if (ndp->ni_makeentry) {
				cache_enter(ndp);
			}
...
)
{
	...
	*tagp = new_parent_tag;
	...
	mutex_enter(&s_context->fsContext_mutex.mutex);
	s_context->fs_flag |= MOD_MTIME;
	s_context->dirty_stats = TRUE;
	mutex_exit(&s_context->fsContext_mutex.mutex);
