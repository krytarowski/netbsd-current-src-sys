...
)
{
	...
	if (dmnP->dmn_panic) {
		 ...
		 * create_rtdn_opx */
		/* will not change the file bfState from ACC_FTX_TRANS and
		 * later */
		/* umount will hang. We must set the state here, after
		 * ftx_done_fs. */
		/* The files in question are the files created when a fileset
		 * is */
...
)
{
	...
	if (bfap->fragState == FS_FRAG_VALID) {
		...
		if (quotas_done == FALSE) {
			/* only an I/O error can make this fail and we just
			 * log that */
