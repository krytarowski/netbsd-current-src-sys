...
)
{
	...
	if (clu_is_ready()) {
		...
		/* Read the inactive log for CFS xid status If we have an
		 * error, we will skip the inactive log read. One reason we
		 * don't continue: issues with potentially reading past
		 * crashRedo (start of active log) if on-disk lsn is
		 * corrupted. */
		...
		while (logaddr.lsn.num < crashRedo.lsn.num) {
			...
			if ((lrsts != EOK) && (lrsts < MSFS_FIRST_ERR)) {
				/* Skip further inactive log processing for
				 * cfs */
...
freexid:
	...
	if (clu_is_ready()) {
		 ...
		 * a scalar container. We can not pass a pointer into the
		 * domain since the domain struct could be freed by the time
		 * we get to the time out routine. We can not malloc a chunk
		 * either since the untimeout wouldn't have access to this
		 * pointer to free it. */
...
)
{
	...
	do {
		...
		if ((lrsts != EOK) && (lrsts < MSFS_FIRST_ERR)) {
			...
			do {
				...
				if (lrsts == E_INVALID_REC_ADDR) {
					...
					/* Give back the memory given to us
					 * from lgr_read */
					ms_free(segptr);
		...
		for (ftxSlot = 0; ftxSlot < ftxTDp->nextNewSlot; ftxSlot++) {
			...
			if ((ftxTDp->tablep[ftxSlot].state == FTX_SLOT_BUSY ||
				...
				/* in case we don't find ent, save location of
				 * avail slot */
				availSlot = ftxSlot;
			}
		}
		...
		if (ftxSlot == ftxTDp->nextNewSlot) {
			 ...
			 * FtxMutex synchronizes updates to FtxDynAlloc
			 * counters. */
			ftxp = newftx();
			MS_SMP_ASSERT(ftxp != NULL);	/* ms_malloc waits, it
							 * never fails */
		...
		/* Buffer all rootdone records on the first pass through
		 * ftx_recovery_pass().
		 * 
		 * For v4 domains, this is META_META_PASS. For pre-v4 domains,
		 * this is META_PASS. */
		...
		while (redoloff < totwordcnt) {
			...
			if (recoveryPass == META_META_PASS) {
				/* Records to be applied in this pass are
				 * ALWAYS marked with the correct pass # */
				...
				/* Any record to be applied to metadata files
				 * that is not explicitly done in the metameta
				 * pass is done now. */
				if (dlrp->atomicRPass != META_META_PASS &&
		...
		}		/* done scanning for record image redo
				 * subrecords */
	...
	for (ftxSlot = 0; ftxSlot < ftxTDp->nextNewSlot; ftxSlot++) {
		...
		ftxp = ftxTDp->tablep[ftxSlot].ftxp;
		if (ftxTDp->tablep[ftxSlot].state != FTX_SLOT_BUSY &&
		    ftxTDp->tablep[ftxSlot].state != FTX_SLOT_EXC) {	/* not in use */
			continue;
		}
    ...
    int done_flag /* in -- TRUE/FALSE [committed/rolled back] */ )
{
	...
	/* The xid status is kept in the high-order bit, originally used to
	 * indicate whether this ftxId was an xid. */
...
start:
	...
	if ((mp = getvfs(&fsid)) != NULL) {	/* UNMOUNT_LOCK(mp) held if
						 * success */
	...
	while (xidInfop != NULL) {
		...
		while (xidInfop->data[slot] != NULL) {
			...
			/* Compare the lower bits of the incoming xid with the
			 * lower bits of the stored xid. */
