...
)
{
	...
	*bsXACnt = undoRec.xCnt;
	 ...
	 * have had their xtnt maps deleted via a prior call to add_stg_undo.
	 * This routine would then update their on-disk xtnt maps. The next
	 * open of the file would trigger the new xtnt maps to be reread from
	 * disk. The RBMT/BMT however is never closed. We can not just remove
	 * the in memory xtnt maps in add_stg_undo since other routines (other
	 * undo routines) expect the maps to be present. So only after we have
	 * actually updated the on-disk xtnt maps should we update the
	 * in-memory xtnt maps. NOTE: The following asumptions about the
	 * RBMT/BMT are being made here:
	 * 
	 * 1) The bfap is available via the vd strucuture. 2) There RBMT/BMT is
	 * NOT striped. 3) The RBMT/BMT is never sparse (The call to
	 * imm_get_alloc_page_cnt is probably unnecessary (just use nextPage). */
...
)
{
	...
	if (FIRST_XTNT_IN_PRIM_MCELL(bfap->dmnP->dmnVersion, bfap->xtnts.type)) {
		/* for version 4 domains the whole problem of mcell not
		 * describing ANY storage has been fixed - therefore, we don't
		 * need to rewrite the extent map to clean up these maps. Just
		 * return here. */
	...
	for (i = xtntMap->updateStart; i <= xtntMap->updateEnd; i++) {
		...
		if (subXtntMap->updateStart < subXtntMap->cnt) {
			...
			if (subXtntMap->mcellState == NEW_MCELL) {
				...
				/* This subxtnt has a brand new Mcell, we must
				 * create its on-disk xtnt rec. */
				...
				/* This is not a new MCELL so just update the
				 * xtnt rec on-disk */
			...
			if ((subXtntMap->mcellState == NEW_MCELL) ||
				...
				/* Test to see if the subxtnt we are updating
				 * is a shadow in which case our previous
				 * mcell is the primary. */

				if (subXtntMap->type != BSR_XTRA_XTNTS) {

					/* Currently we can not tolerate being
					...
					/* Test to see if the subxtnt we are
					 * updating is the first on the update
					 * list. If it is then we are creating
					 * the MCELL for it and must link it
					 * in at the origStart. otherwise just
					 * link the list to the previous one
					 * on the update list. */
...
)
{
	...
	case BSR_XTNTS:
		...
		/* This is the case where we give truncated storage to the
		 * clone. */
		if (xferFlg) {
			odattrp->cloneId = 1;
		} else {
		...
		/* If positive, this is the stripe index+1 this stg belongs
		 * to. */
		/* If negaitive, this is not a striped file. rsvd1 will not be
		 * used. */
...
)
{
	 ...
	 * but depending on how the caller wants to treat the returned xtnt
	 * maps, we will lock the extent maps and return with the requested
	 * locks held.
	 * 
	...
	if (lock_request == X_LOAD_REFERENCE) {
		XTNMAP_LOCK_READ(&(bfap->xtntMap_lk))
		    if (xtnts->validFlag) {
			/* If extents are valid, just return with xtntMap READ
			 * locked */
			return EOK;
		} else {

			/* release the READ lock and then seize both locks in
			 * anticipation of reloading the extent maps; the
			 * mcellList lock get read-locked to prevent racing
		...
		/* This is the rare case: lock_request=X_LOAD_LOCKSOWNED. If
		 * the extents are valid, return.  Otherwise fall through and
		 * reload them. */
		if (xtnts->validFlag) {
	...
	/* If we get here, we have to reload the extent maps from disk; we
	 * have WRITE lock held on the xtntMap lock and either a read or write
	 * lock on the mcellList_lk depending on the value of lock_request. */
	...
	/* If caller only wants to reference the extent maps, release the
	 * mcellList lock and downgrade the xtntMap lock to a read lock.
...
)
{
	...
	while (nextVdIndex != bsNilVdIndex) {
		if (xtntMap->cnt == mcellCnt) {
			/* We must test for shadowRec->mcellCnt wrap. Look at
			 * next mcell. */
			/*
			...
			if (shadowRec != NULL) {
				/* If the next mcell is a BSR_SHADOW_XTNTS
				 * then the mcellCnt */
				/* was correct (it did not wrap). */
				break;
			}
			/* The next mcell is not a BSR_SHADOW_XTNTS so this is
			 * still part */
			/* of a very fragmented stripe. The mcellCnt is at
			 * least 64K more */
			/* than indicated on disk (the short on disk mcellCnt
			 * wrapped). */
			mcellCnt += 0x10000;
		}
...
)
{
	...
	if (mdap != NULL) {
		while (nextVdIndex != bsNilVdIndex) {
			 ...
			 * recovery.  This can happen if the next BMT page
			 * extents have not been flushed to disk, but we
			 * crashed, and will update the BMT extents during
			 * recovery (has not happened yet since vdp->bmtp is
			 * still NULL or state is BFD_RECOVER_REDO). */
			if (sts == EOK &&
			    nextVdIndex == 0 &&
			    RBMT_THERE(dmnP) &&
			    (vdp->bmtp == NULL ||
...
)
{
	...
	/* If the file has extent information in the primary mcell, start
	 * there.  Otherwise, start in the mcell pointed to by the primary
	 * mcell. */
...
x_dealloc_extent_maps(bsInMemXtntT * xtnts)
{
	bsInMemXtntMapT *xtntMap, *nextXtntMap;
