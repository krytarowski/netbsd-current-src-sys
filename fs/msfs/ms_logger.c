 ...
 */
typedef union {
	logPgBlksT logPgBlks;	/* physical log page structure; array of
				 * blocks */
...
)
{
	...
	if (pgp->hdr.pgSafe) {
		 ...
		 * 
		 * Only during recovery should we be concerned with the data in
		 * the log file being incomplete. After domain ACTIVATION all
		 * data in the log file should be fine? */
...
)
{
	...
	if (pgp->hdr.pgSafe) {
		...
		while (fix_st < fix_ed) {
			...
			/* overwritten and therefore it is not saved in
			 * lsnOverwriteVal. */
...
)
{
	...
	while (totWdsWritten < bufWords) {
		...
		/* We must obtain the FLUSH write lock to protect the fields
		 * in the descriptor that govern which log pages are being
		 * written. */
		if (AdvfsSpinTryCount) {
			int try_count = AdvfsSpinTryCount;
		...
		/* Never allow a record to cross a page boundary unless the
		 * record */
		/* (plus header) is larger than a page. NOTE: The ftxDoneLRT
		 * which is present in the segment 0 records is actually
		 * included in bufWords
		 * 
		 * We will unconditionally move off this page if a flush is
		 * currently outstanding. We used to sync up and reuse the
		 * page to save log space? but performance numnbers show that
		 * moving on is better. I'm unsure of the effect on the log
		 * filling up too quickly. We could as an optimization
		 * consider waiting based on how much if the current page is
		 * unused. */

		if ((bufWords + REC_HDR_WORDS > DATA_WORDS_PG - ldP->nextRec.offset) ||
		    (ldP->flushing)) {
			/* There is not enough room left on the LOG page for
			 * this record. */
			...
			if ((bufWords + REC_HDR_WORDS <= DATA_WORDS_PG) ||
				...
				/* first segment needs room for logRecHdrT &
				 * ftxDoneLRT OR */
				/* subsequent segments only need room for
				 * logRecHdrT. If only the header would fit
				 * then move on to the next page */

				/* NOTE other than the first time through the
				 * log page is actually not unpinned here
				 * since the writers count is up */

				sts = release_dirty_pg(ldP);
				if (sts != EOK) {
					RAISE_EXCEPTION(sts);
				}
				/* Clear the flusing bit to indicate that the
				 * last page of the log is now available. */
				ldP->flushing = FALSE;
			}
		}
		...
		if (segment == 0) {
			/* This is the starting point for the record in the
			 * log page. Save this off for actually moving the
			 * data.  */
		...
		/* We have the last log page in place. We may now allow
		 * writers to unpin any old log pages. */
		...
		/* It is important that we initialize our nextRec while
		 * holding the log descriptor lock. Since we update the prev
		 * recs next rec to point to us and this may be done to our
		 * nextrec too. */

		((logRecHdrT *) & ldP->lastPg.pgP->data[ldP->nextRec.offset])->nextRec = logEndOfRecords;

		if (segment == 0) {
			firstSeg = ldP->nextRec;
		}
	...
	for (pli = clvlp->lastPinS; pli >= 0; pli -= 1) {
		 ...
		 * Normally only the buffer cache routines manipulate the
		 * bsBuf fields. */
		bp = (struct bsBuf *) fpp->pgH;
		if (LSN_EQ_NIL(bp->origLogRec.lsn)) {

			/* protect the changing of origLogRec with the
			 * dmnP->lsnLock to make locking more explicit.  This
	...
	/* Now we can copy the log records onto the space that we reserved
	 * previously. */
	...
	while (totWdsWritten < bufWords) {
		...
		/* segWdsToWrite counts ftxDoneLRT but does not count
		 * logRecHdrT. */
		...
		/* We are done with this page. We need to determine if we
		 * should unpin. */
		if (AdvfsSpinTryCount) {
			int try_count = AdvfsSpinTryCount;
			while (FLUSH_TRY_WRITE_LOCK == 0) {
		...
		if (ldP->lastPg.num != current_log_page) {
			/* Another writer moved to the next log page. We need
			 * to locate our active rec from the active list */
			int index;
			...
			if (--ldP->wrtPgD[index].writers == 0) {
				/* We are the last writer of this page we must
				 * now unpin so that it may be flushed. */
			...
			/* This is still the last log page. We must check to
			 * see if a flush was requested while we were writing
			 * this page */
			if ((--ldP->lastPg.writers == 0) && (ldP->flushing)) {
				/* There is an oustanding flush. The only way
				 * the flushing bit can be set is if we are in
				 * the flush range and no other thread called
				 * lgr_writev_ftx. Otherwise we would have
				 * moved off this page. */

				KASSERT(LSN_GTE(ldP->flushLsn, ldP->lastPg.pgP->hdr.thisPageLSN));

				/* We know 3 things: 1) We are the last writer
				 * on the page. 2) No more writers can get in
				 * 3) The lastRec.lsn has not changed since
				 * the thread that started the flush began
				 * waiting for it.
				 * 
				 * We will unpin this page so that it will be put
				 * on the i/o queues (blocking) to be flushed.
				 * Since the flushing bit is set we also know
				 * that any threads about to write the log
				 * will attempt to reuse this page but will be
				 * forced off. */
				...
				/* We do NOT want to clear the flushing bit.
				 * The only time it should be cleared is when
				 * we either sync up and actually know the
				 * page made it to disk or if we move off the
				 * page. */


				/* Don't update log stats here since they were
				 * updated already at flush time */
			}
		}

		if (totWdsWritten < bufWords) {
			/* We have a segmented write underway. We already have
			 * the log space reserved so set up for the next time
			 * through the loop. */
			int index;
			...
			/* By reserving the space beforehand we know that we
			 * have the begining of the next page available for us
			 * (offset = 0) */
...
resetfirstrec(domainT * dmnP)
{
	...
	if (LSN_EQ_NIL(firstRec.lsn)) {
		/* dmnP->lsnList is empty; get value of oldest outstanding
		 * transaction */
	...
	/* Save value of page from log rec above into the oldestPg of log
	 * desc. */
...
)
{
	...
	if (ldP->lastPg.pinned) {
		...
		if (ldP->lastPg.writers == 0) {
			...
			/* The page is full but, it is currently being copied
			 * to. Move it onto the active page queue and it will
			 * be unpinned when the last writer completes */
			int index;
...
)
{
	...
	*buf = NULL;		/* Initialize to NULL for testing at exception
				 * time */
	...
	if (!rdP->refed) {
		if (bs_logpage_dirty(ldP->logAccP, rec_pg)) {
			 ...
			 * flushed to disk). We can not call lgr_refpg because
			 * we will write over the dirty page's lsns. We know
			 * however that the page is safe to reference, copy
			 * locally and then restore. There is no need to wait
	...
	*buf = (uint32_t *) ms_malloc((*bufWords) * 4);
	...
	switch (rdMode) {
	case LR_FWD:
		/* We're doing a forward read so return the next record's
		 * address */
		...
		if (RECADDR_EQ(recHdr.nextRec, logEndOfRecords)) {
			...
			if (recHdr.segment & MORE_SEGS) {
				if ((rec_pg == ldP->nextRec.page) ||
					...
					(ldP->nextRec.offset == 0))) {
					/* We've reached the last record in
					 * the log */
					*recAddr = logEndOfRecords;
					...
					/* Return address of first record in
					 * the next page */
					recAddr->offset = 0;
					recAddr->page = (rec_pg + 1) % ldP->pgCnt;

					/* We have to read the next page to
					 * get the rec's LSN */

					lsts = bs_refpg(&refH, (void *) &pgP, ldP->logAccP,
					    recAddr->page, BS_SEQ_AHEAD);
					if (lsts != EOK) {
						RAISE_EXCEPTION(lsts);
					}
					/* Get LSN from rec header from first
					 * rec in next log pg */
				...
				*recAddr = logEndOfRecords;
				...
				/* Return address of first record in the next
				 * page */

				recAddr->offset = 0;
				recAddr->page = (rec_pg + 1) % ldP->pgCnt;

				/* We have to read the next page to get the
				 * rec's LSN */
				...
				/* Get LSN from record header from first rec
				 * in next log page */
			...
			/* Return the address of the next record (on the
			 * current page ) */
	...
	case LR_BWD:
		/* We're doing a backward read so return the prev record's
		 * address */
...
)
{
	...
	/* The calculation for maxLSNs used to be: maxLSNs = LSN_INCR_AMT *
	 * LOG_RECS_PG * (ldP->pgCnt + (ldP->pgCnt - 1) *
	 * LOG_FLUSH_THRESHOLD);
	 * 
	 * which gives the minimum maximum spacing between any two lsns in the
	 * log this can be replpaced by 1<<29 which is an eighth of the log.
	 * In other words no two consecutive lsns can be more than 1/8 of the
	 * max number of lsns apart. */
...
_finish:
	...
	/* We need to locate the last record on the last log page found from
	 * the above binary search. From this record, we can find the first
	 * active log page. We then must start searching thru each log page
	 * starting from this first page making sure that lsn are increasing.
	 * If we find an lsn that does not increase then the page prior to it
	 * is the new log end. This is necessary because of out of order
	 * writes. We know that all meta data was held up from going out from
	 * any pages in the active range. Thus it is safe to truncate the
	 * active range back. Note that we may end up pulling the start page
	 * back too since recovery will use the start page (crashRedo)
	 * contained on this potentially new log end page, this could be
	 * optimized by saving off the start page for recovery to use.
	 * Regardless it is not incorrect to recover using the new page since
	 * we will only be recovering metadata that is already out on the
	 * disk. This is wasteful but not incorrect. */
	...
	for (i = 1; i <= searchPgs; i++) {
		...
		if (LSN_EQ_NIL(prevLSN)) {
			...
			if ((LSN_LT(pgLSN, crashRedo.lsn) &&
				...
				/* If the range of possible LSNs for this page
				 * are less than the crashRedo's lsn for this
				 * page then this is a stale/old log page as
				 * talked about above */
...
)
{
	...
	if (noBlock) {
		/* Try to get the lock, but if we can't, just return
		 * E_WOULD_BLOCK. This prevents a deadlock if the calling
		 * thread already has a logDescT already locked. */
	...
	/* If the flushing bit is already set then we know the last page is
	 * being flushed. Thus we can skip starting the flush since all
	 * outstanding ftxs are being flushed */
	...
	if (!ldP->flushing &&
		...
		/* If the last page is not within the flush range it really
		 * isn't necessary to set the flushing bit. */

		if (flushlastpage) {
			/* Indicate to future writers that they must wait for
			 * the flush to complete. Also indicate to active
			 * writers that the last page needs to be unpinned. */
...
)
{
	...
	/* We need to be careful here. This sync may have already been
	 * serviced by a previous thread and a new flush may have been
	 * started. We must not clear the flushing flag or attempt to pin the
	 * last page if this is the case. We will know this by checking the
	 * current flushLsn. */
	...
	if (ldP->flushing) {
		 ...
		 * well set up the last page to be reused. This poses a
		 * problem that must be dealt with in recovery if this code is
		 * left here. Basically we are repinning a page that has
		 * already been written to disk once. Lets say we are page 10.
		 * If page 11 now gets pinned and written out to disk prior to
		 * us being written out for the 2nd time (this is possible)
		 * then the records that are written during the repin will be
		 * lost but pointed to by page 11. */
...
)
{
	...
	/* Try to put a BSR_DMN_FREEZE_ATTR on the new Log disk. This record
	 * is for freezefs debugging only so we tolerate failure. */
