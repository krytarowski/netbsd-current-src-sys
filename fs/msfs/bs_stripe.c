...
)
{
	...
	/* This test only tells us if the file is already striped. It is
	 * necessary to again check xtnts.type once we have the mcellList_lk. */
...
)
{
	...
	for (i = 0; i < segmentCnt; i++) {
		...
		/* add to skip list so we allocate next segment from another
		 * disk */
...
)
{
	...
	/* can't stripe reserved files, pass BMT_NORMAL_MCELL to
	 * bmt_alloc_mcell */
