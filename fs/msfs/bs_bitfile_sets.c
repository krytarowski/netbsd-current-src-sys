...
bs_fragbf_thread(void)
{
	...
	while (TRUE) {
		...
		while (!done) {
			 ...
			 * chown, and explicit quota setting - it is never
			 * removed (at least currently). The potential danger
			 * path would be stg_remove_stg_start -->
			 * fs_quota_blks --> dqsync --> rbf_add_stg But it is
			 * tame in this case. */
...
)
{
	...
	if (grpPg == lastGrpPg) {
		/* update free list descriptor tail pointer to point to the
		 * group */
...
)
{
	...
	grpHdrp->nextFreeFrag = BF_FRAG_EOF;	/* used only when 'version ==
						 * 0' */
	grpHdrp->lastFreeFrag = BF_FRAG_EOF;	/* used only when 'version ==
						 * 0' */
...
)
{
	...
	if (setp->fragGrps[BF_FRAG_FREE_GRPS].firstFreeGrp == BF_FRAG_EOG) {
		 ...
		 * imm_get_first_hole. Returns with fragBfAp->xtntMap_lk
		 * read-locked. */
		...
		for (g = initGrps - 1; g >= 0; g--) {
			...
			/* update free list; new group goes to front of free
			 * list */
...
)
{
	...
	ftx_lock_write(&setp->fragLock, ftxH);
		...
		if (grpHdrp->freeFrags == 0) {
			...
			/* Update free list head for this frag list to point
			 * to next group */
			RBF_PIN_FIELD(pinPgH, setAttrp->fragGrps[fragType].firstFreeGrp);
...
unsigned int AdvfsFragGroupDealloc = 1;
 /* sysconfig tunable to set frag group deallocation policy: 0: disables
  * deallocation of frag groups 1: enables deallocation of frag groups
  * (default) */
/*
 * bs_frag_dealloc
 *
...
)
{
	...
	if (grpHdrp->freeFrags == 1) {
		...
		if (lastGrpPg == BF_FRAG_EOG) {
			/* also update the list head pointer if the list was
			 * empty */
...
struct bfsFreeListStats {
	int freeCnt;		/* allocation decrements freeCnt - negative is
				 * OK */
...
)
{
	...
	mutex_init(&LookupMutex.mutex, MUTEX_DEFAULT, IPL_NONE);
...
)
{
	...
	KASSERT(mutex_owned(&LookupMutex.mutex));
...
start:
	...
	while (currbfap != (bfAccessT *) (&bfSetp->accessFwd)) {
		...
		mutex_enter(&currbfap->bfaLock.mutex);
		...
		if (((accState == ACC_INVALID) &&
			...
			mutex_exit(&currbfap->bfaLock.mutex);
...
)
{
	...
	mutex_enter(&LookupMutex.mutex);	/* bfs_lookup synchronization */
	...
	*bfSetp = nilBfSet;
	...
	mutex_init(&bfSetp->cloneDelStateMutex.mutex, MUTEX_DEFAULT, IPL_NONE);
	...
	mutex_init(&bfSetp->bfSetMutex.mutex, MUTEX_DEFAULT, IPL_NONE);
...
bfs_dealloc(
    ...
    int deallocate		/* in - flag indicates to remove the set from
				 * existence */
)
{
	...
	if (deallocate) {
		...
		mutex_destroy(&bfSetp->setMutex.mutex);

		mutex_destroy(&bfSetp->cloneDelStateMutex.mutex);
		lk_destroy(&bfSetp->cloneDelState);

		mutex_destroy(&bfSetp->accessChainLock.mutex);

		mutex_destroy(&bfSetp->bfSetMutex.mutex);
...
)
{
	...
	if (BS_BFTAG_REG(bfSetp->dirTag)) {
		...
		/*
	         ...
	         */
		/* Actually we only have one clone fileset (max) for each
		 * original */
		...
		while (!BS_BFTAG_EQL(setDirTag, NilBfTag)) {
			...
			if (sts != EOK) {
				...
				(void) bfs_close(bfSetp, ftxH);
				/* close the original opened above if clone
				 * requested */
				if (bfSetp != prevSetp)
...
)
{
	...
	while (1) {
		...
		/* Instead of calling bs_access (which opens the clone then
		 * fails if it can't open the orig) I have "inlined" bs_access
		 * here so that if the clone exists and the orig does not (due
		 * to a racing delete the orig) I will still remove the clone. */
		if (BS_BFTAG_RSVD(tag)) {
			continue;
		}
		/* Open the original first. We need to do this since we are
		 * not holding any locks. If the clone file we are about to
		 * clean up has never had its metadata cloned, its primary
		 * mcell will be the same as the original file. If the
		 * original is being deleted then we need to make sure its
		 * primary mcell can not be freed while we are trying to map
		 * in the clone. By opening the original first we will either
		 * find that it is already deleted (we raced) or we will keep
		 * it from being removed by opening it. if it already is
		 * deleted then the call to access_one on the clone will fail
		 * and will just clean up the tag.
		 * 
		 * The race that is being avoided here is when we are mapping in
		 * the extentmaps and the primary mcell goes away */
		nullvp = NULL;
		...
		/*
	         ** Delete the clone.
	         */
		/* The transaction starts here vs before bs_access since any
		 * transaction can behave like an exclusive transaction if the
		 * transaction list wraps. An exclusive transaction (really
		 * any lock in the bs_access path) can deadlock if held thru
		 * bs_access for a clone since that will bs_access the orig
		 * which may block on VINACTIVATING (in vgetm). The thread
		 * that set VINACTIVATING can block on a tranaction. This is a
		 * lock ordering problem. See bs_cow. */
 ...
 */
	...
	static void
		...
		/*
	        ...
	        */
		mutex_enter(&origSetp->dmnP->mutex.mutex);
		origSetp->cloneSetp = NULL;
		mutex_exit(&origSetp->dmnP->mutex.mutex);
 ...
 */
	...
	int
		...
		/* Add in threads currently xfering xtnts in
		 * xfer_xtnts_to_clone. We wait for these xfering threads
		...
		refCnt = 0;	/* refCnt is also a BfSetTablLock-needs-unlock
				 * flag */
		BFSETTBL_UNLOCK(dmnP);
		...
		/* If we're deleting a clone fileset, wait until any threads
		 * currently in xfer_xtnts_to_clone() finish their work.  This
		 * also blocks new xfer_xtnts_to_clone() threads from
		 * accessing the clone fileset. */
		...
		BFSETTBL_UNLOCK(dmnP)
		/* in a cluster ensure removing a clone fset cannot be undone
		 * if the server fails because we are about to notify all
		 * cluster nodes that they can write directly to storage if
		 * they hold the appropriate tokens.  Because this bypasses
		 * AdvFS, no COWs will be performed. */
...
HANDLE_EXCEPTION:
		...
		mutex_enter(&dmnP->mutex.mutex);	/* bind bfsDelPend &
						 * BFS_OD_OUT_OF_SYNC */
		...
		if (bfSetp->bfSetFlags & BFS_OD_OUT_OF_SYNC) {
			...
			/* But bs_bfs_delete failed so now the clone is out of
			 * sync. */
 ...
 */
	...
	static int
		...
		if     (cloneDirParams.numPages < numDirPages) {
			/* we can't require the migTruncLk to be held here
			 * because, even if the fileset is a clone, the
			 * fileset tag directory is never a clone (in the
			 * sense that we never do COW on it - it is created
			 * once and for all here in this routine), and we are
			 * already inside a transaction by the time we get an
			 * access structure for it (in bs_bfs_clone()). But
			 * since the transaction is exclusive, migrate will
			 * not race with the rbf_add_stg() here. */
 ...
 */
	...
	int
		...
		/* Racing bs_bfs_clone will see bfsOpPend or numClones and
		 * bail. */
		 ...
		 * domain to finish, so we must prevent new reads and
		 * non-allocating writes from racing with us or we can wait
		 * here for an indeterminate amount of time while holding an
		 * exclusive transaction.  To do this, we synchronize with
		 * read, write, and other clonefsets using the domain's
		 * BFD_DOMAIN_FLUSH_IN_PROGRESS flag. */
		mutex_enter(&dmnP->mutex.mutex);
		if (dmnP->dmnFlag & BFD_DOMAIN_FLUSH_IN_PROGRESS) {
			/* The domain flush is being completed for us by a
			 * racing clonefset. */
...
HANDLE_EXCEPTION:
	...
	void
		...
		mutex_enter(&bfSetp->bfSetMutex.mutex);
		bfSetp->bfSetFlags |= BFS_OD_OUT_OF_SYNC;
		mutex_exit(&bfSetp->bfSetMutex.mutex);
		return;
	}
 ...
 */
	...
	int
	    ...
	    u_long flag,	/* in - advfs related mount flags - defs in
				 * mount.h */
		...
		/* Try the fast path to the in memory bfSetIdT structure if
		 * the bfSet is mounted. We need to look through the
		 * associated filesets in the active domain structure.  If we
		 * find the matching fileset, then we can grab the bfSetId
		 * from there. Otherwise find the bfSetIdT on the disk. */
		...
		if (bfSetP != NULL) {
			/* Found the bfSetId in memory already.  Let's make a
			 * copy of it for ourselves. */
 ...
 */
	...
	static int
		...
		/*
		    ...
		    mutex_enter(&cloneBfAp->bfaLock.mutex);
		    lk_set_state(&cloneBfAp->stateLk, ACC_INIT_TRANS);
		    mutex_exit(&cloneBfAp->bfaLock.mutex);
		    initState = TRUE;
		 ...
		 * preallocated storage. The preallocated storage gets cloned
		 * but the file_size prohibits the clone from accessing it.
		 * This is correct behavior but inefficient use of storage.
		 * Unfortunately if we used the file_size then truncate would
		 * misbehave (as well as some meta data files do not keep
		 * file_size up to date). It would be a nice optimization to
		 * not clone preallocated storage */
...
HANDLE_EXCEPTION:
		...
		if (cleanXtnts) {
			...
			/* This was set during the call to bs_map_bf. We must
			 * reset this otherwise we will think that our primary
			 * mcell is not the originals */
 ...
 */
	...
	int
	    bs_have_clone(
	        ...
	        bfSetT * cloneSetp,	/* in - clone bitfile set's desc
					 * pointer */
	        ftxHT ftxH	/* in - ftx handle */
	) {
		bfMCIdT mcid;
...
*/
		...
		mutex_enter(&dmnP->mutex.mutex);	/* protects bfSetp->cloneSetp */
		...
		if (cloneSetp->bfsDelPend) {
			/* The clone set is being deleted. Tell the caller it
			 * doesn't exist. */
			bfSetp->bfsHoldCnt++;
...
#define FILE_LOCK_TAKEN 0x10
	...
	static int
		...
		if      (cloneap->dataSafety != BFD_FTX_AGENT &&
			...
			/* If in a cluster, then provide mutual exclusion with
			 * cluster directio reads on the clone.  These reads
			 * may have caused extent maps for this file to be
			 * cached on other cluster nodes.  This callout will
			 * cause all cluster nodes to delete their cached
			 * versions and re-read the map after bs_cow_pg has
			 * finished modifying the xtnt map. If
			 * clu_clonextnt_lk is already locked then a caller
			 * higher in the stack (eg fs_setattr) has already
			 * purged other node's cached xtnt maps and acquired
			 * the tokens. */
			if (clu_is_ready() && !lock_holder(&cloneap->clu_clonextnt_lk)) {
				fsid_t fsid;
				       BS_GET_FSID(cloneap->bfSetp, fsid);

				/* The clu_clonextnt_lk is not necessary here
				 * since the COW lock is obtained when
				 * requesting the clone xtnt maps. */

				/* Ok if this fails.  It just means no cluster
				 * nodes have the extent map cached.  No one
				 * can get the extent map while we hold the
				 * COW lock. */
				if     (cloneap->bfSetp->fsnp) {
					/* The CFS DIO token should be in the
					 * lock (resource) */
			...
			/* The file lock must be taken after the cluster
			 * token.  This lock prevents the file open mode from
			 * changing from cached mode to directIO mode while
			 * the cow is in progress.  This lock may already be
			 * held if we are being called from bs_pinpg(). */
			if  (!lock_holder(&VTOC(bfap->bfVp)->file_lock)) {
				FS_FILE_READ_LOCK_RECURSIVE(VTOC(bfap->bfVp));
				token_taken |= FILE_LOCK_TAKEN;
			}
			/* For files opened for directIO, we need to seize an
			 * active range to guard the page range being cowed
			 * from racing readers/writers. This range must be
			 * seized before the root transaction is started. */
		 ...
		 * and get the token before we lock the cow_lk This would be
		 * bad since we would not know to revoke their token/xtntmaps
		 * after the COW is complete.
		 * 
		 * Each time a cluster client request a clone extent map they
		 * will (while holding the cow_lk) set the cloneXtntsRetrieved
		 * flag in the access structure. This will tell us if they
		 * snuck in during the above window.
		 * 
		 * This loop should generally only happen once. If the client did
		 * sneak in then they will be holding the token and when we
		 * loop we will wait for them to drop the token breaking the
		 * loop. */
...
*/
	static int
		...
		if (cnt && bfap->bfVp && bfap->bfVp->v_flag & VDIRECTIO) {
			 ...
			 * either: 1) outside of our active range (base), or
			 * 2) won't be explicitly invalidated (base and CFS).
			 * 
			 * COW as many pages as the active range covers. Note
			 * that the active range may cover pages partially. It
			 * is OK to COW these pages: the pinpg path worries
			 * explicitly about two active ranges covering the
			 * same page and does the right thing. The first
			 * thread that tries to pin the page marks the bsBuf
			 * DIO_REMOVING. Any subsequent would-be pinpg'ers
			 * wait. When the page has been flushed and
			 * invalidated, the waiters are woken up and go look
			 * up the page again.
			 * 
			 ...
			 * hits a hole in the original, and we will not even
			 * get to this point if the page has been COWed
			 * already.
			 * 
			 * The hope is that we will COW the whole active range in
			 * one gulp (and improve our chances of getting a
			 * single extent for the whole gulp), so all
			 * subsequent attempts to COW will find the pages
			 * COWed already.
			 * 
			...
			while (arPg < pg) {
				...
				/* If the page is already mapped we need to
				 * find the first unmapped page. */
				arCnt -= nextPg - arPg;
				arPg = nextPg;
			}
		...
		/* extent (hole or not) in the orig starts after the cow
		 * range. */
		/* rbf_add_overlapping_clone_stg will not make a permanent
		 * hole */
		/* since holePg is not adjacent to the cow range. */
		/* If holePg < lastPg then the above loop found a hole within
		 * the */
		/* cow range and rbf_add_overlapping_clone_stg will add a
		 * permanent */
		/* hole after the cow pages. */
		/* If holePg == lastPg then the original has an extent (hole
		 * or not) */
		/* That starts right at the end of the cow range. */
		/* This could be because we limited the cow range to the end
		 * of the */
		/* original file (nextPage) or the end or the clone file
		 * (maxClonePgs). */
		/* In those cases we don't want to make a permanent hole after
		 * the */
		/* cow page range. It could be that the next extent in the
		 * original */
		/* just happens to be at the end of the cow range. In this
		 * case also */
		/* we choose not to make a permanent hole after the cow range. */
		/* To force rbf_add_overlapping_clone_stg to not make a
		 * permanent hole */
		...
		if (holePg == lastPg) {
			if (holePg == bfap->nextPage || holePg >= cloneap->maxClonePgs) {
				/* Don't add a hole beyond the end of the
				 * file. */
				holePg++;
			} else if (page_is_mapped(bfap, holePg, &nextPg, TRUE)) {
				/* If the 32 pages ends right on an extent
				 * boundary, */
				/* don't add a hole. */
				holePg++;
			}
 ...
 */
	...
	static bsPageT
		...
		/* This and all succeeding pages are 'uncowable' because they
		 * are after the EOF of the original when it was cloned;
		 * maxClonePgs never changes, so no need to seize locks for
		 * this check. */
		...
		/* Lock the clone's migTrunckLk & the orig's file_lock and
		 * cow_lk. May take clu token. An active range will be seized
		 * if the file is open for directIO. */
		...
		if (cloneSetp->state == BFS_DELETING) {	/* protected by starting
							 * ftx */
			bfap->noClone = TRUE;
			goto error_exit;
		}
		...
		/* Determine how many pages will be cowed; this number will
		 * depend on the open mode (cached vs directIO) and the number
...
error_exit:
		...
		return (0);	/* error occurred; caller should not try to
				 * cow more pgs */
	}
	...
	void
	     bs_cow(
	         ...
	         unsigned long pg,	/* in - page being modified (if
					 * applicable) */
	         unsigned long pgCnt,	/* in - the # of pages being modified
					 * (if app); */
	/* only needed if COW_PINPG is specified  */
		...
		if   (bfap->cloneCnt != bfSetp->cloneCnt) {
			...
			if (cloneSetp->bfSetFlags & BFS_OD_OUT_OF_SYNC) {
				...
				/* clone bfset exists but is out of sync with
				 * the orig We do not have our own cloned
				 * metadata so we can not store outofSyncClone
				 * out to disk. */
				bfap->noClone = TRUE;
				cloneap->outOfSyncClone = TRUE;
				goto error_exit;
			...
			if (sts != EOK) {
				...
				/* We weren't able to get a primary MCELL so
				 * we can't mark the clone as out-of-sync on
				 * disk we'll mark it in memory and in the
				 * fileset. The clone is truely out of sync */
...
_finish:
		...
		if (cowOpt & COW_PINPG) {
			 ...
			 * see if we need to do any 'copy on write'
			 * processing. This loop is needed because the cow
			 * algorithm, although it will try to cow a range of
			 * pages, may not be able to cow the entire range in
			 * one fell swoop, so we go back in again if that
			 * happens. */
			while (i < (pg + pgCnt)) {
 ...
 */
	...
	int
		...
		bsBfSetAttrT bfsAttr;
		bsQuotaAttrT quotaAttr;
		int ftxStarted = FALSE, bfSetTblLocked = FALSE, mcellListLocked = FALSE, exclusiveFtx = FALSE;
		ftxHT ftxH;
		...
		if (parentFtxH.hndl == 0) {
			/* valid xids (xid>0) only come from
			 * bs_set_bfset_params(), * which always starts a new
			 * transaction.                   */
...
*/
	int
		...
		*cloneapA = cloneap;	/* so the caller can remember to deref
					 * the clone */
		...
		while (!do_clxtnt_unlock) {
			/* Try to get the token only if the clone fileset is
			 * mounted. */
			/* hold_cloneset prevents a race here. The clone
			 * fileset cannot */
			/* be transitioning to mounted at this time. It is
			 * either mounted */
			/* or not and will remain so until
			 * release_clu_clone_locks is called. */
			if (clonesetp->fsnp) {
				/* Getting the token retrieves the extent map
				 * that was */
				/* checked-out to the client. Once no client
				 * has the extent map */
				/* we can lock clu_clonextnt_lk and know they
				 * can't get a map. */
				/* CC_CFS_COW_MODE_ENTER will fail if there is
				 * no cnode. */
				if (CC_CFS_COW_MODE_ENTER(fsid, cloneap->tag) == 0) {
					/* Since we got the token
					 * bs_get_clone_xtnt_map can't be */
					/* running. So no race to set
					 * cloneXtntsRetrieved. */
