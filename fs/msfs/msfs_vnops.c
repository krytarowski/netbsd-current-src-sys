...
)
{
	...
	/* Disable the O_CACHE flag for first ship of V5.0. Remove next line
	 * to reenable that capability. */
	mode &= ~O_CACHE;
	...
	if (mode & (O_DIRECTIO | O_CACHE)) {
		...
		if (mode & O_DIRECTIO) {
			 ...
			 * BaseOS: 1. If file is already open for directio,
			 * just return success. 2. If reserved file and we are
			 * enabling the hidden mount '-o directIO' option,
			 * return success w/o setting directIO. If the hidden
			 * option is not enabled, return an error. Reserved
			 * files (and AdvFS special files such as the frag
			 * file, bitfile set tag directories, and quota files)
			 * are never opened for directIO. 3. If this is a file
			 * other than VREG, return an error. 4. If file is
			 * already open for mmapping, return error. 5. If file
			 * is open for atomic-write data logging, return
			 * error. Additional restrictions for CFS Concurrent
			 * DirectIO: 1. Fail opens of metadata files by .tags */
			if ((*vpp)->v_flag & VDIRECTIO) {
				...
				/* Metadata file; check if we open in cached
				 * mode or fail the open.  In no case do we
				 * set the VDIRECTIO bit for metadata files.
				 * This check also includes the AdvFS
				 * 'special' files. The blanket check for
				 * BFD_FTX_AGENT and BFD_FTX_AGENT_TEMPORARY
				 * currently allows data-logging files to be
				 * treated the same as the special files.
				 * Since we are only using the backdoor mount
				 * for internal testing, this is OK.  If we
				 * ever allow mounting a fileset for directIO
				 * to be shipped, then the above check will
				 * have to be restricted to our special files,
				 * but not include data logging files. */
				...
				/* Turn on the directIO flag and flush
				 * buffers. If enabling dio for CFS, then be
				 * sure that any storage allocation
				 * transactions are on disk; do this by
				 * flushing the log.  This is necessary
				 * because CFS DIO assumes that if the block
				 * is allocated to this file, it won't be
				 * removed on a subsequent crash and recovery. */
			 ...
			 * in flight has been removed.  This is dead code
			 * since O_CACHE is explicitly turned off above.  If
			 * this capability is ever re-enabled, consider using
			 * an active range that covers the entire file to
			 * block active directIO threads while the open mode
			 * is changed. */
...
msfs_getattr(
    ...
    register struct vattr * vap,/* out - vnode attribute structure */
    struct ucred * cred		/* in - callers credentials */
)
{
	bfAccessT *bfap;
    ...
    int *updateStatsA)
{
	...
	/* Seize an active range from the truncation point to the end of the
	 * file.  When this range is established, we have locked out any
	 * directIO writers to this range, which is what we want. */
	...
	/* Prevent races with migrate code paths bf_setup_truncation()
	 * requires migTruncLk locked. This lock must be locked before
	 * starting a root ftx. */
	/* can't be a clone */
	 ...
	 * DO need to take it later on in this function because of the
	 * msfs_chown call. */
	...
	if (fs_trunc_test(vp)) {
		if (!bfap->trunc) {
			...
			/* If there is a clone, storage wiil be xfer'd to the
			 * clone. */
	...
	case SETCACHEPOLICY:
		{
			...
			if (cmd == GETCACHEPOLICY) {
				*(int *) data = ((vp->v_flag & VDIRECTIO) ? CS_DIRECTIO : CS_CACHE);
				...
				/* Note: This SETCACHEPOLICY interface is
				 * provided solely for CFS. There's no way for
				 * a user level application to get here since
				 * we check for F_SETCACHEPOLICY in fcntl(),
				...
				if (mode == CS_DIRECTIO) {
					...
					/* The dead code for reverting to
					 * cached policy from directIO has
					 * been removed.  This code is not
					 * reached because of the 'break'
					 * above.  If this capablility is to
					 * be reactivated, consider using an
					 * active range for the entire file to
					 * block directIO threads while
...
#ifdef ADVFS_NO_ZERO_ALLOCATE
		/* special code for quickly allocating storage in test
		 * filesystems. storage is assigned and reused without being
		 * written or cleared. */
	case ADVFS_TESTSTORAGE:
		{
			 ...
			 * report the bytes allocated count rather than the
			 * retval. */
			if (retval) {
...
remove_it_1:
	 ...
	 * and we cannot remove a file from a clone. */
	KASSERT(bfap->bfSetp->cloneId == BS_BFSET_ORIG);
	/* Need index lock because of call chain remove_dir_ent -->
	 ...
	 * potential danger path is: rbf_delete --> chk_blk_quota -->dqsync
...
rename_it:
	...
	if ((sts != EOK) || ((VTOA(undel_dir_vp))->deleteWithClone)) {
		...
		if (sts != EOK) {
			/* most likely an i/o error; ignore it; update not
			 * critical */
		}
		goto remove_it;
	}
...

	{
		...
		/* Set up the following fields in the undel_ndp so that the
		 * call to insert_seq inside msfs_rename() will either create
		 * a namecache entry for this file, or, if a negative-cache
		 * entry is already present, it will be removed. */
 ...
 */
msfs_link(
    register struct vnode * vp,	/* in - vnode is existing file to link to */
...
)
{
	 ...
	 * there is an entry in the quota file already. The potential danger
	 * path is insert_seq --> chk_blk_quota --> dqsync --> rbf_add_stg but
	 * it is tame in this case. */
...
{
	struct vnode *from_vp = NULL, *from_dir_vp = NULL, *to_vp = NULL, *to_dir_vp = NULL,
	     *removed_to_vp = NULL;
	...
	fs_dir_entry *dir_buffer;
	char *p;
	bfTagT found_bs_tag, oldparent_tag, newparent_tag, keep_to_dir_tag,
	      *tagp;
	rbfPgRefHT page_ref;
	int type;
	...
	u_int effective_LINK_MAX;
	boolean_t to_dir_mig_trunc_lock = FALSE, to_file_context_locked = FALSE, from_dir_mig_trunc_lock = FALSE;
	bfTagT to_tag;
...
again:
	...
	if (tndp->ni_vp) {
		...
		/* Ensure that the source and destin are not the same.  This
		 * test is needed to solve a possible race condition between
		 * link and rename regarding the target file.  The target does
		 * not exist initially, but it is found the second time (see
		 * 'someone created the file on us' below).  Since the target
		 * was newly created, we did a goto 'again:'.  We now need to
		 * make sure this new file is not the same as the source file. */
	...
	if (tndp->ni_dirstamp != to_dir_context->dirstamp
		...
		if (ret == I_INSERT_HERE) {
			/* there is no output file - check this against the
			 * status when rename was called */
			if (to_vp != NULL) {
			...
			/* save # useful pages in nameidata for possible
			 * truncation */
			tndp->ni_endoff++;
		} else {
			if (ret == I_FILE_EXISTS) {
				if (to_vp == NULL) {
					...
					/* do another namei on it, and try
					 * again... */
			call_namei:
					...
					/* it's the same to-file, but it could
					 * have moved */
				}
			} else {
	...
	if (to_vp == NULL) {
		 ...
		 * idx_convert_dir-->idx_create_index_file_int --> rbf_create
		 * --> rbf_int_create --> tagdir_alloc_tag -->
		 * init_next_tag_page --> rbf_add_stg */
		MIGTRUNC_LOCK_READ(&(to_dir_accessp->bfSetp->dirBfAp->
			xtnts.migTruncLk));
		/* Need to take the migtrunc lock on the ``to'' directory
		 * index (if it exists), because of the call chain insert_seq
		 * --> idx_insert_filename --> idx_insert_filename_int -->
		 * idx_index_get_free_pgs_int --> rbf_add_stg */
		 ...
		 * because of the call chain remove_dir_ent -->
		 * idx_directory_insert_space -->
		 * idx_directory_insert_space_int -->
		 * idx_index_get_free_pgs_int --> rbf_add_stg Also, don't take
		 ...
		 * them at file creation, chown and explicit quota setting.
		 * rename cannot change the owner, so it does not add storage
		 * to the quota files. The potential danger path is insert_seq
		 * --> chk_bl_quota --> dqsync --> rbf_add_stg but it is tame
		 * in this case. */
		 ...
		 * add storage to the quota files. */
		ret = FTX_START_XID(FTA_OSF_RENAME_V1, &ftx_handle, FtxNilFtxH,
		    dmnP, 6, fndp->ni_xid);
		 ...
		 * pinning any records since there is a rare chance of needing
		 * to fail the ftx. */
	...
	/* finish any directory truncation that might have been started in
	 * insert_seq for the 'to' directory. Must be done outside the
...
)
{
	...
	/* Need to lock the index because of the call chain remove_dir_ent -->
	 * idx_directory_insert_space --> idx_directory_insert_space_int -->
	 * idx_index_get_free_pgs_int --> rbf_add_stg */
...
msfs_strategy(struct buf * bp)
{
	...
	/* Set bp's resid to requested transfer size. b_resid is set to zero
	 * in aio_rw(), but the difference between bp->b_bcount and
	 * bp->b_resid is reported as the number of bytes transferred by the
	 * aio interface routines.  So we set both to the same value here and
	 * adjust the b_resid value in the read/write routines as appropriate. */
	...
	if (vec[2].iov_len == 0) {
		 ...
		 * normal read()/write().  Otherwise b_error will be either an
		 * error retval or the b_error from the IO path. */
...
)
{
	...
	do {
		...
		int starting_byte;	/* starting byte within the page to
					 * read */
    ...
    struct ucred * cred)
{
	...
	if (vp->v_usecount > 1) {
		...
		/* TODO This routine has to be fixed. The field
		 * cp->dirty_stats is not protected here by a fsContex_mutex
		 * as it is elsewhere. If a mutex is taken before the if then
		 * one in the routine fs_update_stats must be removed or
		 * unlocked in coordination with this mutex else the routine
		 * might attempt to lock it again. */
...
get_name(
    ...
    char *buffer,		/* in - out, buffer addr to store file name
				 * string */
...
)
{
	...
	if (!dir_contextp) {
		 ...
		 * vnode still has (v_type == VREG) and no context structure.
		 * All we really want the context structure for is the
		 * file_lock to keep other threads from modifying the
		 * directory data while we read it. In this (rare) case,
		 * allocate a context structure and init minimal # of fields
		 * in the bfap, context, and vnode structures. */
		sts = fscontext_init(dir_bfap->bfVp, dir_bfap, &dir_contextp);
		if (sts != EOK) {
			RAISE_EXCEPTION(sts);
		}
		/* Not a full initialization.  Must be 0 for any later
		 * bf_get_l call. */
