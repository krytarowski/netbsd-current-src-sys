...
)
{
	...
	case BSR_XTNTS:
		...
		if (maxCnt) {
			 ...
			 * 3 bsXA records in its mcell.  The second half of
			 * the test checks for this. Note that this test will
			 * allow a normal file in a Version 4 domain whose
...
)
{
	if (subXtntMap->bsXA != NULL) {
		ms_free(subXtntMap->bsXA);
		subXtntMap->bsXA = NULL;
		subXtntMap->cnt = 0;	/* FIX - should I leave the count
					 * alone? */
	}
	return;
		...
		if ((copyBsXA[src].vdBlk != XTNT_TERM &&
			...
			if (needCnt > (subXtntMap->maxCnt - dst)) {
				...
				if ((sts == EXTND_FAILURE) ||
					...
					/* We need to be careful here not to
					 * end the subxtnt map with a hole.
					 * Check the next to last descriptor
					 * and if it is a hole move it to the
					 * beginning of the newly obtained
					 * descriptor. NOTE we DO allow *
					 * subxtnts to end with a permenant
					 * hole. */

					if (dst > 0 && subXtntMap->bsXA[dst - 1].vdBlk == XTNT_TERM) {
						/* The hole now becomes the
...
imm_get_first_hole(
    bsInMemXtntT * xtnts,	/* in */
    uint32_t * retPageOffset,	/* out */
    uint32_t * retPageCnt	/* out */
)
{
...
)
{
	...
	if (!updateFlg) {
		lwr = 0;
		upr = subXtntMap->cnt - 1 - 1;	/* Exclude the termination
						 * descriptor */
    ...
    ftxHT ftxH)
{
	...
	uint32_t firstOverlayPage;
	uint32_t lastOverlayPage;
	uint32_t moveCnt = 0;
	int i;
	bfMCIdT throwAwayMC;
	...
	/* The most sub extents we ever need is all of the base,
	 * baseXtntMap->cnt. */
	...
	/* The most subextents we need is the number in the base - 1 (the
	 * overlay */
	/* map overlaps at least one of the base subxtnts) + 2 (two new
	 * subxtnts */
...
*/
	...
	for (modSXMapCnt = 0, baseSXMapCnt = 0;
		...
		/* Currently, the first base mcell says we have
		 * baseXtntMap->cnt */
		/* mcells. We may need to change this when we are done. */
		/* Save this here to avoid reading the first base mcell at the
		 * end */
		/* of this routine.  */
		mcellCnt = baseXtntMap->cnt;
...
*/
		...
		/* Find the boundary within a subextent in baseXtntMap of the
		 * first */
		/* page in the overlay. Copy the first part to modXtntMap. If
		 * the */
		/* boundary does not lie on an extent boundary, break the
		 * extent in */
...
*/
		...
		if (baseXtntMap->subXtntMap[baseSXMapCnt].type == BSR_XTNTS) {
			...
			/* Disconnect the mcell that is being split (and all
			 * that follow) */
			...
			if (baseSXMapCnt > 0) {
				...
				/* No mcells were copied in step 1. Case III -
				 * VIII */
			...
			if (modSXMapCnt > 0) {
				...
				if (stripeIndex) {
					/* The file is striped and this is not
					 * the first stripe. */
					/* Attach the mod extents to the
					 * previous stripe. */
...
*/
		...
		/* Continue processing the same baseXtntMap subextent step 2
		 * worked */
		/* on. Use descIndex and part2BsXA from step 2 to know where
		 * to start. */
		...
		/* otherwise we are still processing this base extent and we
		 * need */
		...
		if (lastOverlayPage + 1 >= nextPg) {
			...
			replSXMapCnt = 1;
			baseSXMapCnt++;
			moveCnt = 0;	/* step 7 will start on a new
					 * subextent */
		} else {
			/* Case III - V */
			/* Note that by not incrementing baseSXMapCnt and not
			 * setting */
			/* replSXMapCnt we will still be working on the same
			 * "base" */
			/* subextent and "replace" subextent in step 6 if no
			 * subextents */
			/* are processed in step 5. */
			moveCnt = descIndex;	/* save this descriptor index
						 * for step 7. */
		}
...
*/
		...
		/* If we're finished processing this subextent we need to get
		 * a new */
		/* mcell and free the old one otherwise we'll do all that in
		 * step 6a. */
		if (replSXMapCnt == 1) {
			/* Case I, II, VI - VIII */
			sts = stg_alloc_new_mcell(bfap,
	...
	}			/* end of "if start of overlay divides a base
				 * mcell" */
	/*
	...
	*/
	if (modSXMapCnt > 0) {
		...
		/* There already is some mcells in the modified extent chain
		 * from */
		/* step 1 and/or 2. Save the end of the modified extent chain
		 * to link */
		...
		if (overXtntMap->subXtntMap[0].type == BSR_SHADOW_XTNTS) {
			...
			/* free the BSR_SHADOW_XTNTS mcell at the end of the
			 * transaction */
			...
			/* Change the in memory subextent type to
			 * BSR_XTRA_XTNTS. */
			...
			if (overXtntMap->cnt > 1) {
				/* If there were BSR_XTRA_XTNTS mcells
				 * unlinked from the */
				/* BSR_SHADOW_XTNTS mcell above, link new
				 * BSR_XTRA_XTNTS mcell */
		...
		}		/* If first overlay subextent is
				 * BSR_SHADOW_XTNTS */
	}			/* if ( modSXMapCnt > 0 ) */
	if (mcellCnt == 0) {
		/* Currently the first mcell (the first one in the overlay
		 * chain) */
		/* says we have overXtntMap->cnt mcells. At the end of this
		 * routine */
	...
	/* Handles base maps with more than just 2 mcells. Also now handles
	 * base map with a different page count than the overlay page count. */
	...
	/* overlay contains only one extent, base only contains TERM */
		...
		/* Handle the special cases where there is an extent in the
		 * primary mcell of the base already AND: 1. overlay extent is
		 * physically contiguous on disk to the only extent in the
		 * primary mcell of the base map. - make a single extent in
		 * the primary mcell describing both extents. -OR- 2. overlay
		 * extent's offset is equal to the offset of the base's
		 * primary mcell offset. - make the overlay extent reside in
		 * the primary mcell. */
		if ((FIRST_XTNT_IN_PRIM_MCELL(dmnVersion, bfap->xtnts.type)) &&
		    (overXtntMap->subXtntMap[0].vdIndex ==
			baseXtntMap->subXtntMap[0].vdIndex) &&
		 ...
		 * because we would have to change the tag dir file if we were
		 * to start assigning primary mcell ids to files!! */
		modSubXtntMapp->type = BSR_XTNTS;
		modSubXtntMapp->maxCnt = BMT_XTNTS;
		modSubXtntMapp->vdIndex = baseSubXtntMapp->vdIndex;
		/* Set the type and mcell for modXtntMap slot 0 to be the
		 * primary mcell.  In other words, set the mod mcell = base
		 * mcell so we reuse the prim mcell on disk. */
		modSubXtntMapp->mcellId = baseSubXtntMapp->mcellId;
		modSubXtntMapp->onDiskMaxCnt = baseSubXtntMapp->onDiskMaxCnt;

		if (overXtntMap->subXtntMap[0].pageOffset ==
		    baseXtntMap->subXtntMap[0].pageOffset) {
			/* Overlay extent is NOT contiguous to first extent in
			 * base. Overlay extent will replace prim mcell extent
			 * at least partially
			 * 
			 * imm_copy_sub_xtnt_map above updated the new mod mcell
			 * (which got the id from the old base prim mcell)
			 * with the overlay's extent. This results in old base
			 * primary mcell with new extent from overlay. */

			/* set the baseSXMapCnt to the primary mcell so step 5
			 * will delete the overwritten extent out of the
			 * primary mcell step 5 correctly deals with an extent
			 * in the primary mcell. */
			baseSXMapCnt = 0;
		} else {
			/* Overlay extent is contiguous with the extent in the
			 * PM base. Combine the extents from the base prim
			 * mcell and the overlay mcell and place the single
			 * new extent into the new mod. */
			...
			/* set the base cnt to the next mcell so step 5 does
			 * not try delete the extent out of the old base
			 * primary mcell since we just appended it here. */
			baseSXMapCnt = 1;
		}
		...
		/* v3-complete base was already copied to mod in step 1 above */
			...
			((modSXMapCnt >= 2) &&	/* v4 & non-primary mcells
						 * only */
		/* current mod must be in BSR_XTRA_XTNTS mcells */
			    (modXtntMap->validCnt >= 2))) &&

		    (prevXtnt > 0) &&	/* another non-primary mcells only
					 * check */
		    (overSXMapCnt == 0) &&	/* first run through only */
		    (overXtntMap->validCnt == 1) &&	/* overlay must have 1
							 * mcell only */
		...
		/* Overlay extent is only appended to the end of a wholly
		 * copied base mcell's contents, NOT inserted into middle
		 * resulting in a split mcell. To do the middle we would need
		 * to change this routine to splice the new overlay extent
		 * into the mod mcell, possibly overlying and cutting into
		 * subsequent extents in this mcell, or even possibly spilling
		 * over into a next mcell. Check here is if base was not fully
		 * copied in step 1, above, then the baseSXMapCnt will still
		 * be pointing to the previous mcell with its page offset, and
		 * the base offset and mod offset will not match. pageCnt
		 * comparison is also necessary in case the previous mcell is
		 * the primary mcell and its offset is 0 just as this mcells. */
		...
		/* update the mod mcell header with the new start and end
		 * pages */
...
*/
	...
	for (; baseSXMapCnt < baseXtntMap->cnt; baseSXMapCnt++, replSXMapCnt++) {
		...
		if (baseSubXtntMapp->type == BSR_XTNTS) {
			...
			KASSERT(replSXMapCnt == 0);
				 ...
				 * baseXtntMap to replXtntMap */

	/* If we step 3 produced an mcell and step 5 processed some
	 * subextents, */
	...
	if (baseSXMapCnt == baseXtntMap->cnt) {
		/* Case V & VIII */
		/* Note that in step 3 if we did not finish processing the
		 * base */
		/* subextent, baseSXMapCnt was not incremented. Therefore,
		 * case III, */
		/* for example, will not get here. */
		goto done;
	}
	/* check if overlay lines up on subxtnt boundary. if so skip step 6 &
	 * 7 */
	baseSubXtntMapp = &baseXtntMap->subXtntMap[baseSXMapCnt];
	nextPg = baseSubXtntMapp->pageOffset + baseSubXtntMapp->pageCnt;
	if (lastOverlayPage + 1 != nextPg) {
...
*/
		 ...
		 * to copy */
		/* the subextnt map. replSubXtntMapp->cnt will be == 0 in case
		 * IV */
		...
		/* Find the boundary in baseXtntMap of the last page in the
		 * overlay. */
		/* Copy the first part to replXtntMap. If the boundary does
		 * not lie */
		/* on an extent boundary, break the extent in two. Save
		 * descIndex */
...
*/
		...
		/* If there are any mcells from step 3 or 5, link this mcell
		 * to them. */
		if (replSXMapCnt > 0) {
			/* Case I, VI, VIII */
			sts = bmt_link_mcells(bfap->dmnP,
...
*/
		...
		/* If the overlay is entirely within a baseXtntMap subextent
		 * then */
		/* moveCnt defines how many extents were processed in step 3. */
		/* descIndex defines how many extents were processed in step
		 * 6. */
		/* The sum defines the extent to begin processing in this
		 * step. */
		descIndex += moveCnt;
		...
		/* Continue processing the same baseXtntMap subextent step 6
		 * worked */
		/* on. Use descIndex and part2BsXA from step 6 to know where
		 * to start. */
		modXtntMap->cnt = modXtntMap->validCnt = modSXMapCnt + 1;
...
*/
		...
		/* If this is copied from the first base subextent (case III &
		 * IV), */
		/* the type will be BSR_SHADOW_XTNTS. Since this is not the
		 * first */
		/* new subextent map (at least, step 4 copied some), we must
		 * change */
		/* the type to BSR_XTRA_XTNTS before initializing the new
		 * mcell. */
		...
		baseSXMapCnt++;
		modSXMapCnt++;
	}			/* end of "if end of overlay extents divide a
				 * base mcell" */
	/*
...
)
{
	...
	/* If overlay map contains a fragment only (and nothing else) - return
	 * base. This condition arises when only the fragment in the original
	 * is modified and the original only had a fragment or a single block
	...
	/* If base map contains a fragment only (and nothing else) - return
	 * overlay. Condition arises when the original is truncated into the
	 * first block. */
	if ((baseDesc.pageCnt == 0) &&
...
)
{
	...
	if (sts == EOK) {
		...
		if (pageOffset == subXtntMap->bsXA[descStart].bsPage) {
			...
			if (descStart > 0) {
				...
				if (blk == XTNT_TERM || blk == PERM_HOLE_START) {
					...
					/* Now check if the subxtnt map prior
					 * to us is the primary and pick it up
					 * of we can. */
				...
				/* Removing storage from first extent in
				 * subextent map. */
				while (xtntMap->origStart > 0) {
					...
					if (subXtntMap->cnt > 1) {
						...
						/* subXtntMap->cnt == 1 */
						/* The only 1 entry subxtnt
						 * allowed is the primary. We
						 * are removing storage from
						 * the begining of the file so
						 * include this in the update
						 * range in hopes that we can
						 * reuse it.
						 * 
						 * OR
						 * Version 3 domains may have
						 * left over on-disk
						 * corruption from older
						 * kernels. This seems to show
						 * itself as orphan extents
						 * (each with the same
						 * starting page) ultimately
						 * preceeded by an extent made
						 * up of only a hole. Clean
						 * this mess up by including
		...
		if (sts == E_PAGE_HOLE) {
			...
			if ((descStart == 0) && (xtntMap->origStart == 1)) {
				subXtntMap = &(xtntMap->subXtntMap[0]);
				if (subXtntMap->cnt == 1) {
					/* The only 1 entry subxtnt allowed is
					 * the primary. We are removing
					 * storage from the begining for the
					 * file so include this in the update
		...
		if (endPageOffset == (subXtntMap->bsXA[descEnd + 1].bsPage - 1)) {
			...
			if (descEnd != (subXtntMap->cnt - 2)) {
				...
				if (xtntMap->origEnd != (xtntMap->cnt - 1)) {
					...
					if (subXtntMap->cnt > 1) {
						/* We are about to create a
						 * hole at the end of a sub
						 * extent map. The rule is
						 * that hole must stick to the
						 * storage in front of it. In
						 * other words the subextent
						 * map in front of us must be
						 * modified.  */
	...
	/* Check to see if the range we are removing starts in the middle of
	 * the first subextent or if we are removing all the pages. */
...
imm_compress_subxtnt_map(bsInMemXtntT * xtnts, int bfPageSz)
{
	...
	for (subindx = xtntMap->updateStart;
		...
		for (i = 0, j = 1; i + 1 < cnt; i += k, j++) {
			/* find two regions logically and physically
			 * contiguous */
			for (k = 1;
			    i + k < cnt &&
...
)
{
	...
	int not_sparse = 0;
	uint32_t pages = 0;
	uint32_t maxClonePgs = bfap->file_size / ADVFS_PGSZ + 1;
	...
	if (not_sparse) {
		...
		/* This is not sparse file so we have only one extent with
		 * size equal to file size */
	...
	while (nextPage != XTNT_TERM && nextPage != PERM_HOLE_START) {
		...
		if (!cloned_file) {
			...
			/* Clone bfap has no extents and clone
			 * bfap->maxClonePgs */
			/* has not been set. So here we use the local
			 * maxClonePgs. */
			if (pg < maxClonePgs) {
				/* Don't look at orig file extents past
				 * maxClonePgs. */
				ret = page_is_mapped(orig_bfap, pg, &nextPage, FALSE);
				/* In case of striped files when we reach end
				 * of segment nextPage will be set to
				 * -1(XTNT_TERM). So don't set nextPage to
				 * maxClonePgs when we reach end of segment. */
				if (nextPage != XTNT_TERM && nextPage > maxClonePgs) {
					/* Don't allow nextPage to progress
					 * beyond maxClonePgs. */
			...
			/* when the file is cloned we look at the pages of
			 * both the original and clone to get the sparseness
			 * map. For the clone file look in the clone first. If
			 * the page is mapped to storage OR a permanent hole
			 * then use nextPage to find the end of the extent. If
			 * the page is mapped to a normal hole in the clone
			 * use nextPage to save the end of the hole in the
			 * clone's map. If the clone does not map the page
			 * (normal hole) then look in the original file's
			 * extent map. Look only up to the end of the hole in
			 * the clone's extent map. Never go beyond
			 * maxClonePages since that was the size of the
			 * original file when the fileset was cloned. */
			...
			if (ret == FALSE) {	/* hole or permanent hole or
				...
				if (nextPage == XTNT_TERM) {	/* pg is off the end of
								 * file */
					/* see comment below about stripe
					 * files */
				...
				if (ret == FALSE) {	/* its normal hole, look
							 * at original */
					ret = page_is_mapped(orig_bfap, pg, &nextPage, FALSE);
					...
					/* Permanent hole, so consider as hole
					 * in clone */
					ret = FALSE;
				}
			}
		 ...
		 * In case of striped files this may not be true. The function
		 * page_is_mapped() sets the nextPage argument to -1 when it
		 * hits the end of extent map for the file on any one of the
		 * segment when its is striped across multiple segments. So in
		 * case of striped files we will jump to next segment when we
		 * hit the end of extent map in one segment and we do that
		 * till we go through all the pages for the file. */
		 ...
		 * hole, this is the case in case of striped files we should
		 * not treat this as a hole again, it should be treated as the
		 * continuation of the previous hole and hence new entry
		 * should not be created in the sparseness map. */
	...
	if (i == 0) {
		...
		/* we found at least one extent and must adjust the final
		 * extent for a fragment and for file_size so decrement the
		 * value of i. */
		i--;
		...
		if (bfap->fragState == FS_FRAG_VALID) {
			if ((long) bfap->fragPageOffset * ADVFS_PGSZ == (offset + size)) {
				...
				/* there is a hole before the frag so add a
				 * new extent */
				i++;
				offset = (long) bfap->fragPageOffset * ADVFS_PGSZ;
				size = bfap->fragId.type * BF_FRAG_SLOT_BYTES;
