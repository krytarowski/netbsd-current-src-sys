...
)
{
	...
	/* FIX - before we can migrate the root tag directory, must make a
	 * change in domain activation so that the root tag directory is
	 * opened between pass1 and pass2.  Recovery should open and close it.
	 * Domain activation should open it after recovery. */
...
)
{
	...
	/* If the file is  being created, and it is either a directory or a
	 * "slow" symlink, then we might be racing with the file creation code
	 * while it is adding storage. Synchronize with it by waiting for the
	 * bfState to become valid. */
	...
	case BSXMT_APPEND:
		...
		if (srcVdIndex == (vdIndexT) (-1)) {
			/* moves all the pages in page range on all disk
			 * volumes if dstVdIndex==-1, allocate space on
			 * service class, otherwise on dstVdIndex only. */
			sts = migrate_normal(
			    srcBfap,
			    srcPageOffset,
			    srcPageCnt,
			...
			/* migrate_normal_one_disk moves only the pages for
			 * given srcVdIndex. if dstVdIndex==-1, allocate space
			 * on service class, otherwise on dstVdIndex only.
			 * Calls migrate_normal. */
...
pre_reset_block_map_special(struct bfAccess * bfap)
{
	 ...
	 * bfap. reset_block_map_special is used for remapping cached clone
	 * pages that are still mapped by the original file. (non-COWed pages)
	 * The pageRange is all pages in the file, page 0 through n where n is
	 * the total number of pages that make up the clone. */
...
)
{
	...
	/* Allocate an active range structure; this synchronizes migrate with
	 * local directIO and msfs_cfs_flush_and_invalidate. In the past it
	 * was stated the CFS token mechanism prevented either from occuring,
	 * however, corruption from CFS page invalidation while a migrate is
	 * running has proven that to be unreliable so now we always lock the
	 * backdoor ourselves. */
	...
	/* Put this active range onto the list to block directIO threads in
	 * the same range.  This also prevents truncation/invalidation. */
	...
	/*
		...
		if (pageRange[i].pageType == STG) {
			sts2 = reset_block_map(bfap,
			    ...
			    pageRange[i].pageCnt);
			if (sts2 != EOK) {
				/* We can't deallocate the copy's storage
				 * because there may be some bitfile pages
				 * still in the cache that map both the
				 * original's and the copy's pages. The copy's
				 * storage is still on the delete pending list
				 * and is deallocated during the next system
				 * boot.  */
				remCopyStgFlag = 0;
			}
		}
...
HANDLE_EXCEPTION:
	...
	if (do_cluster_cleanup) {
		 ...
		 * which migrate brought in, since another cluster node could
		 * be performing i/o directly to disk. This would bypass the
		 * cache checking which is normally done for advfs directio on
		 * a single system. All pages should already have been flushed
		 * to storage by cp_copy_page_range() so we don't need to do
		 * that. */

		if (bfap->bfVp->v_flag & VDIRECTIO) {

			/* 1. Files open for direct IO need to invalidate the
			 * UBC pages used by migrate to avoid another node
			 * writing to the disk followed by this node reading
			 * stale data from the UBC page. 2. No direct IO
			 * writer will dirty the page between the flush and
			 * the invalidate since direct IO uses the extent map
			 * token (aka 'read token') that is released below and
			 * we have the active range for these pages which is
			 * also released below. NOT a problem for a clone
			 * being migrated (read only)? */
...
)
{
	...
	if (xmPageOffset != 0 && xmPageOffset == xtntDesc.pageOffset) {
		/* The requested starting page is the first page in an extent
		 * and */
		/* it is not the first extent. If the previous extent is a
		 * hole it */
		/* must be migrated with the hole. */
		imm_get_xtnt_desc(xmPageOffset - 1, xtntMap, 0,
		    &hXtntDescId, &hXtntDesc);
		if (xtntDesc.pageCnt == 0) {
			/* This can't be. The next page is mapped. We don't
			 * allow */
	...
	/* We need to catch if this is a request to just migrate a hole. We
	 * must deal with this, especially in cases where we may be removing a
	 * volume. Not allowing a hole to migrate could cause the rmvol to
	 * fail. */
	...
	if ((xtntDesc.blkOffset == XTNT_TERM) ||
		...
		if (endXtntDesc.pageOffset == xtntDesc.pageOffset) {
			...
			*xmPageRangeCnt = 0;
			...
			if (xtntDesc.blkOffset == PERM_HOLE_START) {
				 ...
				 * hole. Return the range in hand and the
				 * caller needs to setup the vd index */

				*xmHoleVdIndex = bsNilVdIndex;
			} else {
				/* Make sure this hole sticks to the storage
				 * that follows. */
				/*
				...
				if (hXtntDesc.pageCnt == 0) {
					/* This can't be. The next page is
					 * mapped. */
					/* We don't allow xtnt maps to end
					 * with a hole. */
					domain_panic(xtntMap->domain,
		...
		if (xtntDesc.blkOffset == XTNT_TERM) {
			/* If there is a leading hole (not permanent) then we
			 * need to inform the caller so that it will stick to
			 * the storage that follows it */
			xmHoleRange->pageCnt = xtntDesc.pageCnt;
			xmHoleRange->pageOffset = xtntDesc.pageOffset;
			xmHoleRange->pageType = XTNT_TERM;
			/* Make sure this hole sticks to the storage that
			 * follows. */
			hXtntDescId = xtntDescId;
			imm_get_next_xtnt_desc(xtntMap, &hXtntDescId, &hXtntDesc);
			if (hXtntDesc.pageCnt == 0) {
				/* This can't be. The next page is mapped. */
				/* We don't allow xtnt maps to end with a
				 * hole. */
				domain_panic(xtntMap->domain,
				    "get_xm_page_range_info: imm_get_next_xtnt_desc failed");
				return E_PAGE_NOT_MAPPED;
	...
	/* Adjust the starting extent range to be the requested start. In case
	 * we are asked to migrate a partial range. If we are starting with a
	 * permanent hole then move the entire hole. If it is a normal hole
	...
	while (!last_extent) {
		...
		case XTNT_TERM:
			...
			/* We hit a normal hole. End the page range and start
			 * a new page range. If the current pageRange[cnt] is
			...
			if (last_extent) {
				 ...
				 * file by reducing out range. Do it again */
				if (pageRange[cnt].pageType == NOSTG) {
					/* This is actually a more than two
					 * holes in a row case. just remove
					 * the last extent from the page range
					 * array */
					cnt--;
				}
		...
		case PERM_HOLE_START:
			/* This is a permanent hole. End the current range,
			 * add this hole to the range and start a new range. */
			...
			if ((pageRange[cnt].pageType == NOSTG) &&
				...
				/* One could argue that this should be an
				 * assertion. This deals with two consecutive
				 * permanent holes. we just glom them
				 * together. I don't beleive we should ever be
				 * in this situation, but we might as well
				 * handle it. */

				pageRange[cnt - 1].pageCnt += xtntDesc.pageCnt;

				/* Unlike a normal hole which is always
				 * (usually ?) associated with the backing
				 * store of the extent that follows it,
				 * permanent holes can be explicity migrated.
				 * We will impose that the entire hole be
				 * moved. Partial migration of a permanent
				 * hole is not allowed.  */
		...
		default:
			/* We get here if we are a not a hole. Extend the
			 * existing range or start a new one. */

			if (pageRange[cnt].pageType == NOSTG) {
				pageRange[cnt].pageOffset = xtntDesc.pageOffset;

				/* Add the last extent into our page range
				 * array. Truncate the extent if the requested
				 * range does not fully overlap.  */
	...
	if (FIRST_XTNT_IN_PRIM_MCELL(domain->dmnVersion, bfap->xtnts.type)) {
		...
		lock_write(&(bfap->xtntMap_lk));
			...
			/* Do the same for the clone, if necessary. */
				...
				xtntMapp->hdrMcellId.cell =
				    xtntMapp->subXtntMap[0].mcellId.cell = newMcellId.cell;
				lock_done(&(cloneBfap->xtntMap_lk));
			}
		} else {
			lock_done(&(bfap->xtntMap_lk));
		}
...
)
{
	...
	if (vdIndex == (vdIndexT) (-1)) {
		/* We need an MCELL on a disk in the service class so pick one */
...
)
{
	...
	*newcRangeBeginBlk = 0;
	...
	if (sbm_bit_cnt) {
		...
		/* loop, repeatedly scanning the bmt and rbmt, finding those
		 * extents that are lying in the clear range passed in. */
		while (another_scan_required) {

			/* V5 ONDISK FIX -> call sbm_get_vd_bf_inway for new
			 * v5 sbm instead of bmt_get_vd_bf_inway */

			/* find tags and pages for all files in range to be
			 * packed */
			bmt_bit_cnt = 0;
			...
			if ((vdp->ssVolInfo.ssPackHdr.ssPackListCnt > 0) &&
				...
				/* pick up next loop where the list leaves off
				 * this loop */
			...
			while ((php->ssPackListCnt != 0) &&
				...
				if (sts == ENO_SUCH_TAG) {
					...
					lock_write(&(vdp->ddlActiveLk));
					    ...
					    &mcellId,
					    &delFlag);
					lock_done(&(vdp->ddlActiveLk));
					    if (sts == EOK) {
						/* found on ddl, may be there
						 * a long time, abort */
						sts = ENO_MORE_MCELLS;
						goto _cleanup;
				...
				case BSXMT_APPEND:
					...
					lock_done(&(inwayBfap->xtntMap_lk));
					    if (pXtntp->ssPackPageCnt == 0) {
						break;	/* out of switch, get
							 * next pXtnt */
					}
					...
					while (migPageCntSoFar < newPageCnt) {
						 ...
						 * be migrated are equal to
						 * the number of free blocks
						 * between the last
						 * destination blk offset and
						 * the start of this extent.
						 * This is allowed because the
						 * extents are ordered by
						 * block number in the pack
						 * list and it is operated on
						 * in the same order - from
						 * left to right */
						...
						/* pack the number allowed
						 * from above or the number
						 * remaining */
						migPageCnt = MIN(migPageCnt, pgsRemaining);
						/* pack only the number
						...
						/* pages migrated since last
						 * block. */
						thdblkMigPageCnt += migPageCnt;
						/* new page offset */
						xmPageOffset += migPageCnt;

						/* pages migrated so far for
						 * xtnt entry */
				...
				case BSXMT_STRIPE:
					...
					for (k = 0, skip = FALSE; k < bfPageRangeCnt && !skip; k++) {
						...
						/* Now, migrate the pages to
						 * the left, thus packing them
						 * together. */
						while (migPageCntSoFar < newPageCnt) {
							 ...
							 * the extent */
					}	/* end for each stripe segment
						 * in extent found inway. */
