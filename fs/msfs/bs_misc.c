...
find_cfs_server(
    ...
    libParamsT * buf,		/* in/out - pointer to syscall-specific data
				 * structure */
    ServerSearchKeyT key,	/* in - search key for cfs server */
    int *do_local,		/* out - flag to show if op should be carried
				 * out locally */
...
static int
open_set(
    mode_t setMode,		/* in - access mode (read, write, etc) for the
				 * set */
    mode_t dmnMode,		/* in - access mode for the set's domain */
    int setOwner,		/* in - is only the owner allowed access to
				 * set */
    int dmnOwner,		/* in - is only the owner allowed access to
				 * the dmn */
...
void 
msfs_audit_syscall(register libParamsT * libBufp,
    opIndexT opIndex,
    int error);
/*
 * msfs_real_syscall
...
)
{
	...
	/* obtain an exlusive access DLM kernel lock so the mount struct
	 * pointers won't get unmounted while we process the syscalls.
	 * 
	 * This lock also serializes the syscalls clusterwide and prevents a
	 * planned relocation from happening while a syscall is being
	 * processed.
	 * 
...
RETRY:				/* CFS may retry ADD_VOL and REMOVE_VOL */
	...
	case ADVFS_GET_IDX_BF_PARAMS:{
			...
			/* Fall through to ADVFS_GET_BF_PARAMS; based on
			 * OpType,   */
			/* msfs_syscall_op_get_bf_params() will know what to
			 * do */
	...
	case ADVFS_GET_DMN_VOL_LIST:{
			...
			if (libBufp->getDmnVolList.volIndexArray) {
				if (do_local)
					...
					/* if the call was function shipped,
					 * then xdr allocated the memory for
					 * us */
	...
	case ADVFS_ADD_VOLUME:{
			...
			/* Remote addvol is not supported; this call is to
			 * ensure that */
			/* the domain is local, not to function-ship the
			 * operation */
			if (clu_is_ready())
				sts = find_cfs_server(opIndex, libBufp, DMN_NAME,
				    &do_local, &have_domain_lock,
	...
	case ADVFS_REM_VOLUME:{
			...
			/* Remote rmvol is not supported; this call is to
			 * ensure that */
			/* the domain is local, not to function-ship the
			 * operation */
			if (clu_is_ready())
	...
	if (sts != EOK) {
		/* ADD and REM VOLUME may have to retry if a node crashes.
		 * Check error code here.  Release any DLM locks if retrying. */
    ...
    struct bfAccess * bfap)
{
	...
	if (opIndex == ADVFS_GET_IDX_BF_PARAMS) {
		/* The call is actually to get the bf params for the index
		 * file of the passed in directory. Fool this call by
		 * replacing the bfap for the directory with the bfap of the
		 * index. */
	...
	/* bfattrp points to stuff we copied in from the user. no real concern
	 * on object reuse, since things we don't touch retain the
	 * user-supplied values upon copyout below. */
	...
	/* Note: the union (bfattrp->attr) is not always completely filled in
	 * here. */
...
msfs_syscall_op_get_vol_params(libParamsT * libBufp, opIndexT opIndex)
{
	...
	/*
         ...
         */
	if (opIndex == ADVFS_GET_VOL_PARAMS2)
		*flagp |= GETVOLPARAMS2;
	...
	/* these cover the entirety of *vinfop If GETVOLPARAMS_NO_BMT is
	 * specified, the bsVdAttr struct (vdparamsp) will not get loaded with
	 * bmt data. */
...
msfs_syscall_op_get_bf_xtnt_map(libParamsT * libBufp, struct bfAccess * bfap)
{
	...
	if ((bfap->bfSetp->cloneId != BS_BFSET_ORIG) &&
		...
		/* This is a clone that has not had any metadata (or file
		 * pages) COWed. At open time the original's extents were
		 * loaded into it's (the clone's) in memory extent map. This
		 * is basically a bug (to be addressed at some future time).
		 * We can not trust these extents since the original could
		 * have been migrated (which does not cause a COW). So we need
...
msfs_syscall_op_get_cludio_xtnt_map(libParamsT * libBufp, struct bfAccess * bfap)
{
	...
	/* This will do the right thing whether we are operating on the
	 * original file or a clone. */
	...
	/* We need to protect against a volume being removed and referencing
	 * bad data in the vd structure. CFS is holding the xtnt map token on
	 * the file who's extents we just obtained. If we race rmvol, either
	 * 
	 * 1. rmvol is waiting to get the xtnt map token so it can migrate this
	 * file off the volume,
	 * 
	 * 2. Rmvol already moved this file off the volume so even if we obtain
	 * vd info before rmvol finishes, we know that none of the extents in
	 * this file live on the vd being removed. 3. This file does not live
	 * on the vd being removed so neither do its extents.
	 * 
	 * So, the only thing we need to protect against is a bad dereference of
	 * the vd structure. The vdpTblLock protects the vdp from going away
...
#ifdef ESS
	/* We have to check that the domain we select to panic is not * an
	 * advfs root domain. If the root fs is an advfs and its * domain ID
	 * matches the domain ID specified in dmnParams, then * we will not
	 * execute ESS domain_panic event. */
	if (clu_is_ready()) {
		struct mount *pmp = NULL;
	...
	if (clu_is_ready()) {
		...
		BFSETTBL_LOCK_READ(bfSetp->dmnP)
			...
			if ((fsnp->blkHLimit > mlBfSetParamsp->blkHLimit) ||
				...
				pmp = fsnp->mountp;
				/* To prevent a race with unmount, try to get
				 * unmount lock.  failed means that umount is
				 * happening, then don't bother flushing. */
				if ((pmp != NULL) &&
...
msfs_syscall_op_tag_stat(libParamsT * libBufp)
{
	...
	while (1) {
		...
		/* Need to open the index file if this is a dir so we can add
		 * in the allocated block count in bs_get_bf_page_cnt(). */
...
msfs_syscall_op_add_rem_vol_svc_class(libParamsT * libBufp)
{
	...
	/* CFS TODO: check if cfs_domain_change_vol() needs to be called here. */
	...
	/* CFS TODO: if cfs_domain_change_vol() is called earlier, should call
...
msfs_syscall_op_switch_root_tagdir(libParamsT * libBufp)
{
	...
	/* CFS TODO: should check if the cfs_domain_change_vol() need to be
	 * called here. */
    ...
    dlm_lkid_t * cmsdb_sublock)
{
	...
	int sts, fd = -1, len;
	char *dmn_name = NULL, *file_path, name_buf[BS_DOMAIN_NAME_SZ + 1];
...
retry:
	...
	case DMN_NAME:
		...
		case ADVFS_FSET_CLONE:
			...
			if (versw_val == VERSW_ENABLE) {
				/* if CMS proposal issued, cmsdb lock will be
				 * used then */
		...
		case ADVFS_ADD_VOLUME:	/* Not supported remotely */
			...
			if (versw_val == VERSW_ENABLE) {
				/* cluster root must use exclusive global dlm
				 * lock */
				if (!strcmp(dmn_name, "cluster_root"))
					get_domain_lock = 0;
				else {
					/* if CMS proposal issued, cmsdb lock
					 * will be used then */
					get_domain_lock = get_domain_excl = 1;
				}
			} else
	...
	case DMN_ID:
		...
		case ADVFS_REM_VOLUME:	/* Not supported remotely */
			...
			if (versw_val == VERSW_ENABLE) {
				/* when CMS proposal issued, cmsdb lock will
				 * be used then */
				get_domain_lock = get_domain_excl = 1;
			} else
				get_domain_lock = 0;
		...
		/* If mp is not found below, we want to take cmsdb lock
		 * exclusively. If mp is found below, we will release cmsdb
		 * lock soon anyway. Therefor, we always take it exclusively
		 * to avoid having to retry. */
		CC_CMSDB_DLM_LOCK(DLM_EXMODE, cmsdb_sublock);
		*have_cmsdb_lock = 1;
		have_cmsdb_excl = 1;
		mp = CC_DMN_ID_TO_MP(&dmn_id);

		/* special handling for ADVFS_REM_VOLUME on cluster root : we
		 * want to use exclusive global dlm lock */
	...
	if (mp) {		/* domain/file is known to cluster, i.e.,
				 * mounted */
		if (get_domain_lock) {
			if (!dmn_name) {
				dmn_name = name_buf;
		...
		if (versw_val == VERSW_ENABLE) {
			if (key == DMN_NAME) {
				if (dmn_name && *dmn_name) {
					/* Try to take domain lock excl to
					 * avoid concurrent domain activations
					 * in the cluster. */
					...
					if (!sts) {
						...
						*have_cmsdb_lock = 0;
						 ...
						 * paniked for other errors */
					}
				}
			} else {
				/* If took cmsdb shared, must release and
				 * re-take it Not expected to happen since
				 * DMN_ID types always take the lock excl.,
				 * and other types should find mntpt. */
				if (*have_cmsdb_lock && !have_cmsdb_excl) {
					CC_CMSDB_DLM_UNLOCK(cmsdb_sublock);
					*have_cmsdb_lock = 0;
					/* retry holding cmsdb lock
					 * exclusively */
					get_domain_lock = 0;
					goto retry;
				}
