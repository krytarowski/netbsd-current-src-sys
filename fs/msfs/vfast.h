...
#define SS_INIT_STEADY_STATE    24	/* in hours before hot list
					 * operational */
#define SS_INIT_MINUTES_HOT_LST 30	/* minutes before hot files distrib */
#define SS_INIT_HOT_ERROR_PENALTY 60	/* minutes before hot files distrib
					 * again */

/* Minimum values */
#define SS_MIN_PCT_MSGQ_MORE_THDS 50	/* % of msg Q filled before more thds */
#define SS_MIN_MSGQ_SIZE          64	/* message queues sizes */
#define SS_MIN_LIST_THDS         2	/* min number of thds per list worker
					 * pool */
#define SS_MIN_WORK_THDS         2	/* min number of thds per work pool */
#define SS_MIN_PCT_IOS_WHEN_BUSY 0	/* percent IOS when busy */
#define SS_MIN_FILE_ACC_THRESH 200	/* in number of IOs before hot list
					 * bump */
#define SS_MIN_STEADY_STATE      0	/* in hours before hot list
					 * operational */
#define SS_MIN_MINUTES_HOT_LST  5	/* minutes before files on hot list
					 * distrib */
#define SS_MIN_HOT_ERROR_PENALTY 30	/* minutes before hot files distrib
					 * again */
...
#define SS_MAX_MINUTES_HOT_LST  1500	/* minutes before files hot list
					 * distrib */
#define SS_MAX_HOT_ERROR_PENALTY 1440	/* minutes before hot files distrib
					 * again */

/* IO control */
#define SS_PAGECNT       4	/* number of pages to migrate for each work
				 * cycle */

/* monitor thread constants */
#define SS_VD_SAMPLE_INTERVAL    1
#define SS_HOT_LIST_CHK_REVS  600	/* interval to check hot list (in
					 * secs) */
...
#define SS_FRAG_CHK_REVS      300	/* interval to check for fragmented
					 * frag file */
...
#define SS_MAX_MSGS_IN_WORKQ   2/* Maximum msgs allowed in work queue before
				 * adding */
 /* another thread */

/* other constants */
#define SS_HOT_LST_CROSS_PCT  5	/* 5% of top hot files distributed(not incl
				 * rsvd) */
...
#define SS_PACK_RANGE_PCT_FREE      5	/* Pack when largest free chunk is LT
					 * 5% of the total free space. */
#define SS_PACK_RANGE_PCT_MAX_SIZE 25	/* pack when clear size not more than
					 * 25% of total vol size */
...
typedef enum {
	...
	SS_LULL_CHK,		/* send off a ok to run msg to vds that are
				 * bored */
...
typedef enum {
	...
	HOT_REMOVE,		/* remove list entry from both hot and frag
				 * lists */
	SS_LIST_STOP
}    ssListMsgTypeT;
...
typedef enum {
	...
	SS_STOP			/* thd is notified that it should abort due to
				 * dmn deactivate */
}    ssMsgOpT;
...
typedef enum {
	...
	SS_PARKED,		/* thd is parked awaiting continuation
				 * broadcast from io thd */
	SS_ABORT		/* thd is notified that it should abort due to
				 * dmn deactivate */
}    ssMigOpT;
...
typedef struct ssDmnInfo {
	mutexT ssDmnLk;		/* protects ssDmnHotWorking, Msg Counts,
				 * output variables, ssDmnSSThdCnt,
				 * ssDmnSSWaiters */
	...
	int ssPctIOs;		/* pct of ios allowed to ss when IO queues
				 * busy */
	...
	int ssDmnHotWorking;	/* TRUE,FALSE indicates that a thread is
				 ...
				 * threads must not also process this same hot
				 * file */
	int ssDmnHotPlaced;	/* TRUE,FALSE indicates that a hot file
				 * migrate is pending for domain, Prevents
...
typedef struct ssVolInfo {
	...
	cvT ssContMig_cv;	/* cv used to manage single thd/volume
				 * migrates */
	...
	ssMsgOpT ssVdMsgState;	/* state of the vd with regard to work
				 * messages */

	ssPackHdrT ssPackHdr;	/* not-locked, single work thread access */
	mutexT ssFragLk;	/* protects frag list from collisions by work
				 * and list thds */
	ssFragHdrT ssFragHdr;	/* list of ssFragLLT structs */
	bfTagT ssVdHotTag;	/* tag of file to move to this volume in
