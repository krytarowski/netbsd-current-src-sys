...
)
{
	...
	do {
		...
		tmp->ioQ = BLOCKING;
		if (i > 1) {
			/* The following is a prefetch of bsBuf.  It puts the
			 * data structure into cache so that it is ready when
			 * needed. */
	...
	mutex_enter(&vdp->blockingQ.ioQLock.mutex);
	/*
...
)
{
	 ...
	 * Advfs_disable_flushq will force us back to the old algorithm of
	 * putting all non-lazy I/Os onto the blocking queue. */
	...
	mutex_enter(&vdp->flushQ.ioQLock.mutex);
...
)
{
	 ...
	 * Advfs_disable_ubcreq will force us back to the old algorithm of
	 * putting all I/O requests from the UBC for freeing pages onto the
	 * blocking queue. */
	...
	mutex_enter(&vdp->ubcReqQ.ioQLock.mutex);
...
void
bs_io_metacheck(struct bsBuf * bp)
{				/* in */
	switch (bp->metaCheck) {
...
)
{
	...
	if (bp->lock.state & WRITING) {
		...
		/* Seize file's bfIoLock while moving buffer around on dirty
		 * list. */
		mutex_enter(&bfap->bfIoLock.mutex);

		/* Notify any threads which are flushing this file that this
	...
	if (bp->lsnFwd != NULL) {
		...
		if (lsnp->lsnFwd == bp) {
			...
			if (lsnp->length > 1) {
				...
				/* There are no more dirty buffers on the lsn
				 * list */
			...
			/* The meta data for the oldest log entry just got
			 * written, */
	...
	if (contBits) {
		...
		/* this means the log has NOT been initialized if the log
		 * isn't initialized, we can't do log I/O...  panic the system */
		radId = bfap->dmnP->logVdRadId;
		if ((IoMsgQH[radId] == NULL) && (bs_io_rad_start(radId) != 0)) {
			int i;

			for (i = 0; i < nrads; i++) {
				/* if the RAD isn't valid and we can't start
				 * it, find a valid one!! */
				if (IoMsgQH[i] != NULL) {
					radId = i;
					break;
...
clearSignal:
	...
	bp->metaCheck = 0;	/* I/O completion data validity check is now
				 * off */
	...
	if (bp->directIO && bp->aio_bp) {
		...
		/* Pass the I/O result to the AIO's done routine if we don't
		...
		/* Now invoke aio completion routine; may deallocate
		 * bp->aio_bp */
		bp->aio_bp->b_iodone(bp->aio_bp);
	}
	/* The following code was in bs_osf_complete(). It has been moved here
	 * because the  call to  ubc_page_release() at the end of
	 * bs_io_complete(), may trigger the deallocation of the bsBuf. */
	...
	if (!bp->actRangep) {
		...
		if (bp->directIO && bp->aio_bp) {
			 ...
			 * this check before releasing the bufLock. Remember
			 * to clean up bsBuf after we drop locks. */
	...
	/* Release the UBC page except for raw IO requests that do not have a
	 * UBC page, or errors paths where the caller did not make the page
	 * busy.  Caller is responsible for dealing with UBC page contents if
	 * an I/O error occurs. After this call to ubc_page_release(), the
	 * bsBuf should not be referenced unless the caller is certain that
	 * the pg_hold count is up on the vm_page.  This is because if the
	 * pg_hold count is not up, ubc_page_release() will put the vm_page
	 * onto the UBC's LRU.  Another thread may then recycle the vm_page,
	 * which will cause the bsBuf to be deallocated. */
	if (release_ubc_page) {
	...
	if (dmnP->contBits & CK_WAITQ) {
		...
		while (vd_count < dmnP->vdCnt && vdi <= BS_MAX_VDI) {
			...
			if (vdp = vd_htop_if_valid(vdi, dmnP, TRUE, TRUE)) {
				/* call vd_htop_if_valid with zombie_ok = TRUE
				 * since */
				/* this routine is called by bs_io_thread and
				 * we want */
				/* to start the IO even if this volume is
				 * being removed. */

				wait_to_readyq(vdp);

				/* If there are any buffers on the lazy
				 * (consolidate) queue, start some I/Os from
				 * the lazy queue now. (readyQ length was
				 * checked in wait_to_readyq()). */
...
static
void
lsn_io_list(struct domain * dmnP)
{				/* in */
	...
	KASSERT(SLOCK_HOLDER(&dmnP->lsnLock.mutex));
...
loop:
	...
	while (bp != (struct bsBuf *) lsnp &&
		...
		/* If this buffer is already BUSY, either because this routine
		 * already marked it so or because another thread did so, then
		 * skip it since it is already on blocking or device queue.
		 * No need to lock the buffer since the BUSY bit won't be
		 * cleared in bs_io_complete() until after it gets removed
		 * from the lsnList, and we have that lock. */
		...
		if (couldnt_hold) {
			/* The ubc may be in the process of exchanging the
			 * page for numa locality or bigpage consolidation.
			 * We need to wait for it to finish, but we have to
			 * drop our locks and there are no guarantees the page
			 * will be valid on wake up. */
			if (couldnt_hold == UBC_GET_WAIT_EXCHANGE) {
				ubc_fs_page_wait(bp->vmpage, &dmnP->lsnLock.mutex);
				mutex_enter(&dmnP->lsnLock.mutex);
				if (ioListp) {
					/* Point to the last buffer on our
					 * accumulated list and continue on to
					 * the next buffer on the lsnlist,
					 * which if we are lucky is this same
					 * buffer.  This avoids having to
					 * retraverse the list past these
					 * buffers. */
			...
			/* it must be busy from another I/O thead and we can
			 * skip it */
			bp = bp->lsnFwd;
			continue;
		}
		if (!mutex_tryenter(&bp->bufLock.mutex.mutex)) {
			 ...
			 * one.  We do have to be concerned about the buffer
			 * still being on the lsnList, however. */
			mutex_exit(&dmnP->lsnLock.mutex);
			mutex_enter(&bp->bufLock.mutex);
			mutex_enter(&dmnP->lsnLock.mutex);
			if (!lsnp->lsnFwd ||
			    LSN_GT(bp->currentLogRec.lsn, logBfap->hiFlushLsn)) {
				/* this buffer no longer on lsnList or has
				 * changed position, so just skip it!  Enqueue
				 * any buffers we have accumulated and restart
				 * the traversal of lsnList. */
				mutex_exit(&bp->bufLock.mutex);
				if (len) {
					mutex_exit(&dmnP->lsnLock.mutex);
					bs_q_list(ioListp, len, bs_q_blocking);
					mutex_enter(&dmnP->lsnLock.mutex);	/* should get this OK */
				}
		 ...
		 * can lock the vdp->vdIoLock which is higher in the
		 * hierarchy. */
		mutex_exit(&dmnP->lsnLock.mutex);
		rm_from_lazyq(bp, &ioListPart, &listLenPart, &noqfnd);

		if (listLenPart) {
			/* Setup UBC page for IO. Failure should be impossible
			 * so just prevent further damage and crash it in
			 * development. */
			int sts;
			if ((sts = advfs_page_get(bp, UBC_GET_BUSY | UBC_GET_HAVEHOLD))) {
				...
				/* Force advfs io completion to clean up our
				 * chains so we don't just have threads
				 * hanging.  The ubc page remains dirty and
				 * since it is metadata probably stays in
				 * cache until the object is reclaimed.
				 * 
				 * Note we have not set BUSY so the page is not
				 * released in bs_io_complete().  We drop our
				 * hold, queue up the previous busy buffer
				 * list, and restart since the bsBuf is pulled
				 * off the lsn list in bs_io_complete(). */
				bp->ln = SET_LINE_AND_THREAD(__LINE__);
		...
		/* Because we drop the lsnLock, we can lose our way down the
		 * list. If this happens, put any buffers we have accumulated
		 * so far onto the blocking queue, and then retraverse the
		 * list.  This is fairly fast since any we have moved to the
		 * blocking queue already will be marked BUSY and we will skip
		 * them. */
		mutex_enter(&dmnP->lsnLock.mutex);
	...
	/* clear the flag for the pinBlockRunning since we are finishing and a
	 * new pinblock can change the writeToLsn while we are in bs_q_list(). */
	dmnP->pinBlockRunning = FALSE;
...
void
bs_lsnList_flush(struct domain * dmnP)
{				/* in */
	struct bsBufHdr *lsnp = &dmnP->lsnList;
	struct bsBuf *bp, *nextbp;
	ioDescT *tmp;
	...
	/* Only run one domain's metadata lsnList flush at a time. If a
	 * pinblock flush is in progress, then skip this flush. */
...
loop:
	...
	while (bp != (struct bsBuf *) lsnp &&
		 ...
		 * from the lsnList, and we have that lock.
		 * 
		 * Because this is an advisory flush we can also skip
		 * transitioning buffers in FLUSH or IO_TRANS without the need
		 * to lock them. */
		if (bp->lock.state & (BUSY | FLUSH | IO_TRANS)) {
			bp = bp->lsnFwd;
			continue;
		}
		/* this is only an advisory flush so it is ok to just skip
		 * pages */
		...
		/* Skip currently pinned pages. Also skip repinned pages whose
		 * currentLogRec LSN is higher than the highest flushed log
		 * LSN. */
		if (bp->writeRef ||
		    LSN_GT(bp->currentLogRec.lsn, logBfap->hiFlushLsn)) {
			nextbp = bp->lsnFwd;
		...
		/* If the FLUSH and IO_TRANS buffer states are set, then skip
		 * the buffer instead of waiting for them to clear. Otherwise,
		 * we can just go ahead and set the IO_TRANS flag. The FLUSH
...
)
{
	...
	/* Charge the process for i/o Initialize the sync_stamp */
	tmp = ioListp;
	i = len - 1;
	do {
		bp = tmp->bsBuf;
	...
	/* If smoothsync is enabled, call smsync_to_readyq(), otherwise, check
	 * to see if we need to start some I/Os. */
	if (smsync_period) {
		/*
		...
		/* If the readyLazyQ is above threshold, move its buffers to
		 * the consolidate queue and start some I/Os from the lazy
		 * (consolidate) queue. */
	...
	if (!LSN_EQ_NIL(ioListp->bsBuf->origLogRec.lsn)) {
		...
		if (LSN_GT(ioListp->bsBuf->currentLogRec.lsn,
			...
			mutex_enter(&vdp->waitLazyQ.ioQLock.mutex);
...
wait_to_readyq(struct vd * vdp)
{
	...
	while (iop != (ioDescT *) & vdp->waitLazyQ) {
		...
		if (count) {
			...
			/* Need special case help for sort_onto_readyQ(): If
			 * we have a very long list we need to sort it using a
			 * tempQ and that requires us to malloc a marker
			 * struct. Dropping the lock to do the malloc can not
			 * be done after we make the queues inconsistent by
			 * removing the entries below while leaving them
			 * marked as on the waitLazyQ. Here we process what we
			 * found if we can get the struct without dropping our
			 * lock, otherwise we have to do it the hard way: drop
			 * our lock, wait for memory, and restart. */
			...
			/* hand doubly-linked list off to be added to correct
			 * queue */
			end->fwd = start;
			start->bwd = end;
			if (smsync_period) {
			...
			/* The waitLazy queue lock was dropped in
			 * add_to_smsync() or sort_onto_readyq(), so reaquire
			 * it and reset the iop. */
			mutex_enter(&vdp->waitLazyQ.ioQLock.mutex);
			iop = vdp->waitLazyQ.fwd;
		} else {
			iop = iop->fwd;
...
ready_to_consolq(struct vd * vdp)
{
	...
	mutex_enter(&vdp->readyLazyQ.ioQLock.mutex);
	mutex_enter(&vdp->consolQ.ioQLock.mutex);
	...
	while (iop != (ioDescT *) & vdp->readyLazyQ) {
		...
		/* We only want to move eligible bsBufs down. If there are
		 * mutliple i/o descriptors then skip over this iodesc.
		 * Migrate will take care of flushing. */
    ...
    ioDescT * ioListp, int cnt, vdT * vdp)
{
	...
	/* Walk this list resetting 2 fields so that it is apparent to removal
	 * code that each ioDesc is on a temporary queue. */
	iop = first = ioListp;
	do {
		iop->ioQ = TEMPORARYQ;
		iop = iop->fwd;
	...
	if (vdp->tempQ.cnt == 0) {
		...
		if (local_tmpQ_marker == &vdp->tempQ) {
			/* not found, insert new marker and ioList at end of
			 * tempQ */
			tempQ_marker->mfwd = &vdp->tempQ;
...
)
{
	 ...
	 * element.  The code below is optimized for long chains, so there is
	 * too much overhead to efficiently handle single-element chains. */
	if (cnt == 1) {
		mutex_enter(&vdp->readyLazyQ.ioQLock.mutex);
		iop->ioQ = READY_LAZY;
		if (vdp->readyLazyQ.ioQLen == 0) {
			...
			/* The incoming element goes at the end of the readyQ;
			 * this seems to happen frequently when writing large
			 * files sequentially. */
			...
			/* The incoming element goes at the front of the
			 * readyQ; this keeps us from having to walk the whole
			 * list. */
			readyp = (ioDescT *) (&vdp->readyLazyQ);
			KASSERT(vdp->readyLazyQ.fwd != (ioDescT *) & vdp->readyLazyQ);
			iop->bwd = readyp;
			 ...
			 * reduce search time for sequential writes when
			 * pressure to move the elements off the readyq is
			 * low. */
		...
		if (incoming_lockp)
			mutex_exit(incoming_lockp.mutex);
	 ...
	 * shorter than Advfs_sort_factor, we won't do this code. Long list
	 * sort method: 1.  Move any entries already on the readyQ to the
	 * consolQ. 2.  Loop doing the following until incoming list is empty:
	 * A. Sort the first Advfs_sort_factor elements of incoming list B.
	 * Put this list onto the readyQ. C. Call ready_to_consolq() to move
	 * these down. D. Call bs_startio() since we know we moved a bunch to
	 * the consolQ. This also provides us with a preemption point. */
	if (tempQ_marker && cnt >= Advfs_sort_factor) {
		...
		/* Since we will drop the per-queue locks in this path, we
		 * must mark all the ioDescT on the incoming list so
		 * rm_ioq()/rm_from_lazyq() will not try to account for
		 * removing the ioDescT from one of the legitimate queues, but
		 * will be safely able to remove the ioDesc from the temporary
		 * queue. */
		mutex_enter(&vdp->tempQ.ioQLock.mutex);
		...
		while (chain_iop != (ioDescT *) tempQ_marker) {
			...
			/* Sort the incoming list into sublists of some
			 * multiple of readyLazyQ.lenLimit elements. Then
			 * insert each subchain onto the readyQ. */
			while (chain_iop != (ioDescT *) tempQ_marker) {
			...
			/* Restart back at the front of the chain.  The
			 * ioDescT pointed to by chain_iop may have been
			 * removed from this list by rm_ioq() or
			 * rm_from_lazyq() when the temporary queue lock was
			 * dropped above. */
			mutex_enter(&vdp->tempQ.ioQLock.mutex);
	...
	/* Processing of shorter incoming chains.  Sort/merge them onto the
	 * readyQ without dropping the incoming queue lock. */
	...
	while (chain_iop) {	/* NULL when input chain processed */
		...
		/* Sort the original list; this is a really cheap algorithm
		 * and to maximize throughput, we sort sublists of up to 512
		 * elements and then insert each subchain onto the readyQ.  In
		...
		/* Now we have a sorted chain, and we want to add these
		 * elements onto the readyLazyQ.  */
		readyp = (ioDescT *) (vdp->readyLazyQ.fwd);
		if (readyp == (ioDescT *) & vdp->readyLazyQ) {
			/* readyLazyQ is empty; insert entire list in one fell
			 * swoop */
			readyp->fwd = sorted_list->fwd;
			...
			RdySort[0] += chain_len;	/* # elements added in a
							 * group */
		...
		/* Otherwise, add them on one-at-a-time in the correct
		 * locations */
		iop = sorted_list->fwd;
		while (iop != sorted_list) {
			next = iop->fwd;

			/* Advance along readyLazyQ until we know next place
			 * to insert */
			while (readyp != (ioDescT *) & vdp->readyLazyQ &&
			    readyp->blkDesc.vdBlk < iop->blkDesc.vdBlk) {
				readyp = readyp->fwd;
			...
			if (readyp == (ioDescT *) & vdp->readyLazyQ) {
				break;	/* get out; add rest of list as a
					 * group at end */
			}
			KASSERT(iop->blkDesc.vdBlk <= readyp->blkDesc.vdBlk);

			/* Insert this entry onto the list. As an
			 * optimization, try to place as many entries from the
			 * sorted list onto the ready queue as belong at this
			 * location. */
			added_this_group = 1;
			while (next != sorted_list &&
			    next->blkDesc.vdBlk <= readyp->blkDesc.vdBlk) {
				next = next->fwd;
				added_this_group++;	/* # elements added as a
							 * group */
			}
			last = next->bwd;	/* may == iop if only 1 to be
						 * inserted */
			...
			/* keep track of some statistics to be sure this is
			 * working the way we want. */
			RdySort[4] += added_this_group;
			RdySort[5]++;
			cum_added_here += added_this_group;	/* total # added for
								 * this list */
		...
		if (iop != sorted_list) {
			/* We get here if we still have entries on the sorted
			 * list, but have gotten to the end of the readyLazyQ.
			 * Add the remainder of the list in one fell swoop.
			 * readyp is head of queue. */
			iop->bwd = readyp->bwd;
...
call_logflush(domainT * dmnP, lsnT lsn, int wait)
{
	...
	/* The range containing this log page might encompass the last log
	 * page. We need to call lgr_flush to determine if the last page is in
	 * the range and if so force it to be unpinned. */
...
void
logflush_cont(struct bfAccess * bfap)
{				/* in - bfap for log */
...
restart:
	...
	for (bp = head; bp != tail->accFwd;
		 ...
		 * should be OK since we are  not modifying these values and
		 * we just want to skip the buffers with write Refs or that
		 * are BUSY. BUSY may be the result of this routine setting it
		 * below, and having to restart the scan. Any pages that have
		 * writerefs (pinned) will be flushed at unpin time. */
		...
		if (couldnt_hold) {
			...
			if (couldnt_hold == UBC_GET_WAIT_EXCHANGE) {
				...
				if (ioListp) {
					/* Point to the last buffer on our
					 * accumulated list and continue on to
					 * the next buffer on the
					 * dirtyBufList. This avoids having to
					 * retraverse the list past them. */
		...
		/* Try to lock in out-of-hierarchy order since this is
		...
		if (!mutex_tryenter(&bp->bufLock.mutex.mutex)) {
			...
			if (!(bp->lock.state & ACC_DIRTY) ||
				...
				/* we have lost our way down the chain;
				 * restart scan at head of list, but we will
				 * skip any buffers we already put onto our
				 * list since they are marked BUSY. */
				goto restart;
			}
		}
		/* We need to recheck that an unpin did not already start an
		 * io on this buffer since were not holding the buf lock
		 * above. */
		...
		/* Setup UBC page for IO. Failure should be impossible so just
		...
		if ((sts = advfs_page_get(bp, UBC_GET_BUSY | UBC_GET_HAVEHOLD))) {
			 ...
			 * remains dirty and since it is metadata probably
			 * stays in cache until the domain is unmounted.
			 * 
			 * Note we have not set BUSY so the page is not released
			 * in bs_io_complete().  We release our hold and
			 * restart since we pulled the bsBuf from the
			 * dirtyBufList. */
			bp->ln = SET_LINE_AND_THREAD(__LINE__);
			bs_io_complete(bp, &sts);
			ubc_page_release(bp->vmpage, B_WRITE);
...
loop:
	...
	if ((bfap->dataSafety == BFD_FTX_AGENT) ||
		...
		/* Scan the list for the greatest LSN value in the log that
		 * must be flushed before we can flush the data. Note that we
		 * do not lock the buffer to read currentLogRec.lsn to avoid
		 * the lock overhead.  If a buffer is updated after we extract
		 * this value and call call_logflush(), that will be caught in
		 * the loop below. */
		hiCurrentLsn = nilLSN;
		...
		if (LSN_GT(hiCurrentLsn, logBfap->hiFlushLsn)) {
			...
			/* since we released the file's bfIoLock, must restart
			 * the scan */
			goto loop;
		}
	}
	...
	/* Only do this the first time thru; this makes the final flushSeq
	 * value on entry to this routine the terminator of this loop, not the
	 * physical makeup of the dirty list, which can change as we go along.
	 * Remember if a caller wants a specified number of pages flushed. In
	 * that case, only a specific number of buffers are flushed and and
	 * the flushSeq of the last buffer will get passed back to caller. */
	if (!end_has_been_determined) {
		finalLsnWeNeedToFlush = bfap->dirtyBufList.accBwd->flushSeq;
		useFlushCnt = (flushPgCnt) ? TRUE : FALSE;
		end_has_been_determined = 1;
	}
	/* Flush all pages on the dirtyBufList upto the predetermined end or
	 * flush only a specific number of pages the caller wants. */
	for (bp = head;
	    bp != (struct bsBuf *) & bfap->dirtyBufList &&
		...
		if (!mutex_tryenter(&bp->bufLock.mutex.mutex)) {
			...
			mutex_enter(&bp->bufLock.mutex);
			mutex_enter(&bfap->bfIoLock.mutex);
			if (bp->accListSeq != savListSeq) {	/* bp removed from acc
								 * list */
				/* buffer was removed from dirty list while we
				 * dropped the chain lock.  Skip this buffer,
				 * but must go back and restart the scan. */
				mutex_exit(&bp->bufLock.mutex);
				/* Drop UBC page (pg_hold count) reference. */
				ubc_page_release(bp->vmpage, B_WRITE);
				...
				if (ioList) {
					 ...
					 * accumulated list and continue on to
					 * the next buffer on the
					 * dirtyBufList.  This avoids having
					 * to retraverse the list past these
					 * buffers, as well as waiting for
					 * their IO_TRANS state to clear. */
					/* Since these iop are not on any Q
					 * the IO for that bp */
					/* has not completed. Therefore it is
					 * still on the bfap */
					/* dirtyBufList. */
					bp = ioList->bwd->bsBuf;
		...
		if (bp->lock.state & FLUSH) {
			...
			if (listLen) {
				...
				if (bp->accListSeq != savListSeq) {	/* bp removed from list */
					/* We had to release both locks so
					 * make sure I/O didn't complete and
					 * bsBuf didn't get reused before
					 * waiting. */
					mutex_exit(&bp->bufLock.mutex);
					mutex_enter(&bfap->bfIoLock.mutex);
					/* Drop UBC page (pg_hold count)
		 ...
		 * that there is no guarantee that this ioDesc is still on the
		 * lazy queues; it may have completed IO by time
		 * rm_from_lazyq() returns.  If this is so, then we must
		 * determine the identity of the buffer pointed to by 'bp'; it
		 * may no longer be a buffer belonging to this file or on this
		 * file's dirty list.
		 * 
		 * Because the buffer may no longer belong to this file on
		 * return, it is not safe to clear the FLUSH state on return.
		 * Therefore, now that IO_TRANS is set clear the FLUSH state,
		 * but don't awaken any threads waiting for FLUSH.  They will
		 * be awakened when I/O completes or on return when IO_TRANS
		 * is cleared. The other user of FLUSH is the migrate path
		 * which sets FLUSH in set_block_map() prior to pinning the
		 * page.  It will now be able to set FLUSH once we clear it
		 * and release the locks, and proceed to bs_pinpg where it
		 * will wait on IO_TRANS for this buffer. Note that the reason
		 * we have this problem with FLUSH and not IO_TRANS is that
		 * IO_TRANS is cleared by bs_io_complete().  If the buffer is
		 * not representing the same dirty page on return then its i/o
		 * has completed and bs_io_complete() already cleared the
		 * IO_TRANS state. */
		bp->lock.state &= ~FLUSH;
		mutex_exit(&bfap->bfIoLock.mutex);
		rm_from_lazyq(bp, &ioListPart, &listLenPart, &noqfnd);

		/* Enque buffer that was taken off lazy queue or that
		 * originates from a getpage write and was not on a queue. */
		if (listLenPart || (noqfnd && (bp->lock.state & GETPAGE_WRITE))) {
			 ...
			 * mmapped page is wired by an application so we can
			 * not change protection to make the page clean.  In
			 * that case just write the page while keeping it
			 * dirty.  Any other failure should be impossible so
			 * just prevent further damage and crash it in
			 * development. */
			if ((sts = advfs_page_get(bp, UBC_GET_BUSY | UBC_GET_HAVEHOLD))) {
				...
				if (sts) {
					...
					/* Force advfs io completion to clean
					 * up our chains so we don't just have
					 * threads hanging.  The ubc page
					 * remains dirty until lru reuse
					 * forces it through the putpage path,
					 * where it will be tossed by
					 * bs_pinpg_put.
					 * 
					 * Note we have not set BUSY so the page
					 * is not released in
					 * bs_io_complete().  We release our
					 * hold and restart since we pulled
					 * the bsBuf from the dirtyBufList. */
					bp->ln = SET_LINE_AND_THREAD(__LINE__);
					bs_io_complete(bp, &sts);
					ubc_page_release(bp->vmpage, B_WRITE);
					if (listLen)
			...
			/* we didn't get the buffer off the lazy queues, so
			 * check it's identity before continuing.  If changed,
			 * put any accumulated ioDesc structs onto the flushq
			 * queue and restart the traversal. */
			mutex_exit(&bp->bufLock.mutex);
			if (listLen) {
				/* Put any buffers found onto the flushq queue
				 * before going back to the top and
				 * reinitializing the ioList. */
				bs_q_list(ioList, listLen,
				    useFlushCnt ? bs_q_ubcreq : bs_q_flushq);
			}
			mutex_enter(&bfap->bfIoLock.mutex);
...
end_of_loop:
	 ...
	 * will wait.  Note that bs_io_complete() and racing flushers can bump
	 * bfap->hiWaitLsn past the point that we will need to wait in
	...
	if (SEQ_LT(bfap->hiFlushLsn, waitLsn)) {
		...
		while (curFlushWaiter != (flushWaiterT *) & bfap->flushWaiterQ) {
			...
			if (LSN_EQL(curFlushWaiter->waitLsn, waitLsn)) {
				...
				/* Current flushWaiterQ's LSN is lower than
				 * new waitLsn. */
		...
		if (curFlushWaiter == (flushWaiterT *) & bfap->flushWaiterQ) {
			...
			if (bfap->flushWaiterQ.tail ==
				...
				/* The queue has existing entries. Insert new
				 * waiter at the queue head since the new
				 * flush waiter's Lsn is less than all
				 * existing waiters. */
		...
		/* Block til bs_io_complete determines this IO flush sync is
		 * finished. */
		mark_bio_wait;
		...
		/* The target Lsn is already flushed, so this flush sync is
		 * done.  */
		deallocWaiter = TRUE;
	}
...
)
{
	...
	do {
		...
		/* make this quick check as an optimization to see if the
		 * buffer and all those before it have been flushed */
		if (SEQ_GTE(bfap->hiFlushLsn, waitLsn)) {
			break;
		}
		/* Search the dirtyBufList to find the buffer with the highest
		 * sequence number equal or less than our sequence number,
		 * ignoring pinned buffers. */
		...
		if (bp) {
			/* Wait for I/O to complete by checking vm_page
			 * pg_busy which is cleared in bs_io_complete().  Note
			 * that on wakeup the vm_page and bsBuf may be gone so
			 * we don't reference again and we start over to see
			 * if all the buffers are now gone.  The bfIoLock is
			 * unlocked in ubc_fs_page_wait() after the pg/obj
			 * lock is taken and no locks are held on return. */
			mark_bio_wait;
			ubc_fs_page_wait(bp->vmpage, &bfap->bfIoLock.mutex);
			unmark_bio_wait;
		}		/* else bp==NULL, while ends with bfIoLock
				 * still held */
	} while (bp);
...
)
{
	...
	if (priority & FLUSH_IMMEDIATE)
		q_for_io = bs_q_blocking;
	else if (priority & FLUSH_UBC)
		q_for_io = bs_q_ubcreq;
	else
		q_for_io = bs_q_flushq;
	...
	/* If we have dirty buffers we should always have a valid fileset and
	 * domain.  The call from grab_bsacc() without dirty buffers is being
	 * made with a null bfsetp and dmnp that points to trash, but we
	 * believe that is the only invalid (and unneccessary) call. This will
	 * catch any real nasty caller. */
	KASSERT(BFSET_VALID(bfap->bfSetp));
	KASSERT(TEST_DMNP(bfap->dmnP) == EOK);

	/* If both first_page and pages_to_flush are zero, caller wants to do
	 * a full file flush for regular files.  This is not normally used for
	 * metadata files, but it can get called this way if the metadata file
	 * was opened outside the kernel via .tags of VM is requesting an
	 * object fluxh due to low memory conditions */
	...
	/* The last dirtyBufList buffer's flushSeq value will be the
	 * terminator of this loop, not the physical makeup of the dirty list,
	 * which can change as we go along. */
...
loop:
	...
	for (bp = bfap->dirtyBufList.accFwd;
		 ...
		 * since we are past the ones that could have applied to this
		 * flush. This works even if the flushSeq is reassigned to
		 * this bsBuf from a later one since we will process this one
		 * if it is in range. */
		...
		if (couldnt_hold) {
			...
			if (couldnt_hold == UBC_GET_WAIT_EXCHANGE) {
				...
				if (ioList) {
					 ...
					 * retraverse the list past them. */
					bp = ioList->bwd->bsBuf;
					/* make sure page being exchanged gets
					 * rechecked */
					foundTerminator = FALSE;
					continue;
				} else {
		...
		/* If somebody else has this page pinned, mark it. Note that
		 * this check must be performed after the IO_TRANS check above
		 * because if it is performed first, we might try to mark a
		 * temporary buffer placed on the dirty buf by
		 * bs_pinpg_one_int() processing which will then be removed
		 * when that thread realizes he has lost a race with another
		...
		/* Must release the bfap->bfIoLock for rm_from_lazyq() since
		 * it will lock vdp->vdIoLock which is higher in the
		 * hierarchy.  Note bp->bufLock is unlocked and relocked by
		 * rm_from_lazyq().  Thus there is no guarantee that this
		 * ioDesc is still on the lazy queues; it may have completed
		 * IO by the time rm_from_lazyq() returns.  If this is so,
		 * then we must see if this buffer was taken off (and maybe
		 * put back on) the dirty list; it still belongs to this file
		 * since we have the ubc page held.
		 * 
		 * Now that IO_TRANS is set clear the FLUSH state, but don't
		 * awaken any threads waiting for FLUSH.  They will be
		 * awakened when I/O completes or on return when IO_TRANS is
		 * cleared. The other user of FLUSH is the migrate path which
		 * sets FLUSH in set_block_map() prior to pinning the page.
		 * It will now be able to set FLUSH once we clear it and
		 * release the locks, and proceed to bs_pinpg where it will
		 * wait on IO_TRANS for this buffer. */
		bp->lock.state &= ~FLUSH;
		mutex_exit(&bfap->bfIoLock.mutex);
		rm_from_lazyq(bp, &ioListPart, &listLenPart, &noqfnd);
		mutex_enter(&bfap->bfIoLock.mutex);
		...
		if (listLenPart || (noqfnd && (bp->lock.state & GETPAGE_WRITE))) {
			...
			mutex_exit(&bp->bufLock.mutex);
			if (listLen) {
				/* Put any buffers found onto the blocking
				 * queue before going back to the top and
				 * reinitializing the ioList. */
				mutex_exit(&bfap->bfIoLock.mutex);
				bs_q_list(ioList, listLen, q_for_io);
				mutex_enter(&bfap->bfIoLock.mutex);
	...
	mutex_destroy(&rfp->rangeFlushLock.mutex);
	ms_free(rfp);
...
rm_from_lazyq(struct bsBuf * bp,
    ...
    int *listLen,
    int *noqfnd)
{				/* TRUE if all ioDescs were on no queue */
	int first, last, i;
	ioDescT *iop;
	...
	*listLen = 0;
	...
	/* The buffer is scheduled for IO. It is on one of the I/O queues or
	 * is on a temporary queue because it is being moved between I/O
	 * queues. */
	KASSERT(bp->lock.state & ACC_DIRTY);
...
again:
		if (on_lazyq) {
			...
			if (!mutex_tryenter(&lockptr->mutex.mutex)) {

				mutex_exit(&bp->bufLock.mutex);

				/* From this point until the ioQLock is
				 * seized, the bp is free to change. It can
				 * move from queue to queue. One of the two
				 * possible iop's can complete IO. The bp can
				 * even have all IO completed and be recycled
				 * to another page or bfap. */
				...
				if (bp->accListSeq != savListSeq) {
					/* IO got done while we waited for the
					 * lock. If we had removed 1 of the 2
					 * possible iops, the IO could not
					 * have completed. Therefore we must
					 * not have removed any iops. */
					KASSERT(*listLen == 0);
				...
				if (i >= bp->ioList.write + bp->ioList.writeCnt) {
					/* ioList got changed by a call to
					 * buf_remap(). This can only happen
					 * if there were 2 iop and migration
					 * got done then buf_remap() was
					 * called. */
					KASSERT(i == 1);
			...
			/* Recheck the I/O queue the ioDesc is on since it may
			 * have moved between the first check and obtaining
			 * the I/O queue lock above */
			on_lazyq = FALSE;
...
again:
		if (qhdr != NULL) {
			...
			/* Remove ioDesc from queue. The casting is needed if
			 * we are removing from the temporary queue. */
			iop->fwd->bwd = (ioDescT *) iop->bwd;
			iop->bwd->fwd = (ioDescT *) iop->fwd;
...
bs_io_thread(int radId)
{
	...
	while (!done) {
		...
		case START_MORE_IO:
			{
				/* bs_osf_complete() noticed that the devQ is
				 ...
				 * still there. It should no longer be
				 * possible to have the domain and vd go away
				 * while a START_MORE_IO message is queued.
				 * The KASSERT is here to test this
				 * theory and should eventually be removed
				 * (along with the TEST_DMNP).
				 * 
				 * For this reason also we do not bump the refCnt
				 * on the vd; the start_io_posted logic should
				 * be sufficient.
				 * 
				 * We also assume the vdp from the message is
				 * valid. Note that if the volume is being
				 * removed, vdp may not be found in the
				 * dmnP->vdpTbl[]. */
		...
		case RETRY_IO:
			{
				...
				if (iop->desCnt != 0) {
					...
					/* if Direct I/O numBlks is really
					 * number of bytes */
					ioAmt = iop->numBlks;
				} else {
...
bs_init_io_thread(void)
{
	...
	for (i = 0; i < nrads; i++) {
		IoMsgQH[i] = NULL;	/* all queues should be NULL unless
					 * they and accompanying thread */
		/* have been successfully allocated. */
		if ((rad_id_to_rad(i) != NULL) && (bs_io_rad_start(i) == 0)) {	/* checks if rad 'i' is
										 * a valid */
			num_initialized_rads++;	/* rad, and if it is, it does
						 * a bs_io_rad_start on it. if */
		}		/* bs_io_rad_start returns 0, then we have an
				 * initialized, working rad */
	}
	if (num_initialized_rads == 0) {
		/* we have no initialized rads!  Panic the system, because
		 * without at least on queue and matching */
...
bs_io_rad_stop(int radId)
{
	...
	/* save a pointer to the queue and then set the REAL pointer to NULL
	 * so no one else can play with the queue while we're getting rid of
	 * it! */
	...
	for (i = 0; (i < MAX_RAD_DESTROY_ATTEMPTS) && (qToDestroy != NULL); i++) {
		...
		/* ?does this make sense? sleep(
		 * DELAY_BETWEEN_RAD_DESTROY_ATTEMPTS ); */
	}
	if (qToDestroy != NULL) {
		return 2;
	}
	...
	while (iop != incoming_hdrp) {
		...
		if (sched_tick - iop->bsBuf->sync_stamp < smsync_age && period) {
			...
			case 0:
				...
				break;
			case 1:
				stamp = iop->bsBuf->sync_stamp;
				break;
			case 2:
				stamp = iop->bsBuf->sync_stamp >> 1;
				break;
			case 4:
				stamp = iop->bsBuf->sync_stamp >> 2;
				break;
			default:
				stamp = iop->bsBuf->sync_stamp / period;
			}
			 ...
			 * IO_TRANS protecting the ioDesc from removal. So no
			 * need to worry about losing our way down the list. */
			next = iop->fwd;
			iop->bwd->fwd = iop->fwd;
			iop->fwd->bwd = iop->bwd;
			...
			/* Move aged buffers to the aged_list.  After all have
			 * been accumulated, we will call sort_onto_readyq()
			 * to put these buffers onto the readyLazyQ. */
...
smsync_to_readyq(struct vd * vdp, int flushFlag)
{
	...
	/* If the readyLazyQ is already above the threshold, move to consolQ
	 * to avoid overhead of sorting onto a long list. */
	if (vdp->readyLazyQ.ioQLen >= vdp->readyLazyQ.lenLimit) {
		if (ready_to_consolq(vdp)) {
			startiocalls[5]++;
	...
	for (; cnt--; from = (from + 1) % SMSYNC_NQS) {
		 ...
		 * sort_onto_readyq() or bs_startio().  Any racing thread will
		 * know to start on the next queue. */
		mutex_enter(&vdp->vdIoLock.mutex);
		vdp->syncQIndx = (from + 1) % SMSYNC_NQS;
		mutex_exit(&vdp->vdIoLock.mutex);

		/* Need special case help for sort_onto_readyQ(): If we have a
		 * very long list we need to sort it using a tempQ and that
		 * requires us to malloc a marker struct. Dropping the lock to
		 * do the malloc can not be done after we make the queues
		 * inconsistent by removing the entries below while leaving
		 * them marked as on a smsyncQ. Here we process what we found
		 * if we can get the struct without dropping our lock,
		 * otherwise we reset the chain head and length in case it
		 * changed. */
		...
		if (iop != ((ioDescT *) smsyncq)) {
			...
			/* Move the list to the readyQ. The smoothsync queue
			 * lock will be dropped inside sort_onto_readyq(). */
	...
	/* If we flushed all the smoothsync queues, check to see if we need to
	 * give up the processor for other tasks. */
