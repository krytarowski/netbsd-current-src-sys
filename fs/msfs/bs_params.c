...
)
{
	...
	*allocVdIndex = xtntMap->allocVdIndex;
	subXtntBase = xtntMap->subXtntMap;	/* Base address of malloc'd
						 * array */
...
)
{
	 ...
	 * are valid. This only retrieves xtnt map into memory if changes
	 * occur, they occur on the mem copy only This returns with the
	...
	*allocVdIndex = clone_xtntMap->allocVdIndex;	/* not used yet -
							 * stripes? */
	...
	*pageCnt = pagecnt;
	...
	if (clonextnt_locked) {
		/* cloneXtntsRetrieved is protected by clu_clonextnt_lk AND
		 * cow_lk. */
...
)
{
	...
	while (nextPage >= 0) {
		...
		/* if we have consecutive holes which are spread across the
		 * volumes we combine them together and represent it as  one
		 * single hole with the volume index as the first volume which
		 * has this consecutive hole. */
	...
	if (flags & XTNT_XTNTCNT_ONLY) {
		/* return the total size of the extent map. Since it is a
		 * striped file we have an array of extent map, array size
		 * being the size of of the segment. */
		*xtntCnt = idx;
		RAISE_EXCEPTION(EOK);
	}
	idx = idx - startXtnt;
    ...
    vdIndexT * vdIndexP, uint32_t * vdBlkP)
{
	...
	if (!cloned) {
		...
		if (pg < bfap->maxClonePgs) {
			...
			/* In case of striped files when we reach end of
			 * segment nextPage will be set to -1. So don't set
			 * nextPage to maxClonePgs when we reach end of
			 * segment. */
			if (*nextPageP != -1 && *nextPageP > bfap->maxClonePgs) {
				/* Don't allow nextPage to progress beyond
				 * maxClonePgs. */
		...
		if (ret == FALSE) {	/* hole or permanent hole or not
					 * mapped */
			/* Permanent holes return TRUE */
			ret = page_is_mapped_local(bfap, pg, nextPageP, TRUE,
			    vdIndexP, vdBlkP, start_pgP, TRUE);
			if (*nextPageP == -1) {	/* pg is off the end of file
						 * or end of stripe */
			...
			if (ret == FALSE) {	/* its normal hole, look at
						 * original */
				...
				/* Permanent hole, so consider as hole in
				 * clone */
				ret = FALSE;
			}
		}
...
)
{
	...
	case BSXMT_APPEND:
		...
		if (bfap->cloneId == 0) {
			/* file is a regular file or a clone without any
			 * changes */
			if (bfap->bfSetp->cloneId != BS_BFSET_ORIG) {

				/* This is a clone that has not had any
				 * metadata (or file pages) COWed. At open
				 * time the original's extents were loaded
				 * into it's (the clone's) in memory extent
				 * map.  This is basically a bug (to be
				 * addressed at some future time). We can not
				 * trust these extents since the original
				 * could have been migrated (which does not
				 * cause a COW).  So we need to use the
				 * original's bfap not the clone's */

				if (clu_is_ready()) {
					/* Since we are about to use the
					 * original access structure for
					 * obtaining extent maps, we must
					 * protect against it being migrated
					 * or COWed. Migrate will get the
					 * clu_clonextnt_lk on the clone when
					 * migrating the original and
					 * bs_cow/bs_cow_pg will get the COW
		...
		if (bfap->cloneId == 0) {
			...
			if (bfap->bfSetp->cloneId != BS_BFSET_ORIG) {
				...
				if (clu_is_ready()) {
					 ...
					 * obtaining extent maps, we must
					 * protect against it being migrated.
					 * Migrate will get the
					 * clu_clonextnt_lk on the clone when
					 * migrating the original. So this
					 * will protect us here.
					 * 
					 * NOTE: The clu_clonextnt_lk and cow_lk
					 * are obtained in
...
)
{
	...
	bfSetT *bfSetp;
	ftxHT ftxH;
	int ftxStarted = FALSE, file_lock_held = FALSE, migtrunc_lock_held = FALSE;
	...
	if (bfParams->cl.dataSafety != bfap->dataSafety) {
		...
		if (bfap->dataSafety == BFD_FTX_AGENT) {
			...
			if (bfParams->cl.dataSafety == BFD_FTX_AGENT) {
				/* Enable data logging.  Flush any dirty
				 * pages, including dirty mmapped pages.  Must
				 * flush preallocated pages to avoid
				 * corrupting UBC LRU's. */
...
)
{
	...
	*vdParams = bsNilVdParams;
	...
	/* If this is a special 'chvol -A', then the user is to trying to
	 * clean up after an interrupted rmvol command. Go ahead and read from
	 * the disk. */
...
)
{
	...
	while ((vdi <= BS_MAX_VDI) && (vdcnt <= dmnP->vdCnt)) {
		...
		if (vdp = vd_htop_if_valid(vdi, dmnP, TRUE, FALSE)) {
			...
			/* total mcells (number of pages times mcells per
			 * page) */
			bmtap = vdp->bmtp;

			lock_read(&(bmtap->xtntMap_lk));
			/*
			...
			lock_done(&(bmtap->xtntMap_lk));
			/* decrement vdRefCnt on vdp before going on to next
			 * disk */
			    vd_dec_refcnt(vdp);
			vdcnt++;
		}
...
)
{
	...
	/* Bump the vdRefCnt on this vdp so it can't be removed while we are
	 * examining it.  Decrement the refcnt before returning. */
	if (!(vdp = vd_htop_if_valid(vdIndex, dmnP, TRUE, FALSE))) {
		return EBAD_VDI;
	}
	*smsyncQ_cnt = 0;
