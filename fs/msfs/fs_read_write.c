...
)
{
	...
	/* Optimization: if all the pages being allocated are going to be
	 * overwritten in fs_write() and there is no reason to desire zeroing
	 * these pages with immediate flushing to disk, then return. */
	...
	for (pg = start_pg; pg <= end_pg; pg++) {
		...
		if (!force_immediate_write) {
			if (pg == start_pg) {
				...
				/* skip any subsequent pages that will be
				 * overwritten */
	...
	*data_logging = (((bfap->dataSafety == BFD_FTX_AGENT) ||
	...
	if (future_file_size > file_size_limit) {
		...
		*efbig = residual;	/* return bytes not to be written */
		uio->uio_resid -= residual;	/* decrease size of transfer
						 * request */
...
#define BSD_MODE(vp) (((vp)->v_mount->m_flag & M_GRPID) && !sys_v_mode)
	...
	/* If a domain flush is pending, we don't want this I/O to get started
...
_retry:
	...
	if (sparse_alloc || (f_offset + nbyte > size) || (ioflag & IO_APPEND)) {
		...
		/* test for a growing file with a hole and preallocated blocks
		 * at EOF */
		...
		if ((orig_offset > (bfap->file_size + end_of_nxtpage)) &&
			...
			/* At the end of the file that is growing only -
			 * (indicated by f_offset >= size), there is a hole
			 * with preallocated pages filling at least part of
			 * the hole.  We need to remove the prealloced pages
			 * otherwise a block count (du) will show too many
			 * pages for a sparse file.  - fixes a vrestore
			 * problem
			 * 
			 * The byte to be written is not within the current page
			 * or the next page (we wouldn't want to truncate if
			 * it was) AND there already is some preallocated
			 * pages from the previous write to this file --- so
			 * lets truncate the preallocated pages so the hole is
			 * not filled with anything (including a 0'ed,
			 * prealloced page)
			 * 
			 * NOTE:  if seeking into a sparse file and writing,
			 * there may still be preallocated blocks in the
			 * holes, because this code is not run in that case. */

			if (directIO) {
				/* Allocate an active range structure; needed
				 * to prevent racing directIO writers to the
				 * range of pages we are going to remove
				 * storage for. */
				arp = (actRangeT *) (ms_malloc(sizeof(actRangeT)));

				/* Set up active range to include all
				 * underlying pages. Use same calculation here
				 * as in bf_setup_truncation. */
				pagesize = ADVFS_PGSZ;
			 ...
			 * is created or the quota is explicitly set, that
			 * storage may be added to the quota file
			 * 
			 * The potential danger path is bf_setup_truncation -->
			 * stg_remove_stg_start --> fs_quota_blks --> dqsync
			 * --> rbf_add_stg but it is tame in this case. */
		...
		/* we get a recursive read lock in case we fault on the
		 * uiomove() below and enter msfs_getpage() */
	 ...
	 * storage, then seize an active range to cover the file region where
	 * the storage will be added.  This will prevent a race with other
	 * threads trying to read this region while the storage is being
	 * added. */
	if (directIO && need_stg_alloc) {
		...
		if (ioflag & IO_APPEND) {
			/* We are APPENDing and the file is opened for
			 * directIO, so there are some added complications
			 * because the directIO path drops the file_lock,
			 * which O_APPEND depends on to protect the
			 * bfap->file_size.  This seizes an active range from
			 * EOF to infinity. */
	...
	_retry_append_range:
			...
			/* This call can cause file_lock to be dropped
			 * temporarily */
			insert_actRange_onto_list(bfap, arp, contextp);

			/* update these in case file size changed while
			 * waiting */
			f_offset = size = bfap->file_size;
			uio->uio_offset = size;

			/* When this returns, we have the range from the old
			 * eof to infinity, which will be OK to use for this
			 * write so long as the eof did not DECREASE while we
			...
			/* Normal write, seize an active range covering pages
			 * within the requested write region.  This is needed
			 * to prevent a dio reader that is expecting to read
			 * the hole we are writing into from seeing the
			 * storage we are about to add.  The reader may
			 * already have the active range and have dropped the
			 * file lock, so we must be prevented from adding the
			 * storage until he has completed his read. */
			...
			/* The active range seized here will be released at
			 * the end of this routine if we are doing a normal
			 * directIO, or in IO completion if doing an AIO.
			 * There can be only one asynchronous I/O started via
			 * an AIO call, but there can be many calls to
			 * fs_write_direct() for normal DIO calls, and we want
			 * to hold this range across all of them. */
		}
	}
	...
	/* We maintain tmp_starting_byte and tmp_bytes_written counters to
	 * loop through all the pages to be written so we can add any missing
	 * storage even though no actual writes are done yet. We need the loop
	 * below to add storage for the case when there is a page with
	 * associated storage between two holes. */
	...
	while (tmp_bytes_left > 0) {
		...
		if (!page_is_mapped(bfap, tmp_page_to_write, &nextPage, TRUE)) {
			...
			/* only allocate up to extent size to prevent quota
			 * problems */
			...
			if (ret != EOK) {
				/* error code must be saved for pinpg loop and
				 * cfs */
				error = ret;

				/* If some or all the requested storage was
				 * either allocated or already available then
				 * proceed to write what we have by adjusting
				 * the values to be written to avoid both an
				 * E_PAGE_NOT_MAPPED failure and any trailing
				 * unaligned DIO write if the preceding
				 * aligned write won't happen.
				 * 
				 * Partial writes are not allowed for IO_SYNC but
				 * we also don't want to make a larger window
				 * for object reuse so if we had a sparse
				 * allocate, write the data before reporting
				 * failure, else we can just fail with no
				 * write. */
				...
				if (directIO && (uio->uio_rw == UIO_AIORW)) {
					/* if AIO, then remember that this IO
					 * request was truncated.  The
					 * aio_bp->b_resid must return at
					 * least the number of bytes in
					 * tmp_bytes_left.  This value may be
					 * increased later in
					 * bs_pinpg_direct() if other parts of
					 * the I/O fail. */
					struct buf *aio_bp =
		 ...
		 /* endif ( !page_is_mapped() ) */ 
			...
			/* We come here if we are truncating and the page is
			 * mapped. So, we avoid copying 0 to the page. */
		...
		if (pages_allocated) {
			/* increment by the number of pages allocated, if
			 * possible. */
			tmp_number_to_write = MIN(tmp_bytes_left,
			    (pages_allocated * ADVFS_PGSZ) - tmp_starting_byte);
		} else {
			/* Use the value returned from page_is_mapped() to
			 * find the start of the next extent. */
			tmp_number_to_write = MIN(tmp_bytes_left,
	...
	while (bytes_left > 0) {
		...
		/* Count the number of sequentially written pages for
		 * aggressive write flushing below. Reset the counter whenever
		 * a nonsequential page is written. */
		...
		if (directIO) {
			...
			/* normal cached write path.  Call bs_pinpg() if we
			 * haven't already checked for cowing; otherwise call
			 * bs_pinpg_int() which will not try to cow the page. */
		...
		if (!(directIO)) {
			...
			/*
				...
				/* Aggressively flush some of the file's dirty
				 * data when the UBC exceeds its dirty cache
				 * threshold limit and enough pages have been
				 * sequentially written. This helps avoid
				 * draining the UBC cache too quickly when the
				 * amount of sequentially written data exceeds
				 * available cache memory and the I/O can not
				 * keep up. That would potentially put the
				 * system into a low memory (paging) state
				 * causing poor response time to processes.
				 * 
				 * Turning this feature off delays cache writes
				 * until smoothsync or UBC initiates a flush. */
				if (aggressiveWriteFlush &&
					...
					if (vc->vm_free_count < vc->vm_prewrite_target ||
						...
						lsnT seq;
						bfap->seqWritePgCnt = 0;
						/* Flush some of the file's
						 * dirty pages to disk and
						 * synchronously wait for the
						 * IO completion. */
		...
		if (bytes_left != 0) {
			...
			if (data_logging && bytes_pinned == ADVFS_PGSZ) {
				...
				ftx_started = TRUE;
			}	/* endif (data_logging && bytes_pinned ==
				 * ADVFS_PGSZ) */
		} else {	/* (bytes_left == 0) */
			if (data_logging) {
		 ...
		 * interface, we must not call back into fs_write_direct()
		 * because the aio_completion routine has already been called,
		 * and it can only be done once.  Instead, return to the user
		 * with the number of bytes successfully transferred. */
		if (directIO && uio->uio_rw == UIO_AIORW) {
			break;
		}
	}			/* endwhile pinpg/uiomove/unpinpg loop */

	/* If we created an active range in this routine and passed it into
	 * fs_write_direct(), release the active range now that the write has
	 * been completed.  One exception to this is if we started an
	 * asynchronous I/O via AIO where the active range will be released in
...
_no_zero_exit:
		if (data_logging) {
			...
			/* flush data for either O_SYNC or O_DSYNC. Skip if
			 * doing directIO since those writes are implicitly
			 * synchronous, and there are no buffers in the cache
			 * to flush. */
			if (!directIO) {
			...
			/* Update stats if applicable. Prior to the
			 * implementation of the strict-posix-osync sysconfig
			 * variable(vfs), AdvFS would not flush stats if
			 * dirty_alloc was FALSE.  This was in violation of
			 * POSIX (and a bug).  So, we need to uphold that
			 * default behavior in order to maintain consistant
			 * performance between releases.  The customer must
			 * set strict-posix-osync to true to comply with the
    ...
    actRangeT * arp)
{
	...
	/* If there are multiple uio->uio_iov[] structures we are only going
	 * to process the first one, update the uio structure, and return
	 * allowing fs_write() to call us again if need be to process the rest
	 * of the uio->uio_iov[] structures.  So if *number_to_write is
	 * greater than the iov_len, then use the iov_len.
	 * 
	 * If this code is changed so that num_to_write is allowed to be greater
	 * than the iov_len, all the other areas of this routine that
	 * increment or decrement the iov_base and iov_len fields will need to
	 * be fixed so that they can handle an array of uio->uio_iov
	 * structures (readv()/writev() syscalls create them). */
	...
	*number_to_write = 0;
	 ...
	 * directly transfer regardless of size.  Underlying pages need to
	 * have storage allocated to them. */
	...
	if (!arp_passed_in) {
		if (optimize_output_path) {
			/* set up active range to include just underlying
			 * sectors */
		...
		/* Put the active range onto the list to prevent concurrent
		 * use by migrate or other directIO threads.  Because the
		 * file_lock can be dropped while inserting the active range
		 * onto the list we may now have a page in our range that is
		 * no longer mapped. Deal with this by adding that storage
		 * back in blkmap_direct(). If the actRange struct was passed
		 * in to us, it has already been put onto the activeRangeList,
	...
	if (optimize_output_path) {
		 ...
		 * Setup # bytes to transfer and the user's buffer address.
		 * We also adjust uio fields here.  If an error occurs, the
		 * fields will be corrected.  This complies with the way the
		 * rest of the routine is set up. */
		nbytes = num_to_write;
	...
	if (byte_offset_in_page) {
		...
		if ((ret == EOK) && (bytes_written == xfer_bytes)) {
			...
			/* If the entire page wasn't written then something is
			 * wrong! */
			if (ret == EOK)
				ret = EIO;
			/*
		...
		if ((!pg_mapped) || ((ret == EOK) && (bytes_read == xfer_bytes))) {
			...
			if (!ret) {
				...
				if ((ret != EOK) || (bytes_written != xfer_bytes)) {
					...
					/* If we are doing an AIO transfer,
					 * remember that the trailing
					 * unaligned write failed.
					 * bs_pinpg_direct() will need to know
					 * this if there is an aligned write
					 * because it will need to calculate
					 * aio_bp->b_resid, and it needs to
					 * know if this was successful or not. */
	...
	if (nbytes) {
		...
		if (uio->uio_rw == UIO_AIORW) {
			if (aio_bp->b_driver_un_1.longvalue != 2 &&
			    aio_bp->b_driver_un_1.longvalue != 3) {
				/* Set the resid field to the count from the
				 * trailing unaligned write, whether or not
				 * that write was successful.  If only part of
				 * the aligned write makes it out to disk,
				 * trailing unaligned write must NOT be
				 * included in the transfer size since it
				 * occurs after the aligned write in the file.
				 * If this IO request was truncated (longvalue
				 * == 2) so no trailing request was done, we
				 * need to leave aio_bp->b_resid alone so it
				 * contains the # of bytes that were truncated
				 * from the request.  We also don't change
				 * aio_bp->b_resid if the IO request is to
				 * prevent object reuse on a synchronous write
				 * that had a sparse storage allocation error
				 * (longvalue ==  3). */
				aio_bp->b_resid = trailing_bytes;
			}
		}
		/* Write aligned disk blocks.  "page_to_write" is really
		 * "block to write" for direct I/O. */
...
EXIT:
	...
	/* Release any active range that we allocated in this routine if we
	...
	/* If this is an AIO write, we need to let msfs_strategy() know when
	 * there is still an outstanding I/O.  When this I/O completes
...
fs_write_add_stg(
    ...
    unsigned long bytes_to_write,	/* in - bytes that remain to be
					 * written */
...
)
{
	 ...
	 * 
	 * The potential danger path is chk_blk_quota --> dqsync --> rbf_add_stg
	...
	/* when quotas are being enforced we check those limits before doing
	 * the storage allocation, otherwise we allocate first so we know the
	 * exact amount we got and we don't need to fix the quota counts
	 * afterwards. */
	if (quotas_on) {
	...
	/* storage allocator doesn't have "give_me_what_you_can" flag so until
	 * that happens do the ugly ENO_MORE_BLKS retry logic here. 1st -
	 * pages including any preallocation 2nd - pages caller requested 3rd
	 ...
	 * frag, we adjust the quotas based on that page growing only
	 * 16-frag_quota (the additional space).
	 * 
	 * when quotas are enforced and we didn't allocate as many pages as
	 * charged, we have to release the excess charged blocks from the
	 * quota system.  just to be safe in case the allocator gave us more
	 * than the requested pages (probably illegal), we also handle
	 * increasing the charged quota blocks. */
	if (quotas_on)
	...
	if (bfap->fragState == FS_FRAG_VALID && fragPage >= first_pg_to_add) {
		if (quotas_on == FALSE) {
			/* only have to remove frag quota count if we spanned
			 * it */
			 ...
			 * allocation could miss the frag page we already
			 * counted */
	...
	if (pgs_added > 0) {
		...
		/* Figure out how many of the pages we just allocated will be
		 * completely overwritten back in fs_write().  Pass this
		 * information to fs_zero_fill_pages() so it can optimize if
		 * possible. */
		...
		if ((bfap->fragState == FS_FRAG_VALID) &&
			...
			if (fragPage < lastPageAdded) {
				...
				/* Adjust which pages will be overwritten if
				 * the frag page is in the middle of the range
				 * of pages being added. */
	...
	/* when quotas are not enforced we wait to do the bookkeeping last so
	 * that we know the actual allocation and if we fail the storage
	 * transaction nothing has to be undone.  when quotas are enforced we
	 * may still need an adjustment if the allocation didn't match what we
	 * intended to get.
	 * 
	 * we always force the change because it must be legitimate, so the only
	 * failure possible is an I/O error which is logged. */
	if (quota_change) {
...
)
{
	...
	int number_to_read;	/* limited to "int" due to b_bcount of "buf
				 * struct" */
	int error = 0;
	bfPageRefHT page_ref;
	char *page_addr;
	...
	pgStatT local_pg_stats, *pg_statp = NULL, *saved_pg_statp = NULL;
	int pg_stat_cnt;
...
_retry_map:
	...
	/* we get the read lock recursive in case we fault during the
	 * uiomove() below and enter msfs_getpage() */
	FS_FILE_READ_LOCK_RECURSIVE(contextp);
	...
	/* If we don't have to read the frag page at all, set up to skip the
	 * special frag adjustment for DIO at the top of the next loop. */
	...
	while (bytes_left > 0) {
		...
		/* If we are doing a read of a directIO file via the AIO
		 * interface, and the file has a frag, then we want to read
		 * the frag before allowing the asynchronous IO to be spawned.
		 * This will prevent a thread from seeing the AIO completion
		 * and using the data in the user's buffer before the frag
		 * data has been transferred. When appropriate, this paragraph
		 * is entered twice: first to read the frag, and again to
		 * reset the values for the rest of the pages to be read. */
		if (directIO &&
		    (uio->uio_rw == UIO_AIORW) &&
		    (fragPageOffset != outOfRangePage) &&
		    (already_read_frag < 2)) {
			if ((already_read_frag == 0) && (page_to_read == fragPageOffset)) {
				/* reading only the frag; reordering code not
				 * needed */
				already_read_frag = 2;
			} else if (already_read_frag == 0) {
				/* setup to read frag page before the rest of
				 * the pages which will be done via AIO.  Save
				 * off the starting values to restore on next
				 * pass. */
				...
				/* reset original values to read non-frag
				 * pages. Some values are adjusted for the
				 * bytes already read. */
				struct buf *aio_bp;
				...
				already_read_frag = 2;	/* don't come in here
							 * again */
			}
		}
		if (page_to_read != fragPageOffset) {
			...
			if (bfap->origAccp) {
				/* If we are doing a directIO read of this
				 * clone via the normal interface (not AIO),
				 * then we must map the user's pages down now
				 * before we take out the trunc_xfer_lk.  This
				 * resolves a lock hierarchy violation between
				 * the trunc_xfer_lk and the vm_umap.vm_lock
				 * which is seized in vm_map_pageable(). */
				if (directIO &&
				    (uio->uio_rw != UIO_AIORW) &&
				    (uio->uio_segflg == UIO_USERSPACE)) {

					/* We need to protect the pages
					 * associated with each
					 * uio->uio_iov->iov_base.  Typically
					 * there will be only one of these,
					 * but if the user called in via
					 * readv(), there may be more, so we
					 * must loop through all here.  We
					 * must also save these address so we
					 * can unprotect them later.  */
			...
			if (bfap->origAccp) {
				 ...
				 * for storing the page extents.  This is done
				 * for directIO reading of clones only. */
			...
			if (bfap->fragState == FS_FRAG_VALID) {
				...
				if (number_to_read > fragByteCnt) {
					...
					/* If this ASSERT is ever taken, turn
					 * off ASSERTs (sysconfig -r advfs
					 * AdvfsEnableAsserts=0) as a
					 * workaround and then remove this
					 * ASSERT from the pools.  This has
					 * been placed here under the
					 * hypothesis that filling zeros
		...
		/* if ioflag contains IO_RSYNC, POSIX 1003.1b requires that
		 * any pending writes be written out to disk prior to the
		 * read.  We first call msfs_syncdata which does a flush of
		 * the buffer. */
		if (ioflag & IO_RSYNC) {
    ...
    unsigned long byte_offset_in_page)
{
	...
	/* Put the active range onto the list to prevent concurrent use by
	 * migrate or other directIO threads. */
	...
	/* Does the read start on a block boundary?  If not, then we must read
	 * the unaligned portion (up to first block boundary) into a temporary
	 * buffer, and then copy it to the user's buffer. */
	...
	/*
         ...
         */
	if (middle_bytes) {
		/* Adjust the uio struct back for middle bytes read.  This
		 * means making the uio struct appear that the read of the
		 * trailing bytes never occurred.  Later, if the read of the
		 * middle blocks succeeds, we will update the uio struct so it
		 * appears, on return, the way it would if the read were done
		 * as a unit. */
		if (trailing_bytes) {
		...
		if (ret == EOK && bs_number_read > 0) {
			...
			/* If we had to do a separate read of the
			 * non-sector-sized trailing bytes above, account for
			 * that here before returning, If and Only If, we
			 * successfully transferred all the intermediate
			 * sectors.  Yes it is a waste to redo the partial
			 * sector if we need to come back, but that should be
			 * such a rare event that it isn't worth worrying
			 * about. */
...
EXIT:
	...
	*nbytes = bytes_transferred;
	 ...
	 * otherwise, we will have to clean up after the asynchronous I/O has
	 * completed. */
...
)
{
	...
	if (sts != EOK) {
		if (sts != E_PAGE_NOT_MAPPED)
			 ...
			 * the space then the quota changes will be undone by
			 * the ftx_fail */
			if (quotas_on) {
			...
			/* when quotas are off we just force the growth
			 * adjustment now since the only possible failure is
			 * an I/O error */
			if (!quotas_on)
