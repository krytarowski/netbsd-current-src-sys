...
_error:
	if (rootOpen)
		(void) bs_close(rootbfap, 0);
	if (tagsOpen)
		(void) bs_close(tagsbfap, 0);
	if (userQuotaOpen)
		(void) bs_close(userQuotabfap, 0);
	if (groupQuotaOpen)
		(void) bs_close(groupQuotabfap, 0);
...
)
{
	...
	/* The following is to allow backward compatablility. Since a field
	 * was added to the end of the undo record, old versions of the kernel
	 * will not be effected since the size was never verified by this
	 * routine.  The old kernels will just not know of the additional
	 * field. For a new kernel running recovery on an old domain, the
	 * field will be initialized to a benign value (zero) and then the
	 * passed in stucture will be bcopied in, only overwritting the
	 * addtional field if it exists. For future compatabilty the min of
	 * the passed in size and the size of the expected strucutre will be
	 * bcopied. */
...
*/
	...
	/* Since this is an undo routine we can not call the index routines
	 * because they start ftx's. Indicate to the glom routine to not try
	 * to recover lost space if found */
	...
	/* Need this for indexing to work, but also should be here for
	 * non-indexing, otherwise dir space could be lost. */
...
finish:
	...
	if (in_rec.undo_header.old_size > 0) {
		 ...
		 * (under rare conditions during a rename and low space and
		 * indexes) This fortunately we can detect by examining the
		 * alloc page count since the remove_stg_undo has already run
		 * (it is the last thing we do in insert_seq and therefor the
		 * first thing to be undone. */
...
fs_cleanup_thread(void)
{
	...
	while (TRUE) {
		...
		case CLEANUP_CLOSED_LIST:
			 ...
			 * ClosedAcc list/set list to the FreeAcc list. Called
			 * by get_free_acc and bfs_close(). These are really
			 * BS level functions, but this is a convenient place
			 * to call it, so I am. */
		...
		case DEALLOCATE_BFAPS:
			...
			if (((FreeAcc.len > 0) && (NumAccess > MaxAccess)) ||
				...
				if (mutex_tryenter(&FreeAcc.freeFwd->bfaLock.mutex.mutex)) {
