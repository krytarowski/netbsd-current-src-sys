...
)
{
	...
	if (dmnP->state == BFD_ACTIVATED) {
		...
		/* The bfap->bfaLock must be released because it is reseized
		 * in a lower layer (bs_pinpg_one_int).  Set the state
 ...
 */
	...
	if (!MCIDEQL(hrp->nextMCId, bsNilMCId)) {
		if ((sts = pin_link(vdp->bmtp, hrp->nextMCId, &frp, vdp, ftxH)) != EOK) {
			/* Don't let DDL corruption panic the domain: instead,
			 * reset the DDL to empty and continue, but issue a
			 * message to console. This is consistent with what
			 * del_remove_from_del_list() does. */
...
)
{
	...
	if (vdp->bmtp->nextPage <= mcellListPage) {
		/* Since the migrate mcell inuse listhead is maintained on
		 * page MCELL_LIST_PAGE of the BMT, we must assume that the
		 * list is empty if this page doesn't exist. */
		return (EOK);
	}
	...
	while (!MCIDEQL(mcellId, bsNilMCId)) {
		...
		if (clu_is_ready() && (flag & M_FAILOVER)) {
			...
			if (bfSetp == NULL) {
				...
				if (bfap == NULL) {
					...
					/* Since the fragfile (tag 1) is
					 * opened on fset mount and closed on
					 * umount, we will always find a bfap
					 * for it. During failover, if we find
					 * these mcells on the DDL, we must
					 * delete them, otherwise they will
					 * stay on the DDL until the next
					 * cluster shutdown. This is because
		...
		if (delete_this_mcell) {
			 ...
			 * will remain in the primary mcell by calling
			 * bmtr_find(). In case this assumption changes, we'd
			 * need to follow the mcell chain when looking for the
			 * record. */

			/* Furthermore if we do not find a BMTR_FS_STAT record
			 * then we will assume that the mcell is a psuedo
			 * mcell from a truncation which will not have
			 * associated frags, and therfore can be skipped */
...
filesetnode_init(bfSetT * bfSetp, fileSetNodeT ** fsnpA)
{
	...
	for (fsp = FilesetHead; fsp != NULL; fsp = fsp->fsNext) {
		if (BS_BFS_EQL(bfSetp->bfSetId, fsp->bfSetId)) {
			FILESET_UNLOCK(&FilesetLock);
			/* For non cluster, we are the only accessor of this
			 * fs. */
...
)
{
	...
	/* a clone, then give the extents to the clone instead of freeing
	 * them. */
...
)
{
	...
	FTX_LOCKWRITE(&vdp->stgMap_lk, dsc.ftxH)
		...
		while (1) {
			...
			if (pinPages > 0) {
				/* This break is a little tricky.  It relies
				 * on this inner while loop being the last
				 * statement in the enclosing for loop.  Thus,
...
)
{
	...
	if (cloneap == NULL) {
		...
		xfererr = TRUE;
		orig_sts = sts;
		/* Since we are going to discard all these pages, we don't
		 * want to */
		/* interpret them as part of a striped extent. The xtntType
		 * test */
		/* at the start of the "for each page in extent" loop needs
		 * info */
	...
	if (cloneap && cloneap->xtnts.type == BSXMT_STRIPE) {
		...
		/* This is the situation where the clone was created before
		 * the file was striped. The original may have had pages, been
		 * cloned, truncated, then striped. In any case all
		 * maxClonePgs are mapped by the clone. */
		...
		for (mapi = dsc.rstIndex; mapi < xCnt - 1; mapi++) {
				...
				if (xfererr == FALSE &&
					...
					if (pgstofree) {
						del_part_xtnt(pmcid,
						    ...
						    &dsc);
						pgstofree = 0;
						startpg = pg;	/* save the first page
								 * to COW */
			...
			if (pgstoadd) {
				...
				if (sts != EOK) {
					...
					xfererr = TRUE;
					orig_sts = sts;
					/* free the pages we failed to add to
					 * the clone */
					if (startBlk != PERM_HOLE_START) {
						del_part_xtnt(pmcid,
						    pvdp,
	...
	if (cloneap == NULL) {
		/* clone was deleted as part of a delete fileset before we got
		 * here */
    ...
    delFtxDescT * dscp)
{
	...
	while (1) {
		...
		/* del_range will change blksfreed if not all blocks are
		 * freed. */
		blksfreed = numblk;
